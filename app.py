...
with tab_telefony:
    ...
    # ══ 6b1. DIAMENTY Z TELEFONÓW ═══════════════════════════════════════════════
    st.markdown("### 💎 Telefony zakończone kurierem")
    st.caption("Ile rozmów doprowadziło do zamówienia kuriera — czyli do celu, po który dzwonimy. "
               "Diament liczy się tylko wtedy, gdy dla zamówienia powstał REALNY kurier, "
               "a nie gdy w rozmowie sama padła zapowiedź.")
    # 🟥 Liczymy z SAMYCH telefonów: ile rozmów doprowadziło do zamówienia kuriera.
    #    Bez wiązania z Diamentozą — to miara pracy telefonicznej, nie księgowość kurierów.
    _diam_nr = None

    # jeden diament na zamówienie — ostatnia rozmowa z kurierem
    _kur_by_nr = {}
    for _c in _calls:
        _nr_k = str(_c.get("numer_zamowienia") or "").strip()
        if not _c.get("kurier_ustalony") or not _nr_k:
            continue
        _prev = _kur_by_nr.get(_nr_k)
    ...
    _d1.metric("💎 Diamenty z telefonów", len(_diamenty))
    ...
    if _diamenty:
        ...
        for _c in _diamenty:
            ...
            _a = _rows_d.setdefault(_k2, {"Kto": _kto_disp(_c), "Kraj": _kraj(_c),
                                          ...
                                          "Diamenty": 0})
            _a["Diamenty"] += 1
        st.dataframe(pd.DataFrame(sorted(_rows_d.values(), key=lambda r: -r["Diamenty"])),
                     use_container_width=True, hide_index=True)
        # rozbicie na kraje
        _kr_d = {}
        for _c in _diamenty:
        ...
        st.caption("Brak potwierdzonych diamentów z telefonów w tym zakresie.")

    if _zapowiedzi:
        st.caption(f"⚠️ {len(_zapowiedzi)} rozmów zapowiadało kuriera, ale realnego kuriera "
                   f"dla tych zamówień jeszcze nie ma — warto sprawdzić, czy zlecenie poszło.")
