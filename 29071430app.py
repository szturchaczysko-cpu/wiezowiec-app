...
with tab_telefony:
    ...
    st.caption("**Jakość** = % rozmów z ustaleniami (czy jak dzwoni, to coś z tego wynika). "
               "**Wykonalność** = ile z tego, co mógł obdzwonić, wziął na siebie zamiast oddać dalej. "
               "Tylko osoby, które danego dnia były USTAWIONE jako dzwoniące — niedzwoniący tu nie wchodzą.")
    ...
    for _k in _kraje:
        ...
        for (_op, _gr) in sorted(_dzw_set):
            if _gr != _k:
                continue
            _lst = [c for c in _calls if not _grupowy(c) and _kto(c) == _op and _kraj(c) == _k]
            _lang = [str(x).upper() for x in _jez_map.get((_op, _gr), [])]
            _oddane = [z for z in _zlecenia if z.get("zlecil") == _op and _kraj(z) == _k
                       and (not _lang or str(z.get("jezyk") or "").upper() in _lang)]
            _mian = len(_lst) + len(_oddane)
            _zu = sum(1 for c in _lst if _z_ust(c))
            _rows_w.append({"Kto": _op, "Typ": f"operator {_k}", "Rozmowy": len(_lst),
        ...
        for _g in sorted({_kto(c) for c in _calls if _grupowy(c) and _kraj(c) == _k}
                         | {str(z.get("do_kogo") or "") for z in _zlecenia if _kraj(z) == _k and z.get("do_kogo")}):
            _lst = [c for c in _calls if _grupowy(c) and _kto(c) == _g and _kraj(c) == _k]
            _do_niej = [z for z in _zlecenia if str(z.get("do_kogo") or "") == _g and _kraj(z) == _k]
            _przejete = [z for z in _do_niej if z.get("_call") and not _grupowy(z["_call"])]
        ...
        with _w1:
            st.caption("Jakość %")
            st.bar_chart(_dfw.set_index("Kto")[["Jakość %"]], height=210)
        with _w2:
            st.caption("Wykonalność %")
            st.bar_chart(_dfw.set_index("Kto")[["Wykonalność %"]], height=210)
