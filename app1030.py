...
with tab_telefony:
    ...
    for _k in _kraje:
        ...
        for _v in sorted(_agg.values(), key=lambda x: -x["Telefony"]):
            _rows.append({**_v, "Zlecił": _zlec_k.get(_v["Kto"], 0),
                          ...
                          "Skuteczność": f'{round(100*_v["Z ustaleniami"]/_v["Telefony"])}%' if _v["Telefony"] else "—"})
    ...
    # ══ 2. TELEFONY WYKONANE ════════════════════════════════════════════════════
    _K1 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "KTO DZWONIŁ (grupa)", "KTO KONKRETNIE (osoba)",
           "DO KOGO", "STATUS", "CZY W TERMINIE", "PRÓBA", "GRUPA", "UWAGI"]
    ...
    for _c in sorted(_calls, key=_mom, reverse=True):
        ...
        _r1.append({
            ...
            "KTO DZWONIŁ (grupa)": _kto(_c),
            "KTO KONKRETNIE (osoba)": _osoba(_c),
            "DO KOGO": _do_kogo,
            "STATUS": _status(_c),
    ...
    if _tel_all:
        ...
        for _c in _tel_all:
            ...
            _a = _ti.setdefault(_key, {"Grupa": _kto(_c), "Osoba": _key[1], "Kraj": _kraj(_c),
                                       "Telefony": 0, "Z ustaleniami": 0, "Nieodebrane": 0, "Nieproduktywne": 0})
            _a["Telefony"] += 1
            if _z_ust(_c):
                _a["Z ustaleniami"] += 1
            elif (_c.get("wynik") or "") in ("nieodebral", "brak_kontaktu"):
                _a["Nieodebrane"] += 1
            else:
                _a["Nieproduktywne"] += 1
        _rows_ti = []
        for _v in sorted(_ti.values(), key=lambda x: (x["Kraj"], -x["Telefony"])):
            _rows_ti.append({**_v, "Skuteczność": f'{round(100*_v["Z ustaleniami"]/_v["Telefony"])}%'
                             if _v["Telefony"] else "—"})
