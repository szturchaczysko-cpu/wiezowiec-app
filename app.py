...
with tab_telefony:
    ...
    for _ds in _oc_dni:
        try:
            for _d in db.collection(col("ew_phone_log")).document(_ds).collection("calls").stream():
                ...
                if (_wd and _wd in _dni_set) or (not _wd and _ds in _dni_set):
    ...
    if _do_oceny:
        with st.expander(f"✍️ Oceń rozmowy ręcznie ({len(_do_oceny)})", expanded=True):
            st.caption("Rozmowy zapisane z forum, którym Szturchacz nie nadał jeszcze statusu. "
                       "Zwykle uzupełni się sam przy otwarciu sprawy — tutaj możesz to zrobić od ręki, "
                       "żeby nic nie zostało bez domknięcia.")
            _OPCJE = {
