...
with tab_telefony:
    ...
    for _z in _deleg_ext:
        ...
        for _c in _cbn.get(_nr_z, []):
            ...
            _po.append(_c)
        _po.sort(key=_mom)
        _z["_call"] = _po[0] if _po else None
        _z["_call_wcz"] = None
    ...
    # ══ 2. TELEFONY WYKONANE ════════════════════════════════════════════════════
    _K1 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "KTO DZWONIŁ", "STATUS", "GRUPA", "UWAGI"]
    st.markdown("### ☎️ Telefony wykonane przez operatorów")
    st.caption("Wyłącznie telefony operatorów dzwoniących — wszystkie, także te zakończone wpisem na forum. "
               "Co działo się PO wpisie (kto podjął temat) pokazuje tabela zleceń niżej. "
               "Rozmowy telefonistek są w zleceniach oraz w tabeli „Telefonistki imiennie\".")
    # Telefon operatora sprzed JAKIEGOKOLWIEK zlecenia dla tej sprawy — czyli pierwszy ruch.
    # Telefon pobrany z woreczka poprzedza zlecenie, więc należy do tabeli zleceń niżej.
    def _przed_wpisem(_c):
        _nr = str(_c.get("numer_zamowienia") or "").strip()
        return not any(_mom(_d) < _mom(_c) for _d in _dbn.get(_nr, []))
    ...
    def _jezyki_osoby(_c):
        """Przypisane języki operatora w grupie, w której wtedy pracował (z obsady dnia)."""
        return {str(j).upper() for j in _jez_map.get((_kto(_c), _kraj(_c)), [])}

    def _telefon_wiarygodny(_c):
        """🟥 Operator mógł wykonać ten telefon tylko w JĘZYKU, który ma przypisany.
        Osoba dzwoniąca po PL, pracująca w DE, nie wykonuje niemieckich rozmów — takie
        wpisy (z danych sprzed poprawki) nie mogą jej obciążać ani zasilać podsumowań."""
        _lg = _jezyki_osoby(_c)
        _js = _jez_calla(_c)
        if not _lg or not _js:
            return True                      # brak danych → nie wyrokujemy
        return _js in _lg

    # 🟥 ŻADEN wykonany telefon nie może zniknąć — jeśli ktoś zadzwonił, musi być przypisany
    #    i oceniony. Niezgodność języka jest OSTRZEŻENIEM przy wierszu, nie powodem ukrycia.
    # 🟥 WSZYSTKIE telefony operatorów — także te po zleceniu (pobrane z woreczka) i nieodebrane.
    #    Operator poświęcił czas, więc ślad po jego pracy zostaje tutaj; kontekst jest w UWAGACH.
    _calls_op = [c for c in _calls if not _grupowy(c)]
    _podejrzane = [c for c in _calls_op if not _telefon_wiarygodny(c)]
    _r1 = []
    for _c in sorted(_calls_op, key=_mom, reverse=True):
        _g = _grupowy(_c)
        # po nieudanym telefonie operatora idzie WPIS na forum → sprawa do woreczka.
        _po_wpis = None
        if not _g:
            # Wpis na forum zapisuje się przy wysyłce, a wynik telefonu przy rejestracji —
            # wpis bywa więc o kilka minut WCZEŚNIEJSZY. Dopuszczamy okno 15 minut wstecz.
            _t_c = _dt(_c)
        ...
        if _g:
            _do_kogo, _id_postu = _kto(_c), "forum"
        elif _po_wpis:
            _do_kogo, _id_postu = "do woreczka", (_po_wpis.get("id_postu") or "—")
        else:
            _do_kogo, _id_postu = _kto(_c), "sesja"
        _r1.append({
            "NR ZAMÓWIENIA": _c.get("numer_zamowienia", ""),
            "ID POSTU": _id_postu,
            "DATA / GODZINA": _mom(_c),
            "KTO DZWONIŁ": _kto_disp(_c),
            "STATUS": _status(_c),
            "GRUPA": _kraj(_c),
            "UWAGI": ((f'wykryte {_c.get("wykryte_dnia")} · '
                       if (_c.get("wykryte_dnia") and _c.get("wykryte_dnia") != _c.get("_dzien")) else "")
                      + ("z woreczka (po zleceniu) · " if not _przed_wpisem(_c) else "")
                      + ("zakończony wpisem na forum → dalszy ciąg w zleceniach" if _po_wpis
                       else ("telefon owocny — sprawa domknięta" if _z_ust(_c) else "bez ustaleń"))
                      + ("" if _telefon_wiarygodny(_c)
                         else f' · ⚠️ język sprawy {_jez_calla(_c) or "?"} poza przypisanymi '
                              f'({"/".join(sorted(_jezyki_osoby(_c))) or "brak"}) — sprawdź ustawienia')),
        })
    _df1 = pd.DataFrame(_r1, columns=_K1)
    st.dataframe(_df1, use_container_width=True, hide_index=True)
    _arch = [c for c in _calls if (c.get("wynik") or "") == "archiwum"]
    if _arch:
        st.caption(f"📦 {len(_arch)} rozmów zarchiwizowanych — widoczne w tabelach i logu, "
                   f"ale poza skutecznością (nie dało się ich ocenić).")
    if _podejrzane:
        _lst_o = sorted({f'{_kto(c)} ({_kraj(c)}, {_jez_calla(c)})' for c in _podejrzane})
        st.caption(f"⚠️ {len(_podejrzane)} rozmów oznaczonych: język sprawy nie mieści się w przypisanych "
                   f"językach operatora. Telefony ZOSTAJĄ w rozliczeniu — sprawdź ustawienia języków "
                   f"albo popraw status ręcznie: " + ", ".join(_lst_o[:8]))
    if not _r1:
        st.caption("Brak telefonów operatorów w tym zakresie.")
    if _r1:
        st.download_button("⬇️ CSV — telefony", _df1.to_csv(index=False).encode("utf-8"),
                           file_name=f"telefony_{_t_from}_{_t_to}.csv", mime="text/csv", key="_csv1")

    # ══ 3. ZLECENIA NA FORUM ════════════════════════════════════════════════════
    _K2 = ["NR ZAMÓWIENIA", "ID POSTU", "ZLECONO", "DO KOGO", "PODJĘTE", "STATUS", "ZLECIŁ", "UWAGI"]
    st.markdown("### 📨 Zlecenia telefonu na forum")
    _r2 = []
    for _z in sorted(_zlec_only, key=_mom, reverse=True):
        ...
        if _cc:
            # Dwie jedyne możliwości: odpowiedź TELEFONISTKI albo operator, który pobrał z WORECZKA.
            # 🟥 Czas liczymy od wpisu, POD KTÓRYM odpowiedź faktycznie wisi.
            #    Jeśli to podbicie — od podbicia, bo ono było realnym wezwaniem do działania.
            ...
            if _t0_real and _t1:
                ...
                if _bm >= 0:
                    _dni_rob = int(_bm // 480)          # 8 godzin roboczych = 1 dzień
                    _godz_r = int((_bm % 480) // 60)
                    _delta = (f' (+{_dni_rob} dn. rob.)' if _dni_rob
                              else (f' (+{_godz_r}h {int(_bm%60)}min)' if _bm >= 60
                                    else f' (+{int(_bm)}min)'))
                    if _bm > 2 * 480:
                        _uw += " · po terminie"
            if _cc.get("_dzien") not in _dni_set:
                _uw += " · rozmowa spoza wybranego zakresu"
            if _z.get("_ile_rozmow", 0) > 1:
                _uw += f' · rozmów w sprawie: {_z["_ile_rozmow"]}'
            _podj = (f'TAK — {_kto(_cc)} / {_osoba(_cc)} {_mom(_cc)}{_delta}' if _grupowy(_cc)
                     else f'TAK — {_kto(_cc)} z woreczka {_mom(_cc)}{_delta}')
        elif _z.get("_pon", 0) > 0:
            # nikt nie zadzwonił, ale sprawa była ruszana — operator niedzwoniący podbił wpis
            _podj = f'⏳ PODBICIE ×{_z["_pon"]} — nikt nie zadzwonił (ost. {_z.get("_pon_ostatni","")})'
            # 🟥 2 dni ROBOCZE = 960 minut pracy (2 x 8h). Weekend się nie liczy —
            #    zlecenie z piątku 16:50 nie jest przeterminowane w poniedziałek rano.
            if _t0 and _biz_min(_t0, _now) > 960:
                _uw += " · ⚠️ przeterminowane (>2 dni robocze)"
        elif _z.get("anulowane"):
            _podj = "🚫 ANULOWANE — telefon niepotrzebny"
            _uw += (f' · anulowano {_z.get("anulowane_dnia","")}' if _z.get("anulowane_dnia") else "")
        else:
            _podj = "⏳ czeka na aktualizację"
            if _t0 and _biz_min(_t0, _now) > 960:
                _uw += " · ⚠️ przeterminowane (>2 dni robocze)"
        if _z.get("_pon", 0) and _cc:
            _uw += f' · podbite ×{_z["_pon"]}'
        _zl_gr = _jez_map.get((_z.get("zlecil"), _kraj(_z)))
        _r2.append({
            "NR ZAMÓWIENIA": _z.get("numer_zamowienia", ""), "ID POSTU": _z.get("id_postu", ""),
            "ZLECONO": _mom(_z), "DO KOGO": _z.get("do_kogo", ""),
            "PODJĘTE": _podj, "STATUS": _status(_cc),
            "ZLECIŁ": f'{_z.get("zlecil", "")} ({_kraj(_z)})',
            "UWAGI": _uw.replace("zlecenie · ", "").replace("zlecenie", "").strip(" ·") or "—",
        })
    _df2 = pd.DataFrame(_r2, columns=_K2)
    st.dataframe(_df2, use_container_width=True, hide_index=True)
    if _r2:
        st.download_button("⬇️ CSV — zlecenia", _df2.to_csv(index=False).encode("utf-8"),
                           file_name=f"zlecenia_{_t_from}_{_t_to}.csv", mime="text/csv", key="_csv2")
        _byz = {}
        for _z in _zlecenia:
