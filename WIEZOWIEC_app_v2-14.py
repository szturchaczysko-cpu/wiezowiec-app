...
with tab_generuj:
    ...
    if st.button("📊 Przygotuj partycje (bez przeliczania)", type="secondary"):
        ...
        st.session_state["_ew_batches_done"] = 0  # ile partii już przeliczonych
        ...
        st.success(f"📦 Przygotowano **{total_batches} partii** (po ~{BATCH_SIZE} zamówień). "
                   f"Łącznie {len(nowe_szturchacz_parts)} do przeliczenia"
                   + (f", {len(nrzam_gotowe)} gotowych z bazy" if nrzam_gotowe else "") + ".")
        st.rerun()
    ...
    if batches_to_process:
        ...
        for bi, bc in enumerate(batches_to_process):
            status = "✅" if bi < batches_done else ("⏳" if bi == batches_done else "⬜")
            st.caption(f"{status} Partia {bi+1}: {len(bc)} zamówień")
    ...
    # --- FUNKCJA ZAPISU CASÓW DO BAZY (per paczka) ---
    def _save_cases_to_db(batch_cases, batch_num, total_batches):
        ...
        for i, case in enumerate(batch_cases):
            ...
            # Pomiń jeśli operator pracuje
            if existing and existing["status"] in ("przydzielony", "w_toku"):
                skipped += 1
                continue
            
            # Usuń stary wolny/zakończony jeśli istnieje
            if existing and existing["status"] in ("wolny", "zakonczony"):
                db.collection("ew_cases").document(existing["doc_id"]).delete()
        ...
        db.collection("ew_batches").document(batch_id).set({
            ...
            "status": "active",
            "summary": f"Partia {batch_num}/{total_batches}: {saved} zapisanych, {skipped} pominiętych",
...
with tab_autopilot:
    ...
    ALL_OPERATORS = {
        "DE": ["Emilia", "Magda", "Iwona", "Marlena", "Sylwia", "EwelinaG", "Andrzej"],
        "FR": ["Emilia", "Magda", "Iwona", "Marlena", "Sylwia"],
        "UKPL": ["Oliwia", "Ewelina", "Klaudia", "Marta", "Iwona", "Marlena"],
    }
    ...
    with col_obs1:
        st.markdown("**🇩🇪 DE**")
        ap_ops_de = st.multiselect("Operatorzy DE:", ALL_OPERATORS["DE"], key="ap_ops_de")
    with col_obs2:
        st.markdown("**🇫🇷 FR**")
        ap_ops_fr = st.multiselect("Operatorzy FR:", ALL_OPERATORS["FR"], key="ap_ops_fr")
    with col_obs3:
        st.markdown("**🇬🇧 UKPL**")
        ap_ops_ukpl = st.multiselect("Operatorzy UKPL:", ALL_OPERATORS["UKPL"], key="ap_ops_ukpl")
        ...
        if wolne_cases:
            ...
            if not ap_keys:
                ...
                if st.button("▶️ Zacznij odpytywać", type="primary"):
                    ...
                    case_queue = []
                    tz_pl = pytz.timezone('Europe/Warsaw')
                    tomorrow = datetime.now(tz_pl) + __import__('datetime').timedelta(days=1)
    ...
    if state == "running":
        ...
        if not queue_doc.exists:
            ...
            tomorrow_date = ap_cfg.get("tomorrow_date", "")
