...
with tab_telefony:
    ...
    def _ciag_ruchow(_nr, _od=None):
        ...
        for _p2 in _deleg_ext:
            ...
            _kroki.append({"dt": _dt(_p2), "t": _mom(_p2),
                           "txt": (f'🔁 podbicie — {_p2.get("zlecil") or "?"}'
                                   if not _p2.get("bot") else '🤖 ponaglenie'), "kur": False})
    ...
    _do_zlec = {}
    for _z in _zlec_only:
        _nr_dz = str(_z.get("numer_zamowienia") or "").strip()
        _t_zl = _dt(_z)
        for _c4 in _cbn.get(_nr_dz, []):
            if not _t_zl or not _dt(_c4) or _dt(_c4) < _t_zl:
                continue          # rozmowa sprzed zlecenia nie należy do niego
            _kto4 = _kto_disp(_c4)
    ...
    if _do_zlec:
        st.dataframe(pd.DataFrame(sorted(_do_zlec.values(), key=lambda r: -r["ŁĄCZNIE"])),
                     use_container_width=True, hide_index=True)
        st.caption("INNE = nieproduktywne, zwroty, zły numer i rozmowy bez oceny.")
    else:
    ...
    st.caption("Rozmowy ze statusem USTALONO TERMIN — jest data odbioru, więc kurier ma po co "
