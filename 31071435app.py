...
with tab_telefony:
    ...
    if _do_oceny:
        with st.expander(f"✍️ Oceń rozmowy ręcznie ({len(_do_oceny)})", expanded=True):
            ...
            _OPCJE = {
                ...
                "NIEPRODUKTYWNE": "nieproduktywne",
                "❌ to nie była rozmowa — usuń wpis": "__usun__",
            }
                ...
                with _e3:
                    if st.button("💾", key=f'_ocb_{_c.get("_dzien")}_{_c.get("_id")}',
                        ...
                        try:
                            ...
                            if _val == "__usun__":
                                _ref.delete()
                                st.success("Usunięto wpis — to nie była rozmowa.")
                            else:
    ...
    with st.expander("🔎 Surowy log zdarzeń (diagnostyka)", expanded=False):
        ...
        for _c in _calls:
            _raw.append({"MOMENT": _mom(_c), "TYP": "TELEFON", "NR": _c.get("numer_zamowienia", ""),
                         "KTO": _kto(_c), "DO KOGO": "", "WYNIK": _c.get("wynik", ""),
                         "ŹRÓDŁO": _c.get("zrodlo", ""), "GRUPA": _kraj(_c), "ID POSTU": ""})
        st.dataframe(pd.DataFrame(sorted(_raw, key=lambda r: r["MOMENT"], reverse=True),
                     ...
                     use_container_width=True, hide_index=True)
        st.caption(f"Zdarzeń: {len(_raw)} · zleceń/ponagleń: {len(_deleg)} · telefonów: {len(_calls)} · obsada: {len(_obsada)}")
