...
with tab_telefony:
    ...
    for _z in _deleg_ext:
        ...
        _kand = [c for c in _cbn.get(_nr_z, []) if c.get("_wpis_po") is not _z]
        _kand.sort(key=_mom)
        _z["_call"] = _kand[-1] if _kand else None
        _z["_ile_rozmow"] = len(_kand)
    ...
    # ══ 6c. WPISY BEZ OCENY (bezpiecznik) ═══════════════════════════════════════
    if _nieoc:
        _suma_n = sum(int(x.get("ile") or 0) for x in _nieoc)
        st.warning(f"⏳ **{_suma_n} wpisów telefonistek czeka na ocenę** w {len(_nieoc)} sprawach — "
                   f"telefon zostanie policzony dopiero, gdy ktoś otworzy sprawę i Szturchacz oceni treść wpisu. "
                   f"Numery: " + ", ".join(sorted({str(x.get("numer_zamowienia") or "?") for x in _nieoc})[:20]))

    # ══ 6e. RĘCZNA OCENA ROZMÓW ═════════════════════════════════════════════════
    _do_oceny = [c for c in _calls if (c.get("wynik") or "") == "do_oceny"]
    if _do_oceny:
