...
with tab_generuj:
    ...
    if st.button("🚀 Generuj raport priorytetów", type="primary"):
        ...
        # Parsuj NrZamy z tekstu szturchacza
        szturchacz_nrzams = set()
        for line in cur_szturchacz.split('\n'):
            nr_match = re.search(r'NrZam[:\s]+(\S+)', line, re.IGNORECASE)
            if not nr_match:
                nr_match = re.match(r'^(ZN\d+)', line)
            if nr_match:
                nrzam = nr_match.group(1).strip().rstrip(',').rstrip('|')
                szturchacz_nrzams.add(nrzam)
        ...
        if is_incremental:
            ...
            # Wyciąg z puli szturchacza TYLKO zamówienia do przeliczenia
            # Parsuj bloki szturchacza
            szturchacz_blocks = {}
            lines = cur_szturchacz.split('\n')
            current_block = []
            current_nr = None
            for line in lines:
                nr_match = re.search(r'NrZam[:\s]+(\S+)', line, re.IGNORECASE)
            ...
            # Szturchacz tylko dla zamówień do przeliczenia
