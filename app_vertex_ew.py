...
def ew_complete_case(case_doc_id, result_tag=None, result_pz=None):
    ...
    db.collection("ew_cases").document(case_doc_id).update(upd)

def ew_release_case(case_doc_id):
    """Oddaj case z powrotem do puli"""
    db.collection("ew_cases").document(case_doc_id).update({
        "status": "wolny",
        "assigned_to": None,
        "assigned_at": None,
    })
...
def ew_count_available(grupa):
    ...
    return len(db.collection("ew_cases")
               .where("grupa", "==", grupa)
               .where("status", "==", "wolny")
               .limit(500).get())
...
def ew_log_completion(op_name):
    ...
    today = datetime.now(tz_pl).strftime("%Y-%m-%d")
    time_str = datetime.now(tz_pl).strftime("%H:%M")
    db.collection("ew_operator_stats").document(today).collection("operators").document(op_name).set({
...
def detect_tag_in_response(text):
    ...
    # TAG-KOPERTA — luźny regex: C#:DD.MM;PZ=coś;DRABES=coś
    m = re.search(r'(C#:\d{2}\.\d{2};PZ=\S+?;DRABES=\S+)', text)
    if m:
        tag = m.group(1)
...
def ew_find_case_by_nrzam(nrzam, op_name):
    ...
    results = db.collection("ew_cases").where("numer_zamowienia", "==", nrzam).limit(5).get()
    if not results:
...
with st.sidebar:
    ...
    ew_today = db.collection("ew_operator_stats").document(
        ...
        if not st.session_state.get("chat_started"):
            if st.button("▶️ Rozpocznij ten case", type="primary"):
                ...
                if wsad:
                    # Oznacz jako w_toku
                    if case.get("_doc_id"):
                        db.collection("ew_cases").document(case["_doc_id"]).update({
                            "status": "w_toku",
            ...
            if st.button(f"🔍 Szukaj case'a ({wybrany_tryb_kod})", type="primary"):
                if nrzam_input.strip():
                    ...
                    if status == "reserved":
                        ...
                        st.session_state.ew_reverse_manual_nrzam = nrzam_input.strip()
                        st.session_state.ew_reverse_manual_wsad = case_data.get("pelna_linia_szturchacza", "")
                    elif status == "completed":
                        # Zakończony — załaduj dane ale nie rezerwuj
                        st.info(f"ℹ️ Case **{nrzam_input}** jest zakończony. Ładuję wsad.")
                        st.session_state.ew_reverse_manual_nrzam = nrzam_input.strip()
                        st.session_state.ew_reverse_manual_wsad = case_data.get("pelna_linia_szturchacza", "")
                    else:
                        # Nie znaleziono w casach — pozwól wkleić ręcznie
                        st.session_state.ew_reverse_manual_nrzam = nrzam_input.strip()
                        st.session_state.ew_reverse_manual_wsad = ""
                else:
                    st.error("Podaj numer zamówienia!")
            
            # Pole do wklejenia wsadu ręcznie (gdy case nie znaleziony / zajęty / zakończony)
            manual_nrzam = st.session_state.get("ew_reverse_manual_nrzam", "")
            if manual_nrzam:
                manual_wsad = st.session_state.get("ew_reverse_manual_wsad", "")
                if not manual_wsad:
                    st.warning(f"🔍 Nie znaleziono **{manual_nrzam}** w bazie casów.")
                wsad_input = st.text_area(
                    f"📋 Wklej wsad ze szturchacza dla **{manual_nrzam}**:",
                    value=manual_wsad,
                    key="ew_reverse_manual_input",
                    height=150,
                    placeholder="Wklej dane zamówienia ze szturchacza..."
                )
                if st.button(f"▶️ Rozpocznij case {manual_nrzam} ({wybrany_tryb_kod})", key="start_manual_reverse"):
                    if wsad_input and wsad_input.strip():
                        manual_case = {
                            "numer_zamowienia": manual_nrzam,
                            "score": 0,
                            "priority_icon": "📨",
                            "priority_label": f"RĘCZNY — {wybrany_tryb_kod}",
                            "grupa": operator_grupa,
                            "pelna_linia_szturchacza": wsad_input.strip(),
                            "naglowek_priorytetowy": f"Case ręczny {manual_nrzam}",
                            "_reverse_mode": True,
                            "_reverse_type": wybrany_tryb_kod,
                            "_manual": True,
                        }
                        st.session_state.ew_current_case = manual_case
                        st.session_state.pop("ew_reverse_manual_nrzam", None)
                        st.session_state.pop("ew_reverse_manual_wsad", None)
                        st.rerun()
                    else:
                        st.error("Wklej wsad ze szturchacza!")
    ...
    if st.button("🚀 Nowa sprawa / Reset", type="primary"):
        ...
        if st.session_state.ew_current_case:
            case = st.session_state.ew_current_case
            status = db.collection("ew_cases").document(case["_doc_id"]).get().to_dict().get("status")
...
# ==========================================
st.title(f"🤖 Szturchacz EW (Wieżowiec)")
...
if not st.session_state.chat_started:
    ...
    case = st.session_state.ew_current_case
    wybrany_tryb_main = st.session_state.get("ew_tryb_startowy", "Standard")
    ...
    if show_case_main:
        ...
        st.info(f"📨 Tryb **{tryb_kod_main}** — wyszukaj lub wklej case w panelu bocznym.\n\n"
...
# ==========================================
if not st.session_state.chat_started:
    # Pokaż pole ręcznego wsadu TYLKO gdy nie ma case'a z Wieżowca
    if not st.session_state.ew_current_case:
