...
with tab_telefony:
    ...
    _dni_ext = _dni_range(_t_from, _t_to, _extra=5)
    ...
    for _z in _deleg_ext:
        _po = sorted([c for c in _cbn.get(str(_z.get("numer_zamowienia") or "").strip(), [])
                      if _mom(c) >= _mom(_z) and c.get("_wpis_po") is not _z], key=_mom)
