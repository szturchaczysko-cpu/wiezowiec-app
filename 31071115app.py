...
with tab_telefony:
        ...
        try:
            for _d in db.collection(col("ew_phone_log")).document(_ds).collection("calls").stream():
                ...
                if (_v.get("wynik") or "") != "do_oceny":
                    continue
                # 🟥 Do oceny kwalifikuje się TYLKO rozmowa POWIĄZANA z naszym zleceniem.
                #    Zapisy bez powiązania to pozostałości po starym mechanizmie, który wciągał
                #    całą historię wątku — nikt nie będzie oceniał telefonu sprzed miesiąca.
                if not str(_v.get("zlecenie_post_id") or "").strip():
                    _oc_stare += 1
                    _v["_dzien"] = _ds
                    _v["_id"] = _d.id
                ...
                if _wd and _wd not in _dni_set:
                    continue                      # przerobione w innym dniu — nie ta kolejka
                _v["_dzien"] = _ds
                _v["_id"] = _d.id
    ...
    if _oc_stare:
        with st.expander(f"🧹 Stare zapisy bez powiązania ze zleceniem ({_oc_stare}) — do usunięcia",
                         expanded=False):
            st.caption("Powstały mechanizmem, który zapisywał całą historię wątku, także odpowiedzi "
                       "na prośby sprzed tygodni. Nie dotyczą Waszych zleceń, więc nie ma czego oceniać. "
                       "Nowe zapisy już tak nie powstają.")
            _pods = sorted({f'{x.get("operator","?")} · {x.get("data_str") or x.get("_dzien","")}'
                            for x in _oc_stare_lista})[:12]
            st.caption("Przykłady: " + " · ".join(_pods))
