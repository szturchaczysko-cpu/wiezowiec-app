...
WIEZOWIEC_PROMPT_URLS = {
    "Wieżowiec v5 (stabilny)": "https://raw.githubusercontent.com/szturchaczysko-cpu/szturchacz/refs/heads/main/prompt_wiezowiec_v4_gemini-2.md",
}
...
with tab_wsady:
    ...
    with col_clr2:
        if st.button("🗑️ Wyczyść kolejkę casów (ew_cases)"):
            # Przenieś casy z nienaprawionymi komentarzami do archiwum
            ...
            for bdoc in all_batches:
                ...
                for c in batch_cases:
                    ...
                    if cdata.get("skip_reason") and not cdata.get("skip_fixed"):
                        ...
                        cdata["archived_from_batch"] = bdoc.id
            ...
            msg = f"🗑️ Usunięto {deleted} casów i wszystkie batche."
            if archived > 0:
...
with tab_cases:
    ...
    if not filtered:
        ...
        st.markdown(f"📊 **Łącznie: {total}** | 🔵 Wolne: {n_wolny} | 🟡 Pobrane: {n_przydz} | 🟢 Zakończone: {n_zakonczony} | 🤖 Przeliczone: {n_przeliczone} | ⚪ Nieprzeliczone: {n_nieprzeliczone} | ❓ Bez grupy/Score 0: {n_score0}")
