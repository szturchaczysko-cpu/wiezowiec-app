...
with tab_telefony:
    ...
    def _link_md(_url, _pid):
        """Krótki klikalny link — nie rozpycha kolumny."""
        _p = str(_pid or "").strip()
        if _url:
            return f"[{_p or 'wpis'}]({_url})"
        return _p or "—"

    _KOL = ["NR ZAMÓWIENIA", "WPIS", "ZLECONO", "OD KOGO", "DO KOGO",
            "PODJĘTE", "STATUS", "UWAGI"]
    _r_all = []
    ...
    for _c in _calls_op:
        ...
        _r_all.append({
            ...
            "ZLECONO": _mom(_c),
            "OD KOGO": _kto_disp(_c),
            "DO KOGO": ("do woreczka" if _po_wpis else "— telefon bez wpisu"),
            ...
            "UWAGI": ((f'wykryte {_c.get("wykryte_dnia")} · '
                      ...
                      + ("telefon operatora → wpis na forum" if _po_wpis else "telefon operatora")
    ...
    for _z in sorted(_zlec_only, key=_mom, reverse=True):
        ...
        if _cc:
            ...
            if _t0_real and _t1:
                ...
                if _bm >= 0:
                    ...
                    if _bm > 2 * 480:
                        _uw += " · po terminie"
            _podj = (f'FORUM — {_kto(_cc)} / {_osoba(_cc)} · {_mom(_cc)[11:]}{_delta}' if _grupowy(_cc)
                     else f'SESJA — {_kto(_cc)} z woreczka · {_mom(_cc)[11:]}{_delta}')
            if _cc.get("_dzien") not in _dni_set:
                _uw += " · rozmowa spoza zakresu"
        ...
        _r_all.append({
            ...
            "WPIS": _link_md(_z.get("link"), _z.get("id_postu")),
            "ZLECONO": _mom(_z),
    ...
    st.dataframe(_df_all, use_container_width=True, hide_index=True,
                 column_config={"WPIS": st.column_config.LinkColumn("WPIS", width="small",
                                                                    display_text=r"\[(.*?)\]")})
    if _r_all:
