...
with tab_autopilot:
    ...
    if state == "running":
        ...
        st.info("⏸️ Autopilot zatrzymuje się po bieżącym casie...")
...
"""
                ...
                for attempt in range(5):
                    try:
                        ...
                        if "429" in err_str or "Quota" in err_str or "ResourceExhausted" in err_str:
                            ...
                            progress_placeholder.progress(
                                idx / max(total, 1),
                                text=f"🤖 Case {idx+1}/{total}: {nrzam} — ⏳ rate limit, czekam {wait_time}s (próba {attempt+1}/5)"
                            )
                            time.sleep(wait_time)
                        else:
                            set_autopilot_status({"last_error": f"{nrzam}: {err_str[:200]}"})
                            with live_container:
                                st.caption(f"❌ {nrzam}: {err_str[:150]}")
                            break
                ...
                if idx < total - 1:
                    progress_placeholder.progress(
                        (idx + 1) / max(total, 1),
                        text=f"⏳ Pauza {pause_sec}s przed następnym casem..."
                    )
