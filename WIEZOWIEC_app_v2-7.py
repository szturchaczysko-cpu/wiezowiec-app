...
"""
        ...
        # Debug: pokaż rozmiar wiadomości
        msg_size = len(user_msg)
        st.caption(f"📏 Rozmiar wiadomości do AI: {msg_size:,} znaków (~{msg_size//4:,} tokenów)")
        ...
        with st.spinner("🏢 Wieżowiec analizuje... To może potrwać kilka minut."):
            ...
            for attempt in range(3):
                try:
                    ...
                    # Sprawdź czy odpowiedź ma tekst
                    if resp.candidates:
                        candidate = resp.candidates[0]
                        finish_reason = candidate.finish_reason if hasattr(candidate, 'finish_reason') else "?"
                        if candidate.content and candidate.content.parts:
                            ai_text = candidate.content.parts[0].text
                        else:
                            st.warning(f"⚠️ AI zwróciło pustą odpowiedź. Finish reason: {finish_reason}")
                            if hasattr(candidate, 'safety_ratings'):
                                st.caption(f"Safety ratings: {candidate.safety_ratings}")
                    else:
                        # Próba fallback przez resp.text
                        ai_text = resp.text
                    ...
                    if "429" in err_str or "Quota" in err_str:
                        ...
                        st.warning(f"⚠️ Safety block (próba {attempt+1}/3): {err_str[:300]}")
                        time.sleep(5)
                    else:
                        st.error(f"Błąd AI (próba {attempt+1}/3): {err_str[:500]}")
                        break
        ...
        if is_incremental and nrzam_gotowe:
            ...
            # Posortuj CAŁĄ listę per grupa po score (malejąco)
