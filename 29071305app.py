...
# ==========================================
# 📞 TELEFONY — REJESTR I PODSUMOWANIE
# ==========================================
with tab_telefony:
    st.subheader("📞 Telefony — rejestr i podsumowanie")

    _TZ_T = pytz.timezone("Europe/Warsaw")
    _d1, _d2, _d3 = st.columns([2, 2, 3])
    with _d1:
        _t_from = st.date_input("Od dnia:", value=datetime.now(_TZ_T).date(), key="_tel_d_from")
    with _d2:
        _t_to = st.date_input("Do dnia:", value=datetime.now(_TZ_T).date(), key="_tel_d_to")

    _dni, _cur = [], _t_from
    while _cur <= _t_to and len(_dni) < 120:
        _dni.append(_cur.strftime("%Y-%m-%d")); _cur += timedelta(days=1)

    _deleg, _calls = [], []
    for _ds in _dni:
        for _sub, _bag in (("delegacje", _deleg), ("calls", _calls)):
            try:
                for _doc in db.collection(col("ew_phone_log")).document(_ds).collection(_sub).stream():
                    _x = _doc.to_dict() or {}
                    _x["_dzien"] = _ds
                    _bag.append(_x)
            except Exception:
    ...
    with _d3:
        _gr_all = sorted({str(d.get("grupa") or "?") for d in _deleg} | {str(c.get("grupa") or "?") for c in _calls})
        _f_gr = st.multiselect("Grupa operatorska:", _gr_all, key="_tel_f_gr")
    if _f_gr:
        _deleg = [d for d in _deleg if str(d.get("grupa") or "?") in _f_gr]
        _calls = [c for c in _calls if str(c.get("grupa") or "?") in _f_gr]

    # ── wspólne narzędzia ──
    def _dt_of(_r):
        try:
            return _TZ_T.localize(datetime.strptime(
                f'{_r.get("_dzien","")} {_r.get("godzina","00:00")}', "%Y-%m-%d %H:%M"))
        except Exception:
            return None
    ...
    _STATUS = {
        "brak_kontaktu": "NIE ODBIERA",
        "kontakt_bez_konkretu": "ODEBRAŁ, BEZ USTALEŃ",
        "przelozenie": "TERMIN NA TERMIN",
        "konkret": "USTALONO",
        "nieznany": "— (czeka na przerobienie)",
    }

    def _status_txt(_c):
        if not _c:
            return ""
        _w = _c.get("wynik") or "nieznany"
        if _w == "konkret" and _c.get("kurier_ustalony"):
            return "ZAMÓWIONY"
        return _STATUS.get(_w, _w)

    # kategoria do statystyk: gluchy / bez_ustalen / z_ustaleniami / brak_wyniku
    def _kat(_c):
        _w = _c.get("wynik") or "nieznany"
        if _w == "brak_kontaktu":
            return "gluchy"
        if _w == "kontakt_bez_konkretu":
            return "bez_ustalen"
        if _w in ("konkret", "przelozenie"):
            return "z_ustaleniami"
        return "brak_wyniku"

    _op_calls = [c for c in _calls if c.get("zrodlo") != "telefonista_zewn"]
    _tel_calls = [c for c in _calls if c.get("zrodlo") == "telefonista_zewn"]
    _zlecenia = [d for d in _deleg if (d.get("typ") or "zlecenie") == "zlecenie"]
    _ponaglenia = [d for d in _deleg if (d.get("typ") or "") == "ponaglenie"]

    # ── dopasowania ──
    _deleg_by_num, _calls_by_num = {}, {}
    for _d in _deleg:
        _deleg_by_num.setdefault(str(_d.get("numer_zamowienia") or "").strip(), []).append(_d)
    for _c in _calls:
        _calls_by_num.setdefault(str(_c.get("numer_zamowienia") or "").strip(), []).append(_c)

    # telefon operatora → czy po nim poszedł wpis na forum (wróć za 2h)
    for _c in _op_calls:
        _nr = str(_c.get("numer_zamowienia") or "").strip()
        _po = sorted([d for d in _deleg_by_num.get(_nr, []) if _mom(d) >= _mom(_c)], key=_mom)
        _c["_wpis_po"] = _po[0] if _po else None

    # zlecenie → kto je podjął (pierwszy telefon po zleceniu)
    for _z in _deleg:
    ...
    _n_otw = sum(1 for z in _zlecenia if not z.get("_call"))
    _m1, _m2, _m3, _m4 = st.columns(4)
    _m1.metric("Telefony operatorów", len(_op_calls))
    _m2.metric("Telefony telefonistek", len(_tel_calls))
    _m3.metric("Zlecenia na forum", len(_zlecenia))
    _m4.metric("Zlecenia otwarte", _n_otw)
    _n_recz = sum(1 for z in _zlecenia if z.get("recznie"))
    if _n_recz:
        st.caption(f"↳ w tym {_n_recz} zlecenie(a) wklejone RĘCZNIE po błędzie zapisu na forum.")

    if not _deleg and not _calls:
        st.info("Brak danych w wybranym zakresie dat. Wybierz inny dzień lub zakres — rejestr zapisuje "
                "telefony i zlecenia od wdrożenia nowej wersji Szturchacza.")

    # ══ TABELA 1 — TELEFONY OPERATORÓW DZWONIĄCYCH ══════════════════════════════
    st.markdown("### ☎️ Telefony wykonane przez operatorów dzwoniących")
    st.caption("Telefon wykonany w sesji Szturchacza — znamy zalogowanego, więc imiennie. "
               "Nieodebrany powoduje wpis na forum („wróć za 2h\") i trafia do woreczka — ID tego wpisu w UWAGACH.")
    if _op_calls:
        _r1 = []
        for _c in sorted(_op_calls, key=_mom, reverse=True):
            _wp = _c.get("_wpis_po")
            _kt = _kat(_c)
            if _kt == "gluchy":
                _uw = (f'zlecono, wróć za 2h (post {_wp.get("id_postu","?")})' if _wp
                       else "nieodebrany — brak wpisu na forum")
            else:
                _uw = "telefon owocny"
            _r1.append({
                "NR ZAMÓWIENIA": _c.get("numer_zamowienia", ""),
                "ID POSTU": "sesja",
                "DATA / GODZINA": f'{_c.get("_dzien","")} {_c.get("godzina","")}',
                "DO KOGO": _c.get("operator", ""),
                "CZY PODJĘTE / przez kogo": f'TAK / {_c.get("operator","")}',
                "STATUS": _status_txt(_c),
                "CZY W TERMINIE": "TAK (od razu)",
                "GRUPA": _c.get("grupa", ""),
                "UWAGI": _uw,
            })
        _df1 = pd.DataFrame(_r1)
        st.dataframe(_df1, use_container_width=True, hide_index=True)
        st.download_button("⬇️ CSV — telefony operatorów", _df1.to_csv(index=False).encode("utf-8"),
                           file_name=f"telefony_operatorow_{_t_from}_{_t_to}.csv", mime="text/csv", key="_csv_t1")
    else:
        st.caption("Brak telefonów operatorów w tym zakresie.")

    # ══ TABELA 2 — WPISY NA FORUM (ZLECENIA) ════════════════════════════════════
    st.markdown("### 📨 Telefony zlecone na forum (grupy telefonistek)")
    st.caption("Każdy wpis delegujący telefon. Podjęte TAK = zadzwoniła grupa telefonistek; NIE = telefon "
               "wykonał operator z woreczka (jego imię obok). Czas podjęcia liczony od godziny zlecenia.")
    if _deleg:
        _r2 = []
        for _z in sorted(_deleg, key=_mom, reverse=True):
            _cc = _z.get("_call")
            _t0, _t1 = _dt_of(_z), (_dt_of(_cc) if _cc else None)
            if _cc:
                _przez_tel = _cc.get("zrodlo") == "telefonista_zewn"
                _podjete = (f'TAK / {_cc.get("operator","")}' if _przez_tel
                            else f'NIE / zadzwonił {_cc.get("operator","")}')
                _wykonano = f'{_cc.get("_dzien","")} {_cc.get("godzina","")}'
                if _t0 and _t1:
                    _diff = (_t1 - _t0).total_seconds() / 60
                    _delta = f'{int(_diff//60)}h {int(_diff%60)}min'
                    _termin = (f'✅ ten sam dzień (+{_delta})' if _cc.get("_dzien") == _z.get("_dzien")
                               else f'❌ inny dzień (+{_delta})')
                else:
                    _termin = "—"
            else:
                _podjete, _wykonano, _termin = "NIE — nikt jeszcze", "", "⏳ otwarte"
            _r2.append({
                "NR ZAMÓWIENIA": _z.get("numer_zamowienia", ""),
                "ID POSTU": _z.get("id_postu", ""),
                "DATA / GODZINA": f'{_z.get("_dzien","")} {_z.get("godzina","")}',
                "DO KOGO": _z.get("do_kogo", ""),
                "CZY PODJĘTE / przez kogo": _podjete,
                "DATA WYKONANIA": _wykonano,
                "STATUS": _status_txt(_cc),
                "CZY W TERMINIE": _termin,
                "ZLECIŁ": _z.get("zlecil", ""),
                "GRUPA": _z.get("grupa", ""),
                "UWAGI": ("ponaglenie" if (_z.get("typ") == "ponaglenie") else "zlecenie")
                         + (" · wpis ręczny" if _z.get("recznie") else ""),
            })
        _df2 = pd.DataFrame(_r2)
        st.dataframe(_df2, use_container_width=True, hide_index=True)
        st.download_button("⬇️ CSV — zlecone na forum", _df2.to_csv(index=False).encode("utf-8"),
                           file_name=f"telefony_zlecone_{_t_from}_{_t_to}.csv", mime="text/csv", key="_csv_t2")
    else:
        st.caption("Brak wpisów zlecających telefon w tym zakresie.")

    # ══ TABELA 3 — PODSUMOWANIE ═════════════════════════════════════════════════
    st.markdown("### 📊 Podsumowanie — kto ile wykonał i ze skutkiem")
    st.caption("Głuche = nie odebrał. Bez ustaleń = rozmowa bez efektu. Z ustaleniami = konkret lub umówiony "
               "termin. Skuteczność = z ustaleniami ÷ wszystkie z wynikiem.")

    def _wiersz_stat(_nazwa, _typ, _lst, _czas=None):
        _g = sum(1 for c in _lst if _kat(c) == "gluchy")
        _b = sum(1 for c in _lst if _kat(c) == "bez_ustalen")
        _z = sum(1 for c in _lst if _kat(c) == "z_ustaleniami")
        _brak = sum(1 for c in _lst if _kat(c) == "brak_wyniku")
        _suma = _g + _b + _z
        return {
            "Kto": _nazwa, "Typ": _typ,
            "Głuche": _g, "Bez ustaleń": _b, "Z ustaleniami": _z,
            "Razem": _suma + _brak,
            "Skuteczność": (f"{round(100*_z/_suma)}%" if _suma else "—"),
            "Bez wyniku": _brak,
            "Śr. czas podjęcia": _czas or "—",
        }

    _rows3 = []
    _per_op = {}
    for _c in _op_calls:
        _per_op.setdefault(_c.get("operator") or "?", []).append(_c)
    for _o, _lst in sorted(_per_op.items(), key=lambda x: -len(x[1])):
        _rows3.append(_wiersz_stat(_o, "operator", _lst))

    # grupy telefonistek + średni czas podjęcia (od zlecenia do ich wpisu)
    _per_gr, _reakcje = {}, {}
    for _c in _tel_calls:
        _per_gr.setdefault(_c.get("operator") or "?", []).append(_c)
    for _z in _deleg:
        _cc = _z.get("_call")
        if not _cc or _cc.get("zrodlo") != "telefonista_zewn":
            continue
        _t0, _t1 = _dt_of(_z), _dt_of(_cc)
        if _t0 and _t1 and _t1 >= _t0:
            _reakcje.setdefault(_cc.get("operator") or "?", []).append((_t1 - _t0).total_seconds() / 60)
    for _g, _lst in sorted(_per_gr.items(), key=lambda x: -len(x[1])):
        _rl = _reakcje.get(_g, [])
        _cz = (f'{int((sum(_rl)/len(_rl))//60)}h {int((sum(_rl)/len(_rl))%60)}min' if _rl else "—")
        _rows3.append(_wiersz_stat(_g, "grupa telefonistek", _lst, _cz))

    if _rows3:
        _df3 = pd.DataFrame(_rows3)
        st.dataframe(_df3, use_container_width=True, hide_index=True)
        _all = _op_calls + _tel_calls
        _sg = sum(1 for c in _all if _kat(c) == "gluchy")
        _sb = sum(1 for c in _all if _kat(c) == "bez_ustalen")
        _sz = sum(1 for c in _all if _kat(c) == "z_ustaleniami")
        _ss = _sg + _sb + _sz
        if _ss:
            _p1, _p2, _p3, _p4 = st.columns(4)
            _p1.metric("Wszystkie telefony", _ss)
            _p2.metric("Głuche", f"{_sg} ({round(100*_sg/_ss)}%)")
            _p3.metric("Bez ustaleń", f"{_sb} ({round(100*_sb/_ss)}%)")
            _p4.metric("Z ustaleniami", f"{_sz} ({round(100*_sz/_ss)}%)")
        _brak_w = sum(1 for c in _all if _kat(c) == "brak_wyniku")
        if _brak_w:
            st.caption(f"⏳ {_brak_w} telefon(ów) telefonistek czeka na przerobienie sprawy — wynik "
                       f"dopisze się, gdy operator otworzy sprawę i Szturchacz odczyta ich wpis.")
        st.download_button("⬇️ CSV — podsumowanie", _df3.to_csv(index=False).encode("utf-8"),
                           file_name=f"telefony_podsumowanie_{_t_from}_{_t_to}.csv", mime="text/csv", key="_csv_t3")
    else:
        st.caption("Brak telefonów do podsumowania w tym zakresie.")
