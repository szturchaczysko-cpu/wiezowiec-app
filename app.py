...
with tab_zastepstwa:
    ...
    st.markdown("---")
    st.markdown("### 📞 Sprzedawcy dzwoniący")
    st.caption("Dla sprzedawców, którzy TEŻ dzwonią do klientów. **Dzwoni** = ma prawo dzwonić sam; "
               "**Języki** = w jakich językach dzwoni. Bez woreczka. Zapis do konfiguracji tej osoby — "
               "działa, gdy loguje się do Szturchacza pod tym samym nickiem.")
    _LANGI_S = ["DE", "FR", "PL", "IT", "ES", "ENG"]
    for _sni, (_snick, _skraj) in enumerate(SPRZEDAWCY_LISTA):
        try:
            _sd = db.collection(col("operator_configs")).document(_snick).get().to_dict() or {}
        except Exception:
            _sd = {}
        ...
        with _sc2:
            _sdzw = st.checkbox("dzwoni", value=bool(_sd.get("dzwoni", False)),
                                key=f"_szdz_{_snick}", label_visibility="collapsed")
        with _sc3:
            _sjz = st.multiselect("języki", _LANGI_S,
                                  default=[str(x).upper() for x in _sd.get("jezyki_dzwoniacy", []) if str(x).upper() in _LANGI_S],
                                  key=f"_szjz_{_snick}", label_visibility="collapsed")
        with _sc4:
            if st.button("💾", key=f"_szsave_{_snick}", help=f"Zapisz dzwonienie dla {_snick}"):
                try:
                    db.collection(col("operator_configs")).document(_snick).set({
                        "dzwoni": _sdzw, "jezyki_dzwoniacy": _sjz,
                    }, merge=True)
