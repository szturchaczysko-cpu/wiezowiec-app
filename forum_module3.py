...
def forum_write(post_id, do_odp_id, user_do, tresc, user_do_type=1, user_od=None, ai_user=None, tytul=None):
    ...
    # FIX AUTORYZACJI ZAPISU (analogicznie do FIX 406 przy czytaniu, potwierdzone przez IT forum):
    # po migracji API stare pole "login" usunieto, a endpointy WYMAGAJA LoginRequestedBy; przy pisaniu
    # w imieniu grupy dodatkowo LoginActingAs + MemberTypeActingAs. GetPostTree juz to wysyla, a
    # CreatePost NIE -> serwer zwracal {"Message":"Brak autoryzacji lub token nieprawidlowy."} dla
    # KAZDEGO wpisu jako grupa (autopilot i sesja operatora pisza z fromUserType=2).
    #   LoginRequestedBy   = konto wykonujace zadanie (token) = chatoszturek (FORUM_USER)
    #   LoginActingAs      = tozsamosc, w imieniu ktorej piszemy = user_od (grupa operatorow lub chatoszturek)
    #   MemberTypeActingAs = 1=user, 2=grupa (spojne z from_user_type dla user_od)
    _write_login_acting_as = user_od if user_od else FORUM_USER
    payload = {
        "LoginRequestedBy": FORUM_USER,
        "LoginActingAs": _write_login_acting_as,
        "MemberTypeActingAs": from_user_type,
        "thread": {
            "id": post_id,
            "title": tytul,
            "fromUser": user_od,
