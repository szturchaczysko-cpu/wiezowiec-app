...
with tab_telefony:
    ...
    for _z in _deleg_ext:
        ...
        # telefon operatora z woreczka nie ma powiązania z postem — wiążemy po czasie
        _po += [c for c in _cbn.get(_nr_z, [])
                if not c.get("zlecenie_post_id") and not _grupowy(c)
                and c.get("_wpis_po") is not _z and _dt(c) and _dt(_z) and _dt(c) >= _dt(_z)]
        _po.sort(key=_mom)
    ...
    _calls_op = [c for c in _calls if not _grupowy(c) and _przed_wpisem(c)]
    ...
    for _c in sorted(_calls_op, key=_mom, reverse=True):
        ...
        _r1.append({
            ...
            "STATUS": _status(_c),
            "GRUPA": _kraj(_c),
            "UWAGI": (("zakończony wpisem na forum → dalszy ciąg w zleceniach" if _po_wpis
