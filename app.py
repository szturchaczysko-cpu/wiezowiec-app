...
with tab_telefony:
    ...
    if _wszyscy:
        ...
        def _slupki(_dane, _wart, _maxv, _kolor, _fmt):
            _kol = []
            for _d in _dane:
                _v = _d[_wart]
                ...
                _kol.append(
                    ...
                    f'<div style="width:34px;height:{_h}px;background:{_kolor};border-radius:3px 3px 0 0{_op}"></div>'
                    '</div>'
                    f'<div style="font-size:12px;margin-top:5px;font-weight:500">{_fmt(_d)}</div>'
        ...
        _slupki(_sort, "_j", 100, "#1D9E75", lambda d: f'{d["_j"]}%')
    ...
    for _k in _kraje:
        ...
        _rows_vs.append({
            "Kraj": _k,
            "Telefonistki — rozmowy": len(_tel),
            "Telefonistki — ustalenia": sum(1 for c in _tel if _z_ust(c)),
            "Operatorzy — rozmowy": len(_ope),
            "Operatorzy — ustalenia": sum(1 for c in _ope if _z_ust(c)),
            "Udział telefonistek": f"{round(100*len(_tel)/len(_cl_k))}%",
        })
    if _rows_vs:
