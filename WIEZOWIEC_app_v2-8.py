...
"""
            ...
            for attempt in range(3):
                try:
                    ...
                    if "429" in err_str or "Quota" in err_str:
                        st.toast(f"⏳ Limit API, partia {batch_num}, próba {attempt+1}/3...")
                        time.sleep(15)
                        ...
                        st.error(f"Błąd AI (partia {batch_num}, próba {attempt+1}/3): {err_str[:300]}")
                        break
            ...
            if ai_text:
                ...
                st.warning(f"⚠️ Partia {batch_num}: brak odpowiedzi AI — pominięta.")
