        ...
        if state == "running":
            ...
            if not dl_obsada:
                ...
                if dolewka_queue:
                    ...
                    # Parametry: czytaj z session_state lub domyślne
                    dl_prompt_url = st.session_state.get("_gen_ap_prompt_url", "https://raw.githubusercontent.com/szturchaczysko-cpu/szturchacz/refs/heads/main/prompt4624.txt")
                    dl_prompt_name = st.session_state.get("_gen_ap_prompt_name", "v4")
                    dl_model = st.session_state.get("_gen_ap_model", "gemini-2.5-pro")
                    dl_pause = st.session_state.get("_gen_ap_pause", 30)
                    dl_key_indices = st.session_state.get("_gen_ap_key_indices", list(range(len(GCP_PROJECTS))) if GCP_PROJECTS else [0])
