...
with tab_telefony:
    ...
    for _ds in _oc_dni:
        try:
            for _d in db.collection(col("ew_phone_log")).document(_ds).collection("calls").stream():
                ...
                if not _wd and _ds not in _dni_set:
                    _oc_stare += 1
                    continue                      # dane sprzed wprowadzenia pola
                _v["_dzien"] = _ds
                _v["_id"] = _d.id
    ...
    if _oc_stare:
        st.caption(f"({_oc_stare} rozmów bez oceny pochodzi z innych dni — poza tą kolejką. "
                   f"To zwykle pozostałości po starym sposobie zapisu; można je usunąć przyciskiem "
                   f"czyszczenia na dole.)")
