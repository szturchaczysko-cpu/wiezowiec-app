...
with tab_telefony:
    ...
    for _c in _calls_op:
        ...
        _r_all.append({
            ...
            "PODJĘTE": (f'📞 SESJA — {_kto_disp(_c)} · {_mom(_c)[11:]}'
                        + (' → 💎 kurier zamówiony'
                           if (_c.get("wynik") or "") in ("ustalono_termin", "konkret") else '')),
    ...
    for _z in sorted(_zlec_only, key=_mom, reverse=True):
        ...
        if _cc:
            ...
            if (_cc.get("wynik") or "") in ("ustalono_termin", "konkret"):
    ...
    for _c in _calls:
        ...
        _cel = _kur_by_nr if _w_k in ("ustalono_termin", "konkret") else (
            _zwr_by_nr if _w_k == "zwrot" else None)
        if _cel is None:
            continue
