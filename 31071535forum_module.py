...
def ostatnie_posty(nrzam):
    """Struktura wpisów z ostatniego odczytu wątku dla danego zamówienia."""
    return _OSTATNIE_POSTY.get(str(nrzam or "").strip(), [])
    ...
    try:
        ...
        for cel, info in memory.items():
            ...
            if result.get("success") and result.get("posts"):
                ...
                try:
                    _OSTATNIE_POSTY[str(numer_zamowienia or "").strip()] = [{
                        "Id": p.get("Id"),
                        "Do_Odpid": p.get("Do_Odpid"),
                        "Hierarchy": p.get("Hierarchy", ""),
                        "Autor": p.get("UserAddName", ""),
                        "Osoba": p.get("UserOdInGroup", "") or "",
