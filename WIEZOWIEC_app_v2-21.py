...
with tab_cases:
    ...
    with fc4:
        f_skip = st.checkbox("Tylko z komentarzem pominięcia", key="f_skip")
    ...
    if fs != "Wszystkie":
        q = q.where("status", "==", fs)
    # Nie filtrujemy po assigned_to w Firestore (wymaga composite indexu)
    # Filtrujemy po stronie klienta
    q = q.order_by("score", direction=firestore.Query.DESCENDING).limit(1000)
    results = q.get()
    
    # Filtr operatora (po stronie klienta)
    if fo:
        results = [d for d in results if d.to_dict().get("assigned_to", "") == fo]
    
    # Filtr komentarzy (po stronie klienta)
    if f_skip:
        results = [d for d in results if d.to_dict().get("skip_reason")]
    ...
    if not results:
        ...
        # Statystyki
        all_data = [d.to_dict() for d in results]
        n_wolny = sum(1 for d in all_data if d.get("status") == "wolny")
        n_przydz = sum(1 for d in all_data if d.get("status") in ("przydzielony", "w_toku"))
        n_zakonczony = sum(1 for d in all_data if d.get("status") == "zakonczony")
        st.markdown(f"📊 **Łącznie: {total}** | 🔵 Wolne: {n_wolny} | 🟡 Pobrane: {n_przydz} | 🟢 Zakończone: {n_zakonczony}")
