...
with tab_telefony:
    ...
    _zlec_only = [d for d in _deleg if (d.get("typ") or "zlecenie") == "zlecenie"]
    _pon_only = [d for d in _deleg if (d.get("typ") or "") == "ponaglenie"]
    _pon_bn = {}
    for _p in _pon_only:
    ...
    _calls_op_all = [c for c in _calls if not _grupowy(c) and _przed_wpisem(c)]
    _calls_op = [c for c in _calls_op_all if _telefon_wiarygodny(c)]
    _odrzucone = [c for c in _calls_op_all if not _telefon_wiarygodny(c)]
    _r1 = []
    ...
    if _odrzucone:
        _lst_o = sorted({f'{_kto(c)} ({_kraj(c)}, {_jez_calla(c)})' for c in _odrzucone})
        st.caption(f"⚠️ Pominięto {len(_odrzucone)} rozmów, w których język sprawy nie mieści się "
                   f"w przypisanych językach operatora — te osoby w tej grupie tylko zlecają: "
                   + ", ".join(_lst_o[:8]))
    if not _r1:
