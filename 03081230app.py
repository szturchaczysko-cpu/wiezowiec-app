...
with tab_telefony:
    ...
    _SKUTECZNE = ("ustalono_termin", "ustalono_termin_na_termin", "konkret", "przelozenie")
    ...
    def _oceniony(_c):
        ...
        return (_c.get("wynik") or "") != "do_oceny"
    ...
    for _z in _deleg_ext:
        ...
        _moje = {str(_z.get("id_postu") or "").strip()} | _pon_id.get(_nr_z, set())
        _moje.discard("")
        _po = [c for c in _cbn.get(_nr_z, [])
               if str(c.get("zlecenie_post_id") or "").strip() in _moje]
        # Rozmowy BEZ zapisanego numeru zlecenia (starsze, sprzed wprowadzenia powiązania)
        # wiążemy po czasie — inaczej po ocenie werdykt nie miałby jak trafić do tego wiersza.
        # MINIMALNY ODSTĘP dotyczy telefonu operatora: rozmowa w tej samej minucie co wpis to ta,
        # która ten wpis WYWOŁAŁA (zadzwonił, nie dodzwonił się, poszło zlecenie) — nie wykonanie.
        _MIN_ODSTEP = timedelta(minutes=2)
        for _c in _cbn.get(_nr_z, []):
            if str(_c.get("zlecenie_post_id") or "").strip():
                continue                      # ma twarde powiązanie — już uwzględniony wyżej
            if not _dt(_c) or not _dt(_z):
                continue
            if _grupowy(_c):
                # telefonistka nigdy nie dzwoni bez zlecenia — wystarczy, że rozmowa jest po nim
                if _dt(_c) >= _dt(_z):
                    _po.append(_c)
            else:
                if _c.get("_wpis_po") is not _z and _dt(_c) >= _dt(_z) + _MIN_ODSTEP:
                    _po.append(_c)
        _po.sort(key=_mom)
    ...
    _niewyk = [z for z in _zlecenia if _niewyk_do(z, min(_gr_koniec, _now))]
    ...
    for _k in _kraje:
        ...
        for _v in sorted(_agg.values(), key=lambda x: -x["Telefony"]):
            ...
            _rows.append({**_v, "Zlecił": _zlec_k.get(_v["Kto"], 0),
                          ...
                          "Skuteczność": (f'{round(100*_v["Z ustaleniami"]/_oc_v)}%' if _oc_v else "⏳ czeka")})
    ...
    for _c in sorted(_calls_op, key=_mom, reverse=True):
        ...
        _r1.append({
            ...
            "STATUS": _status(_c),
            "GRUPA": _kraj(_c),
            "UWAGI": (("z woreczka (po zleceniu) · " if not _przed_wpisem(_c) else "")
    ...
    # ══ 3. ZLECENIA NA FORUM ════════════════════════════════════════════════════
    _K2 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "PRÓBA",
           "CZY PODJĘTE / przez kogo", "STATUS", "ZLECIŁ", "GRUPA", "UWAGI"]
    ...
    for _z in sorted(_zlec_only, key=_mom, reverse=True):
        ...
        _uw = ("ponaglenie" if _z.get("typ") == "ponaglenie" else "zlecenie") \
              + (" · wpis ręczny" if _z.get("recznie") else "")
        if _cc:
            # Dwie jedyne możliwości: odpowiedź TELEFONISTKI albo operator, który pobrał z WORECZKA.
            _delta = ""
            if _t0 and _t1:
                _mn = (_t1 - _t0).total_seconds() / 60
                if _mn >= 0:
                    _delta = f' (+{int(_mn//60)}h {int(_mn%60)}min)'
                    if _mn > 2*24*60:
                        _uw += " · podjęte po terminie"
            ...
            _prz = ((_now - _t0).total_seconds()/3600) if _t0 else 0
            if _prz > 48:
                _uw += " · ⚠️ przeterminowane (>2 dni)"
        else:
        ...
        _r2.append({
            ...
            "DATA / GODZINA": _mom(_z), "DO KOGO": _z.get("do_kogo", ""),
            "PRÓBA": _z.get("_proba_nr", 1),
            "CZY PODJĘTE / przez kogo": _podj, "STATUS": _status(_cc),
            "ZLECIŁ": _z.get("zlecil", ""), "GRUPA": _kraj(_z),
            "UWAGI": _uw,
        })
    ...
    # ══ 4. DZIEŃ PO DNIU ════════════════════════════════════════════════════════
    st.markdown("### 📅 Dzień po dniu")
    _rd = []
    for _ds in _dni:
        _g17 = _TZ.localize(datetime.strptime(_ds, "%Y-%m-%d")).replace(hour=_KON)
        _zl_d = [z for z in _zlecenia if z.get("_dzien") == _ds]
        _cl_d = [c for c in _calls if c.get("_dzien") == _ds]
        _nw_d = [z for z in _zl_d if _niewyk_do(z, _g17)]
        _poz = [z for z in _nw_d if z.get("_call")]
        _zu = sum(1 for c in _cl_d if _z_ust(c))
        _czeka = sum(1 for z in _zl_d if not z.get("_call"))
        _rd.append({"Dzień": _ds, "Zlecone": len(_zl_d), "Telefony": len(_cl_d),
                    "Nie wyrobili": len(_nw_d), "Domknięte później": len(_poz),
                    "Skuteczność": (f"{round(100*_zu/len(_cl_d))}%" if _cl_d else "—"),
                    "Status dnia": (f"⏳ wstępny — czeka: {_czeka}" if _czeka else "✅ kompletny")})
    if _rd:
        st.dataframe(pd.DataFrame(_rd), use_container_width=True, hide_index=True)
        st.caption("„Nie wyrobili\" liczy się do 17:00 danego dnia i już się nie zmienia. Telefon wykonany "
                   "później wchodzi do dnia, w którym się odbył — dlatego suma z dni NIE równa się "
                   "„niewykonanym\" w rozliczeniu okresu.")
    ...
    st.caption("**Jakość rozmów** = ile z wykonanych telefonów dało ustalenia (czy rozmowy są skuteczne). "
               "**Wykonalność** = jaki udział w telefonach CAŁEJ grupy wykonała ta osoba — udziały w obrębie "
               "kraju sumują się do 100%. Tylko osoby USTAWIONE danego dnia jako dzwoniące oraz grupy telefonistek.")
    ...
    def _wykres_grup(_dane, _tytul):
        """Pionowe słupki: nad każdym imieniem dwie kolumny (jakość / wykonalność) + legenda."""
        _leg = ('<div style="display:flex;gap:18px;font-size:12px;color:#666;margin-bottom:10px">'
                '<span><span style="display:inline-block;width:11px;height:11px;background:#1D9E75;'
                'border-radius:2px;margin-right:5px"></span>Jakość rozmów</span>'
                '<span><span style="display:inline-block;width:11px;height:11px;background:#7F77DD;'
                'border-radius:2px;margin-right:5px"></span>Wykonalność (udział w grupie)</span></div>')
        _kol = []
        for _d in _dane:
            _j, _w = int(_d["Jakość %"]), int(_d["Wykonalność %"])
            _kol.append(
                '<div style="display:flex;flex-direction:column;align-items:center;min-width:96px;flex:1">'
                '<div style="display:flex;align-items:flex-end;gap:5px;height:130px">'
                f'<div style="width:26px;height:{max(2, int(_j*1.2))}px;background:#1D9E75;border-radius:3px 3px 0 0" title="Jakość {_j}%"></div>'
                f'<div style="width:26px;height:{max(2, int(_w*1.2))}px;background:#7F77DD;border-radius:3px 3px 0 0" title="Wykonalność {_w}%"></div>'
                '</div>'
                f'<div style="font-size:11px;color:#666;margin-top:4px">{_j}% · {_w}%</div>'
                f'<div style="font-size:12px;font-weight:500;margin-top:2px;text-align:center;line-height:1.25">{_d["Kto"]}</div>'
                f'<div style="font-size:10px;color:#999">{_d["Rozmowy"]} rozm.</div>'
                '</div>')
        st.markdown(_leg + '<div style="display:flex;gap:10px;align-items:flex-end;overflow-x:auto;'
                    'padding:6px 0 2px 0">' + "".join(_kol) + '</div>', unsafe_allow_html=True)

    for _k in _kraje:
        _cl_k = [c for c in _calls if _kraj(c) == _k]
        _suma_k = len(_cl_k)                      # baza wykonalności = wszystkie telefony w kraju
        _rows_w = []
        for (_op, _gr) in sorted(_dzw_set):
            if _gr != _k:
                continue
            _lst = [c for c in _cl_k if not _grupowy(c) and _kto(c) == _op]
            _zu = sum(1 for c in _lst if _z_ust(c))
            _rows_w.append({"Kto": _op, "Typ": f"operator {_k}",
                            "Języki": "/".join(_jez_map.get((_op, _gr), [])) or "—",
                            "Rozmowy": len(_lst),
                            "Jakość %": (round(100*_zu/len(_lst)) if _lst else 0),
                            "Wykonalność %": (round(100*len(_lst)/_suma_k) if _suma_k else 0),
                            "Uwaga": ("dzwoniący, 0 telefonów" if not _lst
                                      else ("mała próba" if len(_lst) < 5 else ""))})
        # telefonistki — IMIENNIE (osoba z nawiasu), a pod spodem wiersz zbiorczy grupy
        for _gd in sorted({_kto_disp(c) for c in _cl_k if _grupowy(c)}):
            ...
            _grp = _kto(_lst[0]) if _lst else "?"
            _rows_w.append({"Kto": _gd, "Typ": f"telefonistka ({_grp})", "Języki": "—", "Rozmowy": len(_lst),
                            "Jakość %": _jp,
                            "Wykonalność %": (round(100*len(_lst)/_suma_k) if _suma_k else 0),
                            "Uwaga": ("mała próba" if 0 < len(_lst) < 5 else "")})
        for _g in sorted({_kto(c) for c in _cl_k if _grupowy(c)}):
            _lst = [c for c in _cl_k if _grupowy(c) and _kto(c) == _g]
            _zu = sum(1 for c in _lst if _z_ust(c))
            _rows_w.append({"Kto": f"▸ {_g} — RAZEM", "Typ": "grupa (suma)", "Języki": "—", "Rozmowy": len(_lst),
                            "Jakość %": (round(100*_zu/len(_lst)) if _lst else 0),
                            "Wykonalność %": (round(100*len(_lst)/_suma_k) if _suma_k else 0),
                            "Uwaga": ""})
        if not _rows_w:
        ...
        st.dataframe(pd.DataFrame(_rows_w), use_container_width=True, hide_index=True)
        _wykres_grup(sorted([r for r in _rows_w if r["Typ"] != "grupa (suma)"],
                            key=lambda r: -r["Wykonalność %"]), _k)
    ...
    for _k in _kraje:
        ...
        if not _cl_k:
            continue
        for _lbl, _sel in (("Telefonistki", True), ("Operatorzy dzwoniący", False)):
            _lst = [c for c in _cl_k if _grupowy(c) == _sel]
            _zu = sum(1 for c in _lst if _z_ust(c))
            _rows_vs.append({"Kraj": _k, "Kto": _lbl, "Rozmowy": len(_lst),
                             "Z ustaleniami": _zu,
                             "Jakość %": (round(100*_zu/len(_lst)) if _lst else 0),
                             "Udział w kraju %": round(100*len(_lst)/len(_cl_k))})
    if _rows_vs:
        st.dataframe(pd.DataFrame(_rows_vs), use_container_width=True, hide_index=True)
    else:
        st.caption("Brak telefonów w wybranym zakresie.")

    # ══ 6d. TELEFONISTKI IMIENNIE ═══════════════════════════════════════════════
    _tel_all = [c for c in _calls if _grupowy(c)]
    if _tel_all:
        st.markdown("### 👩‍💼 Telefonistki imiennie")
        st.caption("Kto konkretnie w grupie wykonuje telefony — imię odczytane z podpisu wpisu na forum. "
                   "„nie udało się określić\" = wpis bez osoby w nawiasie (np. z konta osobistego).")
        _ti = {}
        for _c in _tel_all:
            _key = (_kto(_c), (_c.get("osoba") or "").strip() or "nie udało się określić", _kraj(_c))
            _a = _ti.setdefault(_key, {"Grupa": _kto(_c), "Osoba": _key[1], "Kraj": _kraj(_c),
                                       "Telefony": 0, "Z ustaleniami": 0, "Nieodebrane": 0,
                                       "Nieproduktywne": 0, "Do oceny": 0})
            _a["Telefony"] += 1
            _w = _c.get("wynik") or ""
            if _w == "do_oceny":
                _a["Do oceny"] = _a.get("Do oceny", 0) + 1
            elif _z_ust(_c):
                _a["Z ustaleniami"] += 1
            elif _w in ("nieodebral", "brak_kontaktu"):
                _a["Nieodebrane"] += 1
            else:
                _a["Nieproduktywne"] += 1
        _rows_ti = []
        for _v in sorted(_ti.values(), key=lambda x: (x["Kraj"], -x["Telefony"])):
            _oc = _v["Telefony"] - _v.get("Do oceny", 0)
            _rows_ti.append({**_v, "Skuteczność": (f'{round(100*_v["Z ustaleniami"]/_oc)}%' if _oc > 0 else "⏳ czeka")})
        st.dataframe(pd.DataFrame(_rows_ti), use_container_width=True, hide_index=True)
        _bez_osoby = sum(v["Telefony"] for k, v in _ti.items() if k[1] == "nie udało się określić")
        if _bez_osoby:
            st.caption(f"⚠️ {_bez_osoby} telefonów bez rozpoznanej osoby — wpisy nie miały podpisu w nawiasie.")

    # ══ 6b2. DO KONTROLI I DO ZROBIENIA ═════════════════════════════════════════
    _niepro = [c for c in _calls if (c.get("wynik") or "") == "nieproduktywne"]
    ...
    if _niepro or _z_ozn:
        ...
        with _kk1:
            ...
            st.caption("Rozmowy, które nic nie wniosły. Osoba sprawdzająca podsumowania powinna je "
                       "zweryfikować na forum — stąd numer wpisu.")
            if _niepro:
    ...
    for _ds in _oc_dni:
        try:
            for _d in db.collection(col("ew_phone_log")).document(_ds).collection("calls").stream():
                ...
                _v["_id"] = _d.id
                # 🟥 TRZY warunki naraz:
                #    (1) automat nie ocenił rozmowy,
                #    (2) operator PRZEROBIŁ sprawę w wybranych dniach,
                #    (3) sama ROZMOWA jest z dnia przerobienia albo poprzedniego dnia roboczego.
                #    Bez (3) do kolejki wpadały rozmowy sprzed tygodni, zapisane starym mechanizmem
                #    przy okazji otwarcia sprawy — a to nie jest bieżąca praca operatora.
                _wd = str(_v.get("wykryte_dnia") or "")
                _swieza = False
                if _wd and _wd in _dni_set:
                    try:
                        _d_roz = datetime.strptime(str(_v.get("data_str") or _ds)[:10], "%Y-%m-%d").date()
                        _d_prz = datetime.strptime(_wd[:10], "%Y-%m-%d").date()
                        _rob, _cur = 0, _d_roz
                        while _cur < _d_prz and _rob < 10:
                            _cur += timedelta(days=1)
                            if _cur.weekday() < 5:
                                _rob += 1
                        _swieza = _rob <= 1        # ten sam dzień albo poprzedni DZIEŃ ROBOCZY
                    except Exception:
                        _swieza = False
                if _swieza:
            ...
            for _nr_oc2, _lst_oc in _grp_oc.items():
                ...
                with _e1:
                    ...
                    for _cx in _lst_oc:
                        ...
                        st.caption(f'🕒 {_cx.get("forum_godzina") or _cx.get("godzina","")}'
                        ...
                        try:
                            ...
                            for _cx in _lst_oc:
                                ...
                                # 🟥 NIE kasujemy — oznaczamy. Nic wykonanego nie może zniknąć.
                                    _ref.update({
                                        "wynik": ("bez_rozmowy" if _val == "__usun__" else _val),
    ...
    with st.expander("🔎 Surowy log zdarzeń (diagnostyka)", expanded=False):
        ...
        for _z in _deleg:
            _raw.append({"MOMENT": _mom(_z), "TYP": (_z.get("typ") or "zlecenie").upper(),
                         ...
                         "GRUPA": _kraj(_z), "ID POSTU": _z.get("id_postu", "")})
        ...
        st.dataframe(pd.DataFrame(sorted(_raw, key=lambda r: r["MOMENT"], reverse=True),
                                  columns=["MOMENT", "TYP", "NR", "KTO", "DO KOGO", "WYNIK", "ŹRÓDŁO", "GRUPA", "ID POSTU"]),
                     use_container_width=True, hide_index=True)
