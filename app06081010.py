    ...
    with st.expander("🕓 Historia spraw — pełny przebieg krok po kroku", expanded=False):
        ...
        for _z in _deleg_ext:
            ...
            _hist.setdefault(_nrh, []).append({
                ...
                "txt": (("podbicie" if _pon else "zlecenie")
                        + f' — {_z.get("zlecil") or "?"}'
                        + (f' → {_z.get("do_kogo")}' if _z.get("do_kogo") else "")),
        ...
        for _c in _calls_ext:
            ...
            _hist.setdefault(_nrh, []).append({
                ...
                "txt": (f'{_kto_disp(_c)}' + ("" if _grupowy(_c) else " (sesja)")
                        + f' · {_status(_c)}'),
                "typ": "tel",
            })
        ...
        if not _karty:
            ...
            for _k in _karty[:_LIMIT]:
                ...
                for _r in _k["kroki"]:
                    ...
                    _lin.append(f'<span style="color:#888">{_r["t"]}</span> {_r["ico"]} {_r["txt"]}{_delta}')
                    _prev = _r
                if _k["czeka"]:
