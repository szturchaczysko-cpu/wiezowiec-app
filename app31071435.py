...
with tab_telefony:
    ...
    for _z in _deleg_ext:
        ...
        # Starsze telefony operatorów nie mają zapisanego numeru zlecenia — wtedy wiążemy po czasie,
        # ale z MINIMALNYM ODSTĘPEM: rozmowa w tej samej minucie co wpis to ta, która ten wpis
        # WYWOŁAŁA (operator zadzwonił, nie dodzwonił się, poszło zlecenie) — nie jego wykonanie.
        _MIN_ODSTEP = timedelta(minutes=2)
        _po += [c for c in _cbn.get(_nr_z, [])
                if not c.get("zlecenie_post_id") and not _grupowy(c)
                and c.get("_wpis_po") is not _z and _dt(c) and _dt(_z)
                and _dt(c) >= _dt(_z) + _MIN_ODSTEP]
        _po.sort(key=_mom)
