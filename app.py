...
with tab_telefony:
    ...
    _KONIEC_DNIA = 17          # po tej godzinie nikt już nie dzwoni
    ...
    _dni_ext = _dni_range(_t_from, _t_to, _extra=5)     # +5 dni: telefony do zleceń z zakresu

    def _pobierz(_lista_dni, _sub):
        _out = []
        for _ds in _lista_dni:
            try:
    ...
    with _c3:
        _kraje = sorted({str(d.get("grupa") or "?") for d in _deleg} | {str(c.get("grupa") or "?") for c in _calls})
        _f_kraj = st.multiselect("Kraj / grupa operatorska:", _kraje, key="_tel_kraj")
    if _f_kraj:
    ...
    def _status(_c):
        if not _c:
            return ""
        _w = _c.get("wynik") or ""
        if _w == "konkret" and _c.get("kurier_ustalony"):
            return "ZAMÓWIONY"
        return _STAT.get(_w, _w)

    def _z_ustaleniami(_c):
    ...
    _ponaglenia = [d for d in _deleg if (d.get("typ") or "") == "ponaglenie"]

    # dopasowanie zlecenie → telefony po nim (także poza zakresem)
    _calls_by_num = {}
    for _c in _calls_ext:
        _calls_by_num.setdefault(str(_c.get("numer_zamowienia") or "").strip(), []).append(_c)
    for _z in _deleg:
        _nr = str(_z.get("numer_zamowienia") or "").strip()
        _po = sorted([c for c in _calls_by_num.get(_nr, []) if _mom(c) >= _mom(_z)], key=_mom)
        _z["_call"] = _po[0] if _po else None
        _z["_ile_tel"] = len(_po)

    # numeracja prób w obrębie sprawy
    _prob = {}
    for _c in sorted(_calls_ext, key=_mom):
        _nr = str(_c.get("numer_zamowienia") or "").strip()
        _prob[_nr] = _prob.get(_nr, 0) + 1
        _c["_proba"] = _prob[_nr]
    for _c in _calls_ext:
        _c["_prob_razem"] = _prob.get(str(_c.get("numer_zamowienia") or "").strip(), 1)

    _now = datetime.now(_TZ)
    _koniec_zakresu = _TZ.localize(datetime.combine(_t_to, datetime.min.time())).replace(hour=_KONIEC_DNIA)

    def _niewykonane_do(_z, _granica):
        _cc = _z.get("_call")
        if not _cc:
            return True
        _d = _dt(_cc)
        return bool(_d and _d > _granica)

    _niewyk_okres = [z for z in _zlecenia if _niewykonane_do(z, min(_koniec_zakresu, _now))]

    # ══ 1. ROZLICZENIE OKRESU ═══════════════════════════════════════════════════
    st.markdown("### 🥧 Rozliczenie okresu")
    st.caption(f"Podstawa 100% = telefony wykonane w okresie + zlecenia, przy których nikt nie zadzwonił "
               f"(stan do {_KONIEC_DNIA}:00 ostatniego dnia). Każde zdarzenie liczy się raz.")
    _udzial = {}
    for _c in _calls:
        _a = _udzial.setdefault(_kto(_c), {"Kto": _kto(_c),
                                           "Typ": ("grupa telefonistek" if _jest_grupa(_c) else "operator"),
                                           "Telefony": 0, "Z ustaleniami": 0})
        _a["Telefony"] += 1
        if _z_ustaleniami(_c):
            _a["Z ustaleniami"] += 1
    _baza = len(_calls) + len(_niewyk_okres)
    _rows = []
    for _v in sorted(_udzial.values(), key=lambda x: -x["Telefony"]):
        _rows.append({**_v, "Udział": (f'{round(100*_v["Telefony"]/_baza)}%' if _baza else "—"),
                      "Skuteczność": (f'{round(100*_v["Z ustaleniami"]/_v["Telefony"])}%' if _v["Telefony"] else "—")})
    if _niewyk_okres:
        _rows.append({"Kto": "⚠️ NIEWYKONANE", "Typ": "—", "Telefony": len(_niewyk_okres), "Z ustaleniami": 0,
                      "Udział": (f'{round(100*len(_niewyk_okres)/_baza)}%' if _baza else "—"), "Skuteczność": "—"})
    if _rows:
        st.dataframe(pd.DataFrame(_rows), use_container_width=True, hide_index=True)
        _m1, _m2, _m3, _m4 = st.columns(4)
        _m1.metric("Do zrobienia (100%)", _baza)
        _m2.metric("Telefony wykonane", len(_calls))
        _m3.metric("Niewykonane", len(_niewyk_okres))
        _zu = sum(1 for c in _calls if _z_ustaleniami(c))
        _m4.metric("Skuteczność ogółem", f"{round(100*_zu/len(_calls))}%" if _calls else "—")
        # skuteczność per sprawa
        _spr = {}
        for _c in _calls:
            _n = str(_c.get("numer_zamowienia") or "").strip()
            _spr[_n] = _spr.get(_n, False) or _z_ustaleniami(_c)
        if _spr:
            st.caption(f"Skuteczność per sprawa: {sum(1 for v in _spr.values() if v)} z {len(_spr)} spraw "
                       f"domkniętych telefonem ({round(100*sum(1 for v in _spr.values() if v)/len(_spr))}%).")
    else:
    ...
    _rows_d = []
    for _ds in _dni:
        _gr17 = _TZ.localize(datetime.strptime(_ds, "%Y-%m-%d")).replace(hour=_KONIEC_DNIA)
        _zl_d = [z for z in _zlecenia if z.get("_dzien") == _ds]
        _cl_d = [c for c in _calls if c.get("_dzien") == _ds]
        _niewyk = [z for z in _zl_d if _niewykonane_do(z, _gr17)]
        _pozniej = [z for z in _niewyk if z.get("_call")]
        _zu_d = sum(1 for c in _cl_d if _z_ustaleniami(c))
        _czeka = sum(1 for z in _zl_d if not z.get("_call"))
        _rows_d.append({
            "Dzień": _ds,
            "Zlecone": len(_zl_d),
            "Telefony": len(_cl_d),
            "Nie wyrobili": len(_niewyk),
            "Domknięte później": len(_pozniej),
            "Skuteczność": (f"{round(100*_zu_d/len(_cl_d))}%" if _cl_d else "—"),
            "Status dnia": ("⏳ wstępny — czeka na przerobienie: " + str(_czeka)) if _czeka else "✅ kompletny",
        })
    if _rows_d:
        st.dataframe(pd.DataFrame(_rows_d), use_container_width=True, hide_index=True)
        st.caption("„Nie wyrobili\" liczy się do 17:00 danego dnia i już się nie zmienia. Telefon wykonany później "
                   "wchodzi do dnia, w którym się odbył — dlatego suma z dni NIE równa się „niewykonanym\" w rozliczeniu okresu.")

    # ══ 3. PODZIAŁ NA KRAJE ═════════════════════════════════════════════════════
    if not _f_kraj:
        st.markdown("### 🌍 Podział na kraje")
        _rows_k = []
        for _k in sorted({str(d.get("grupa") or "?") for d in _zlecenia} | {str(c.get("grupa") or "?") for c in _calls}):
            _zl_k = [z for z in _zlecenia if str(z.get("grupa") or "?") == _k]
            _cl_k = [c for c in _calls if str(c.get("grupa") or "?") == _k]
            _nw_k = [z for z in _zl_k if _niewykonane_do(z, min(_koniec_zakresu, _now))]
            _zu_k = sum(1 for c in _cl_k if _z_ustaleniami(c))
            _rows_k.append({"Kraj": _k, "Zlecone": len(_zl_k), "Telefony": len(_cl_k),
                            "Niewykonane": len(_nw_k),
                            "Skuteczność": (f"{round(100*_zu_k/len(_cl_k))}%" if _cl_k else "—")})
        if _rows_k:
            st.dataframe(pd.DataFrame(_rows_k), use_container_width=True, hide_index=True)

    # ══ 4. JAKOŚĆ I WYKONALNOŚĆ ═════════════════════════════════════════════════
    st.markdown("### 📊 Jakość i wykonalność")
    st.caption("**Jakość** = % rozmów z ustaleniami. **Wykonalność** = ile z tego, co należało do tej osoby/grupy, "
               "faktycznie wzięła na siebie. Z bazy grupy wypadają zlecenia przejęte przez operatora; "
               "z bazy operatora — zlecenia w językach, których nie obsługuje.")
    try:
        _cfg_ops = {d.id: (d.to_dict() or {}) for d in db.collection(col("operator_configs")).stream()}
    except Exception:
        _cfg_ops = {}

    _rows_w = []
    # operatorzy — imiennie
    for _o, _lst in sorted({k: [c for c in _calls if _kto(c) == k and not _jest_grupa(c)]
                            for k in {_kto(c) for c in _calls if not _jest_grupa(c)}}.items()):
        if not _lst:
            continue
        _lang = [str(x).upper() for x in (_cfg_ops.get(_o, {}) or {}).get("jezyki_dzwoniacy", [])]
        _oddane = [z for z in _zlecenia if z.get("zlecil") == _o
                   and (not _lang or str(z.get("jezyk") or "").upper() in _lang)]
        _mian = len(_lst) + len(_oddane)
        _zu = sum(1 for c in _lst if _z_ustaleniami(c))
        _rows_w.append({"Kto": _o, "Typ": "operator", "Rozmowy": len(_lst),
                        "Jakość %": round(100*_zu/len(_lst)),
                        "Wykonalność %": (round(100*len(_lst)/_mian) if _mian else 0),
                        "Uwaga": ("mała próba" if len(_lst) < 5 else "")})
    # grupy telefonistek
    for _g in sorted({_kto(c) for c in _calls if _jest_grupa(c)} | {str(z.get("do_kogo") or "") for z in _zlecenia if z.get("do_kogo")}):
        _lst = [c for c in _calls if _jest_grupa(c) and _kto(c) == _g]
        _do_niej = [z for z in _zlecenia if str(z.get("do_kogo") or "") == _g]
        _przejete = [z for z in _do_niej if z.get("_call") and not _jest_grupa(z["_call"])]
        _mian = len(_do_niej) - len(_przejete)
        _zu = sum(1 for c in _lst if _z_ustaleniami(c))
        _rows_w.append({"Kto": _g, "Typ": "grupa telefonistek", "Rozmowy": len(_lst),
                        "Jakość %": (round(100*_zu/len(_lst)) if _lst else 0),
                        "Wykonalność %": (round(100*len(_lst)/_mian) if _mian > 0 else 0),
                        "Uwaga": ("mała próba" if len(_lst) < 5 else "")})
    if _rows_w:
        _dfw = pd.DataFrame(_rows_w)
        st.dataframe(_dfw, use_container_width=True, hide_index=True)
        try:
            st.bar_chart(_dfw.set_index("Kto")[["Jakość %", "Wykonalność %"]])
        except Exception:
            pass
        st.caption("Przy mniej niż 5 rozmowach procenty są mało wiarygodne — jeden wynik przesuwa je o 20 punktów.")
    else:
        st.caption("Brak danych do porównania.")

    # ══ 5. TABELA — TELEFONY ════════════════════════════════════════════════════
    _KOL1 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "CZY PODJĘTE / przez kogo",
             "STATUS", "CZY W TERMINIE", "PRÓBA", "GRUPA", "UWAGI"]
    st.markdown("### ☎️ Telefony wykonane")
    _r1 = []
    for _c in sorted(_calls, key=_mom, reverse=True):
        _grupowy = _jest_grupa(_c)
        _r1.append({
            "NR ZAMÓWIENIA": _c.get("numer_zamowienia", ""),
            "ID POSTU": ("forum" if _grupowy else "sesja"),
            "DATA / GODZINA": _mom(_c),
            "DO KOGO": _kto(_c),
            "CZY PODJĘTE / przez kogo": f'TAK {_mom(_c)} / {_kto(_c)}',
            "STATUS": _status(_c),
            "CZY W TERMINIE": "TAK (od razu)" if not _grupowy else "—",
            "PRÓBA": (f'{_c.get("_proba",1)} z {_c.get("_prob_razem",1)}' if _c.get("_prob_razem", 1) > 1 else "1"),
            "GRUPA": _c.get("grupa", ""),
            "UWAGI": ("wykryte " + _c.get("wykryte_dnia", "")) if _c.get("wykryte_dnia") else
                     ("telefon owocny" if _z_ustaleniami(_c) else "bez ustaleń"),
        })
    _df1 = pd.DataFrame(_r1, columns=_KOL1)
    ...
    # ══ 6. TABELA — ZLECENIA ════════════════════════════════════════════════════
    _KOL2 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "CZY PODJĘTE / przez kogo",
    ...
    for _z in sorted(_deleg, key=_mom, reverse=True):
        ...
        if _cc:
            _podj = (f'TAK {_mom(_cc)} / {_kto(_cc)}' if _jest_grupa(_cc)
                     else f'NIE / zadzwonił {_kto(_cc)} {_mom(_cc)}')
            if _t0 and _t1:
                _mn = (_t1 - _t0).total_seconds() / 60
                _d = f'{int(_mn//60)}h {int(_mn%60)}min'
                _term = (f'✅ ten sam dzień (+{_d})' if _cc.get("_dzien") == _z.get("_dzien")
                         else (f'⚠️ po terminie (+{_d})' if _mn > 2*24*60 else f'❌ inny dzień (+{_d})'))
            else:
                _term = "—"
        else:
        ...
        _r2.append({
            ...
            "ID POSTU": _z.get("id_postu", ""),
            "DATA / GODZINA": _mom(_z),
            "DO KOGO": _z.get("do_kogo", ""),
            "CZY PODJĘTE / przez kogo": _podj,
            "STATUS": _status(_cc),
            "CZY W TERMINIE": _term,
            "TELEFONÓW": _z.get("_ile_tel", 0),
            "ZLECIŁ": _z.get("zlecil", ""),
            "GRUPA": _z.get("grupa", ""),
            "UWAGI": ("ponaglenie" if _z.get("typ") == "ponaglenie" else "zlecenie")
                     + (" · wpis ręczny" if _z.get("recznie") else ""),
        })
    _df2 = pd.DataFrame(_r2, columns=_KOL2)
    ...
    if _zlecenia:
        ...
        _byz = {}
        for _z in _zlecenia:
            _a = _byz.setdefault(_z.get("zlecil") or "?", {"Operator": _z.get("zlecil") or "?",
    ...
    # ══ 7. SUROWY LOG (DIAGNOSTYKA) ═════════════════════════════════════════════
    with st.expander("🔎 Surowy log zdarzeń (diagnostyka)", expanded=False):
        st.caption("Dokładnie to, co zapisało się do bazy — bez liczenia. Służy do sprawdzenia, "
                   "czy telefony trafiają na właściwe konta.")
        _raw = []
        for _z in _deleg:
            _raw.append({"MOMENT": _mom(_z), "TYP": (_z.get("typ") or "zlecenie").upper(),
                         ...
                         "GRUPA": _z.get("grupa", ""), "ID POSTU": _z.get("id_postu", "")})
        for _c in _calls:
            _raw.append({"MOMENT": _mom(_c), "TYP": "TELEFON", "NR": _c.get("numer_zamowienia", ""),
                         "KTO": _kto(_c), "DO KOGO": "", "WYNIK": _c.get("wynik", ""),
                         "ŹRÓDŁO": _c.get("zrodlo", ""), "GRUPA": _c.get("grupa", ""), "ID POSTU": ""})
        _dfr = pd.DataFrame(sorted(_raw, key=lambda r: r["MOMENT"], reverse=True),
