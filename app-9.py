...
def parse_wiezowiec_output(text):
    ...
    grupa_patterns = {
        ...
        # \b: "OPERATORZY UK" nie złapie "OPERATORZY UKPL" (po K jest P)
        "UK": r'▬+\s*OPERATORZY\s+UK\b',
        "PL": r'▬+\s*OPERATORZY\s+PL\b',
        "UKPL": r'▬+\s*OPERATORZY\s+UKPL',   # LEGACY → normalizowane do UK niżej
    }
    lines = text.split('\n')
    i = 0
    while i < len(lines):
        ...
        for grupa, pattern in grupa_patterns.items():
            if re.search(pattern, line):
                # LEGACY: stary wsad z sekcją UKPL → traktujemy jako UK (PL wyodrębniona osobno)
                current_grupa = "UK" if grupa == "UKPL" else grupa
                break
...
GRUPA_MAP_GLOBAL = {"DE": "Operatorzy_DE", "FR": "Operatorzy_FR", "UK": "OPERATORZY_UK",
                    "PL": "OPERATORZY_PL", "UKPL": "Operatorzy_UK/PL"}   # UK/PL WIELKIMI; UKPL = legacy
...
_GRUPA_KEYS = ["DE", "FR", "UK", "PL"]
_GRUPA_FLAGS = {"DE": "🇩🇪", "FR": "🇫🇷", "UK": "🇬🇧", "PL": "🇵🇱"}
...
def render_group_summary_now():
    ...
    for d in docs:
        ...
        if s in ("odroczony", "zakonczony", "pominiety"):
            ...
            g[grp][key] += 1
            firma[key] += 1
    cols = st.columns(len(_GRUPA_KEYS))     # 4 grupy: DE/FR/UK/PL (było 3 → PL się ucinała)
...
def render_group_summary_range(d_from, d_to):
    ...
    for ds, day in daily.items():
        ...
        ndays += 1
    if ndays > 0:
        cols = st.columns(len(_GRUPA_KEYS))     # 4 grupy: DE/FR/UK/PL
        for c, gname in zip(cols, _GRUPA_KEYS):
            with c:
...
tab_wsady, tab_generuj, tab_autopilot, tab_batches, tab_cases, tab_skipped, tab_prompty, tab_hasla, tab_zastepstwa, tab_diamentoza, tab_telefony = st.tabs([
    "📂 Wsady",
    "⚡ Generuj + Autopilot",
    "🤖 Dolewka + Status",
    "📦 Historia partii",
    ...
    "🧪 Prompty",
    "⚙️ Ustawienia operatorów",
    "⚙️ Ustawienia sprzedawców",
    "💎 Diamentoza",
    "📞 Rejestr telefonów",
])
...
with tab_generuj:
    ...
    GRUPA_MAP = {"DE": "Operatorzy_DE", "FR": "Operatorzy_FR", "UK": "OPERATORZY_UK", "PL": "OPERATORZY_PL"}
    ROLE_TO_GRUPA = {"Operatorzy_DE": "DE", "Operatorzy_FR": "FR", "OPERATORZY_UK": "UK",
                     "OPERATORZY_PL": "PL", "Operatorzy_UK": "UK", "Operatorzy_PL": "PL",
                     "Operatorzy_UK/PL": "UK"}   # tolerancja pisowni; UK/PL = legacy → UK
    
    ops_by_grupa = {"DE": [], "FR": [], "UK": [], "PL": []}
    for op_name_candidate in ALL_OPERATORS_LIST:
        try:
            cfg_doc = db.collection(col("operator_configs")).document(op_name_candidate).get()
            if cfg_doc.exists:
                if not cfg_doc.to_dict().get("pracuje", True):
                    continue          # poza bieżącą obsadą → nie pokazuj na liście
    ...
    with col_obs3:
        st.markdown("**🇬🇧 UK**")
        gen_ops_uk = st.multiselect("Operatorzy UK:", ops_by_grupa["UK"], key="gen_ops_uk")
    with col_obs4:
        st.markdown("**🇵🇱 PL**")
        gen_ops_pl = st.multiselect("Operatorzy PL:", ops_by_grupa["PL"], key="gen_ops_pl")
    
    gen_obsada = {}
    if gen_ops_de: gen_obsada["DE"] = gen_ops_de
    if gen_ops_fr: gen_obsada["FR"] = gen_ops_fr
    if gen_ops_uk: gen_obsada["UK"] = gen_ops_uk
    if gen_ops_pl: gen_obsada["PL"] = gen_ops_pl
    ...
    def _save_cases_to_db(batch_cases, batch_num, total_batches):
        ...
        # PL = tylko Polska. UK = cała reszta (Luxembourg, Portugal, Sweden, Netherlands, UK, itd.)
        PL_COUNTRIES = {"poland", "polska", "polen", "pologne"}
        
        def detect_country_grupa(text):
            """Wykryj kraj z pelna_linia_szturchacza i zwróć poprawną grupę.
            DE/FR/PL — jawne listy. Każdy inny wykryty kraj → UK. Brak kraju → None."""
            ...
            for country in FR_COUNTRIES:
                if country in text_lower:
                    return "FR"
            # Polska → własna grupa PL (wyodrębniona z UK)
            for country in PL_COUNTRIES:
                if country in text_lower:
                    return "PL"
            # Szukaj znanych krajów → UK
            known_countries = [
                "luxembourg", "portugal", "netherlands", "sweden", "denmark",
...
"""
            ...
            if data_obrobki:
                ...
                if st.session_state.get("_ew_plan_written_for") != _ds_plan:
                    try:
                        ...
                        _plan_count = {g: {"total": 0, "odsiane": 0} for g in ["DE", "FR", "UK", "PL"]}
    ...
    if raw_output:
        ...
        if all_cases:
            ...
            uk = [c for c in all_cases if c.get("grupa") in ("UK", "UKPL")]   # UKPL = legacy
            pl = [c for c in all_cases if c.get("grupa") == "PL"]
            st.success(f"📊 Dotychczas przeliczono: **{len(all_cases)}** casów — DE={len(de)} | FR={len(fr)} | UK={len(uk)} | PL={len(pl)}")
...
with tab_autopilot:
    ...
    ROLE_TO_GRUPA_DL = {"Operatorzy_DE": "DE", "Operatorzy_FR": "FR", "OPERATORZY_UK": "UK",
                        "OPERATORZY_PL": "PL", "Operatorzy_UK": "UK", "Operatorzy_PL": "PL",
                        "Operatorzy_UK/PL": "UK"}   # legacy → UK
    ops_by_grupa_dl = {"DE": [], "FR": [], "UK": [], "PL": []}
    for op_c in ALL_OPERATORS_LIST_DL:
        try:
            cfg_doc = db.collection(col("operator_configs")).document(op_c).get()
            if cfg_doc.exists:
                if not cfg_doc.to_dict().get("pracuje", True):
                    continue          # poza bieżącą obsadą
                role = cfg_doc.to_dict().get("role", "Operatorzy_DE")
    ...
    col_do1, col_do2, col_do3, col_do4 = st.columns(4)
    with col_do1:
    ...
    with col_do3:
        dl_ops_uk = st.multiselect("🇬🇧 UK:", ops_by_grupa_dl["UK"], default=[o for o in gen_obs.get("UK", []) if o in ops_by_grupa_dl["UK"]], key="dl_ops_uk")
    with col_do4:
        dl_ops_pl = st.multiselect("🇵🇱 PL:", ops_by_grupa_dl["PL"], default=[o for o in gen_obs.get("PL", []) if o in ops_by_grupa_dl["PL"]], key="dl_ops_pl")
    
    dl_obsada = {}
    if dl_ops_de: dl_obsada["DE"] = dl_ops_de
    if dl_ops_fr: dl_obsada["FR"] = dl_ops_fr
    if dl_ops_uk: dl_obsada["UK"] = dl_ops_uk
    if dl_ops_pl: dl_obsada["PL"] = dl_ops_pl
    ...
    bak_data = {"DE": {"w_baku": 0, "do_dolania": 0}, "FR": {"w_baku": 0, "do_dolania": 0},
                "UK": {"w_baku": 0, "do_dolania": 0}, "PL": {"w_baku": 0, "do_dolania": 0}}
    ...
    col_bak1, col_bak2, col_bak3, col_bak4 = st.columns(4)
    dolewka_pcts = {}
    
    for col_b, gname, flag in [(col_bak1, "DE", "🇩🇪"), (col_bak2, "FR", "🇫🇷"),
                               (col_bak3, "UK", "🇬🇧"), (col_bak4, "PL", "🇵🇱")]:
        with col_b:
            bd = bak_data[gname]
                    ...
                    if ai_response:
                        ...
                        if FORUM_ENABLED:
                            for forum_iter in range(3):  # max 3 iteracje forum
                                ...
                                # Mapuj grupa_operatorska → krótka grupa (DE/FR/UK/PL) do logu
                                _ROLE_TO_GRUPA_DIAM = {"Operatorzy_DE": "DE", "Operatorzy_FR": "FR", "OPERATORZY_UK": "UK",
                                                       "OPERATORZY_PL": "PL", "Operatorzy_UK/PL": "UK"}
                                ...
                                for fw in forum_result.get("forum_writes", []):
                                    if fw.get("success"):
                                        ...
                                        _pid_ap = fw.get("new_post_id") or fw.get("FORUM_ID")
                                        st.caption(f"  📤 Forum WRITE: post {_pid_ap or '?'} wysłany")
                                        if nrzam and _pid_ap and fw.get("cel"):
                                            save_forum_memory(db, col, nrzam, fw["cel"], _pid_ap, fw.get("tresc_skrot", ""))
                                        # 🟥 REJESTR TELEFONÓW: autopilot to normalny operator —
                                        #    jego delegacje i ponaglenia MUSZĄ trafić do panelu.
                                        #    Dotąd zapisywał tylko pamięć forum, więc nocne wpisy
                                        #    chatoszturka były niewidoczne w podsumowaniach.
                                        try:
                                            _tr_ap = str(fw.get("tresc_skrot") or "")
                                            _ud_ap = str(fw.get("user_do") or fw.get("fallback_user_do") or "")
                                            if (nrzam and _pid_ap and _ud_ap.lower().startswith("telefoni")):
                                                _tz_ap = pytz.timezone("Europe/Warsaw")
                                                _now_ap = datetime.now(_tz_ap)
                                                _ds_ap = _now_ap.strftime("%Y-%m-%d")
                                                _nr_ap = str(nrzam).strip()
                                                # czy w tej sprawie jest już wpis → wtedy PONAGLENIE
                                                _byl_ap = False
                                                for _i_ap in range(14):
                                                    _dd_ap = (_now_ap - timedelta(days=_i_ap)).strftime("%Y-%m-%d")
                                                    try:
                                                        _h_ap = db.collection(col("ew_phone_log")).document(_dd_ap) \
                                                                  .collection("delegacje") \
                                                                  .where("numer_zamowienia", "==", _nr_ap).limit(5).stream()
                                                        if any(True for _ in _h_ap):
                                                            _byl_ap = True
                                                            break
                                                    except Exception:
                                                        pass
                                                _jest_del_ap = bool(re.search(r"delegacja telefonu", _tr_ap, re.IGNORECASE))
                                                db.collection(col("ew_phone_log")).document(_ds_ap) \
                                                  .collection("delegacje").add({
                                                      "numer_zamowienia": _nr_ap,
                                                      "typ": ("zlecenie" if (_jest_del_ap and not _byl_ap) else "ponaglenie"),
                                                      "zlecil": "chatoszturek (automat)",
                                                      "zlecil_dzwoniacy": False,
                                                      "grupa": str(case_grupa_op or "?").replace("OPERATORZY_", ""),
                                                      "do_kogo": _ud_ap,
                                                      "id_postu": str(_pid_ap),
                                                      "link": fw.get("link") or "",
                                                      "tresc": _tr_ap[:300],
                                                      "data_str": _ds_ap,
                                                      "godzina": _now_ap.strftime("%H:%M"),
                                                      "bot": True,
                                                      "created_at": firestore.SERVER_TIMESTAMP,
                                                  })
                                        except Exception:
                        ...
                        if _ap_grupa in ("DE", "FR", "UK", "PL", "UKPL"):
                            try:
...
with tab_cases:
    ...
    with fc1:
        fg = st.selectbox("Grupa:", ["Wszystkie", "DE", "FR", "UK", "PL", "Brak grupy / Score 0"])
    with fc2:
    ...
    if not filtered:
        ...
        if odroczone_cases:
            ...
            with col_odr2:
                force_grupa = st.selectbox("Wymuś grupę (dla brakujących):", ["—", "DE", "FR", "UK", "PL"], key="odr_force_grupa")
        ...
        for doc_id, c in filtered[start:end]:
            ...
            if c.get("status") == "odroczony":
                ...
                with oc2:
                    new_grupa = st.selectbox("Grupa:", ["—", "DE", "FR", "UK", "PL"],
                                            index=["—", "DE", "FR", "UK", "PL"].index(cur_grupa) if cur_grupa in ["DE", "FR", "UK", "PL"] else 0,
...
with tab_prompty:
    ...
    # ==========================================
with tab_hasla:
    st.subheader("⚙️ Ustawienia operatorów")
    ...
    # ⚙️ FLAGA "PRACUJE" — kto jest w bieżącej obsadzie. Brak wartości = pracuje.
    _pracuje_state = {}
    for _op in _ALL_OPS_HASLA:
        try:
            _pd = db.collection(col("operator_configs")).document(_op).get().to_dict() or {}
            _pracuje_state[_op] = bool(_pd.get("pracuje", True))
        except Exception:
            _pracuje_state[_op] = True
    _nieaktywni = [o for o in _ALL_OPS_HASLA if not _pracuje_state.get(o, True)]
    _pokaz_wszystkich = st.checkbox(
        f"👁️ Pokaż też nieaktywnych ({len(_nieaktywni)})", value=False, key="_pokaz_nieaktywnych",
        help="Odznaczeni nie pojawiają się na listach obsady i NIE mogą zalogować się do Szturchacza.")
    if not _pokaz_wszystkich:
        _ALL_OPS_HASLA = [o for o in _ALL_OPS_HASLA if _pracuje_state.get(o, True)]
    ...
    _grupy_state = {}
    # 🟥 Nazwa roli = literalna nazwa grupy na forum (idzie jako nadawca/odbiorca wpisu).
    #    UK i PL są WIELKIMI literami: OPERATORZY_UK / OPERATORZY_PL.
    _ROLE_MAP = {"Operatorzy_DE": "DE", "Operatorzy_FR": "FR",
                 "OPERATORZY_UK": "UK", "OPERATORZY_PL": "PL",
                 "Operatorzy_UK": "UK", "Operatorzy_PL": "PL",      # tolerancja pisowni
                 "Operatorzy_UK/PL": "UK"}                          # legacy → UK
    _ROLE_MAP_REV = {"DE": "Operatorzy_DE", "FR": "Operatorzy_FR",
    ...
    for _op in _ALL_OPS_HASLA:
        ...
        with _c1:
            st.markdown(f"**{_op}**")
            _prac_now = _pracuje_state.get(_op, True)
            _prac_new = st.checkbox("pracuje", value=_prac_now, key=f"_prac_{_op}",
                                    help="Odznacz = znika z list obsady i nie zaloguje się do Szturchacza")
            if _prac_new != _prac_now:
                try:
                    db.collection(col("operator_configs")).document(_op).set(
                        {"pracuje": _prac_new}, merge=True)
                    st.toast(f"{'✅' if _prac_new else '🚫'} {_op}: {'pracuje' if _prac_new else 'nieaktywny'}")
                    st.rerun()
                except Exception as _e:
                    st.error(f"❌ Błąd zapisu: {_e}")
        with _c2:
            _curr_grupa = _grupy_state.get(_op, "DE")
            _options = ["DE", "FR", "UK", "PL"]
    ...
    st.caption("**Dzwoni** = ma prawo dzwonić sam. **Języki** = w jakich językach dzwoni (ustalenie — może być węższe niż realne umiejętności). "
               ...
               "„forum” zostawia tylko wpis do Telefonistów. Domyślnie „tylko forum”. Zmiana wchodzi od razu.")
    ...
    _thh1, _thh2, _thh3, _thh4 = st.columns([2, 1.3, 3, 1.3])
    with _thh1: st.markdown("**Operator**")
    with _thh2: st.markdown("**Dzwoni**")
    with _thh3: st.markdown("**Języki**")
    with _thh4: st.markdown("**Zapisz**")
    for _op in _ALL_OPS_HASLA:
        _ss = _tel_state[_op]
        _ttc1, _ttc2, _ttc3, _ttc4 = st.columns([2, 1.3, 3, 1.3])
        with _ttc1:
            st.markdown(f"**{_op}**")
        with _ttc2:
        ...
        with _ttc4:
            if st.button("💾", key=f"_tel_save_{_op}", help=f"Zapisz telefon dla {_op}"):
                try:
                    db.collection(col("operator_configs")).document(_op).set({
                        "dzwoni": _dzw, "jezyki_dzwoniacy": _jz,
                    }, merge=True)
                    st.success(f"✅ {_op}: dzwoni={_dzw}, języki={_jz or '—'}")
                    st.rerun()
                except Exception as _e:
...
# ==========================================
with tab_zastepstwa:
    st.subheader("⚙️ Ustawienia sprzedawców")
    ...
    st.markdown("---")
    st.markdown("### 📞 Sprzedawcy — dzwonienie (możliwość i języki)")
    st.caption("Dla sprzedawców, którzy dzwonią do klientów. **Dzwoni** = możliwość dzwonienia; "
               "**Języki** = w jakich językach. Czasem telefon pójdzie bezpośrednio do sprzedawcy — "
               "reguły kierowania dodamy później, na razie sam wybór. Zapis do wspólnej konfiguracji sprzedawców.")
    _LANGI_S = ["DE", "FR", "PL", "IT", "ES", "ENG"]
    try:
        _sd_map = db.collection(col("admin_config")).document("sprzedawcy_dzwoniacy").get().to_dict() or {}
    except Exception:
        _sd_map = {}
    for _sni, (_snick, _skraj) in enumerate(SPRZEDAWCY_LISTA):
        _cur = _sd_map.get(_snick) if isinstance(_sd_map.get(_snick), dict) else {}
        _sc1, _sc2, _sc3, _sc4 = st.columns([2, 1.3, 3, 1.4])
        with _sc1:
            st.markdown(f"**{_snick}** ({_skraj})")
        with _sc2:
            _sdzw = st.checkbox("dzwoni", value=bool(_cur.get("dzwoni", False)),
                                key=f"_szdz_{_snick}", label_visibility="collapsed")
        with _sc3:
            _sjz = st.multiselect("języki", _LANGI_S,
                                  default=[str(x).upper() for x in _cur.get("jezyki", []) if str(x).upper() in _LANGI_S],
                                  key=f"_szjz_{_snick}", label_visibility="collapsed")
        with _sc4:
            if st.button("💾", key=f"_szsave_{_snick}", help=f"Zapisz dzwonienie dla {_snick}"):
                try:
                    db.collection(col("admin_config")).document("sprzedawcy_dzwoniacy").set(
                        {_snick: {"dzwoni": _sdzw, "jezyki": _sjz}}, merge=True)
                    st.success(f"✅ {_snick}: dzwoni={_sdzw}, języki={_sjz or '—'}")
                    st.rerun()
                except Exception as _e:
                    st.error(f"❌ Błąd zapisu {_snick}: {_e}")
...
with tab_diamentoza:
    ...
    with st.expander("➕ Dodaj diament ręcznie", expanded=False):
        ...
        with _rd_c6:
            if _rd_wykonawca == "Czatoszturek":
                _rd_grupa_sel = st.selectbox("🌍 Grupa (WYMAGANE):", ["— wybierz —", "DE", "FR", "UK", "PL"], key="_rd_grupa_cz")
        ...
        if st.button("💾 Zapisz diament ręcznie", type="primary", key="_rd_zapisz"):
            ...
            elif _rd_wykonawca == "Czatoszturek" and _rd_grupa not in ("DE", "FR", "UK", "PL"):
                st.error("❌ Dla wykonawcy Czatoszturek grupa jest WYMAGANA — wybierz DE/FR/UK/PL.")
            else:
    ...
    if by_day:
        ...
        st.caption("Brak diamentów w wybranym zakresie — nie ma czego pokazać na wykresie.")
    ...
    if by_op:
        ...
        st.caption("Brak diamentów w wybranym zakresie.")
...
# ==========================================
# 📞 TELEFONY — ROZLICZENIE
# ==========================================
with tab_telefony:
    st.subheader("📞 Telefony — rozliczenie")

    _TZ = pytz.timezone("Europe/Warsaw")
    _KON = 17          # koniec dnia pracy

    _c1, _c2, _c3 = st.columns([2, 2, 3])
    with _c1:
        _t_from = st.date_input("Od dnia:", value=datetime.now(_TZ).date(), key="_tel_from")
    with _c2:
        _t_to = st.date_input("Do dnia:", value=datetime.now(_TZ).date(), key="_tel_to")

    _szukaj_nr = st.text_input("🔍 Szukaj po numerze zamówienia (cała historia sprawy):",
                               key="_tel_szukaj", placeholder="np. 384178").strip()

    def _dni_range(_a, _b, _extra=0):
        _o, _c = [], _a
        while _c <= _b + timedelta(days=_extra) and len(_o) < 150:
            _o.append(_c.strftime("%Y-%m-%d")); _c += timedelta(days=1)
        return _o

    _dni = _dni_range(_t_from, _t_to)
    _dni_ext = _dni_range(_t_from - timedelta(days=14), _t_to, _extra=5)

    def _pobierz(_lista, _sub):
        _out = []
        for _ds in _lista:
            try:
                for _d in db.collection(col("ew_phone_log")).document(_ds).collection(_sub).stream():
                    _x = _d.to_dict() or {}
                    _x["_dzien"] = _ds
                    _x["_id"] = _d.id
                    _out.append(_x)
    ...
    _deleg = _pobierz(_dni, "delegacje")
    # 🟥 Do DOPASOWANIA (czy telefon był przed wpisem, czy po) potrzebne są też zlecenia
    #    sprzed zakresu — inaczej ten sam numer wygląda inaczej zależnie od ustawionych dat.
    # 🟥 Tyle samo, ile wstecz sięga wykrywanie w Szturchaczu — inaczej rozmowa istnieje,
    #    a jej zlecenia nie ma w tabeli.
    _dni_hist = _dni_range(_t_from - timedelta(days=30), _t_to, _extra=5)
    _deleg_ext = _pobierz([d for d in _dni_hist if d not in _dni], "delegacje") + _deleg
    _calls = _pobierz(_dni, "calls")
    _calls_ext = _pobierz(_dni_ext, "calls")
    _obsada = _pobierz(_dni, "obsada")
    _nieoc = _pobierz(_dni, "nieocenione")

    with _c3:
        _kraje_all = sorted({str(d.get("grupa") or "?") for d in _deleg}
                            | {str(c.get("grupa") or "?") for c in _calls}
                            | {str(o.get("grupa") or "?") for o in _obsada})
        _f_kraj = st.multiselect("Kraj / grupa operatorska:", _kraje_all, key="_tel_kraj")
    if _f_kraj:
        _deleg = [d for d in _deleg if str(d.get("grupa") or "?") in _f_kraj]
        _calls = [c for c in _calls if str(c.get("grupa") or "?") in _f_kraj]
        _calls_ext = [c for c in _calls_ext if str(c.get("grupa") or "?") in _f_kraj]
        _obsada = [o for o in _obsada if str(o.get("grupa") or "?") in _f_kraj]

    # filtr po użytkowniku — działa łącznie z filtrem kraju
    _userzy = sorted({str(c.get("operator") or "") for c in _calls if c.get("operator")}
                     | {str(d.get("zlecil") or "") for d in _deleg if d.get("zlecil")}
                     | {str(o.get("operator") or "") for o in _obsada if o.get("operator")})
    _f_user = st.multiselect("Użytkownik / grupa telefonistek:", _userzy, key="_tel_user")
    if _f_user:
        _calls = [c for c in _calls if str(c.get("operator") or "") in _f_user]
        _calls_ext = [c for c in _calls_ext if str(c.get("operator") or "") in _f_user]
        _deleg = [d for d in _deleg if str(d.get("zlecil") or "") in _f_user
                  or str(d.get("do_kogo") or "") in _f_user]
        _obsada = [o for o in _obsada if str(o.get("operator") or "") in _f_user]

    # ── NOTKA O OBSADZIE ──
    if _obsada:
        with st.expander(f"👥 Obsada — kto danego dnia był dzwoniący ({len(_dni)} dni)", expanded=True):
            for _ds in _dni:
                _ob_d = [o for o in _obsada if o.get("_dzien") == _ds]
                if not _ob_d:
                    st.markdown(f"**{_ds}** — brak danych o obsadzie")
                    continue
                _txt = []
                for _o in sorted(_ob_d, key=lambda x: (str(x.get("grupa")), str(x.get("operator")))):
                    _jz = "/".join(_o.get("jezyki") or []) or "—"
                    _txt.append((f'🟢 **{_o.get("operator")}** ({_o.get("grupa")}, dzwoni {_jz})'
                                 if _o.get("dzwoni") else
                                 f'⚪ {_o.get("operator")} ({_o.get("grupa")}, nie dzwoni)'))
                st.markdown(f"**{_ds}** — " + " · ".join(_txt))
            st.caption("🟢 dzwoniący · ⚪ tylko zleca. Zapis powstaje przy logowaniu; zmiana ustawień "
                       "lub grupy w ciągu dnia tworzy kolejny wpis.")
    else:
        st.info("Brak zapisu obsady w tym zakresie — pojawi się od dnia wdrożenia nowej wersji Szturchacza.")

    # ── narzędzia ──
    def _mom(_r):
        return f'{_r.get("_dzien","")} {_r.get("godzina","00:00")}'

    def _dt(_r):
        try:
            return _TZ.localize(datetime.strptime(_mom(_r), "%Y-%m-%d %H:%M"))
        except Exception:
            return None

    # Jednolity słownik + tolerancja starych oznaczeń (dane sprzed zmiany).
    _STAT = {
        "ustalono_termin": "USTALONO TERMIN",
        "ustalono_termin_na_termin": "USTALONO TERMIN NA TERMIN",
        "nieodebral": "NIEODEBRAŁ",
        "nieproduktywne": "NIEPRODUKTYWNE",
        "konkret": "USTALONO TERMIN", "przelozenie": "USTALONO TERMIN NA TERMIN",
        "brak_kontaktu": "NIEODEBRAŁ", "kontakt_bez_konkretu": "NIEPRODUKTYWNE",
        "do_oceny": "⏳ DO OCENY",
        "bez_rozmowy": "— WPIS BEZ ROZMOWY",
        "archiwum": "📦 ZARCHIWIZOWANE (bez oceny)",
        "zwrot": "ZWROT TOWARU",
        "zly_numer": "☎️✖ ZŁY NUMER (nie rozmowa)",
        "brak_telefonu": "— NIE ZADZWONIŁA (nie rozmowa)",
    }
    # Kategorie, które NIE są rozmową — widoczne, ale poza liczbą telefonów i skutecznością.
    _NIE_ROZMOWA = ("zly_numer", "brak_telefonu", "bez_rozmowy", "archiwum")
    _SKUTECZNE = ("ustalono_termin", "ustalono_termin_na_termin", "konkret", "przelozenie", "zwrot")
    ...
    def _status(_c):
        ...
        _w = _c.get("wynik") or ""
        _s = _STAT.get(_w, _w)
        if _c.get("ocena_reczna"):
            _s += " ✍️"
        elif _c.get("ocena_zrodlo") == "auto":
            _s += " 🤖"
        return (_s + " (kurier)") if (_c.get("kurier_ustalony") and _s == "USTALONO TERMIN") else _s

    def _z_ust(_c):
        return (_c.get("wynik") or "") in _SKUTECZNE

    def _oceniony(_c):
        """Telefon z ocenioną treścią — tylko takie wchodzą do procentu skuteczności.
        Zarchiwizowane są poza rachunkiem: rozmowa się odbyła, ale werdyktu nie będzie."""
        # 🟥 Oceniona = ma JEDEN Z WŁAŚCIWYCH statusów. Wcześniej wystarczyło, że nie jest
        #    „do oceny" — przez to pusty albo nieznany wynik liczył się jako oceniony („6 z 6").
        return (_c.get("wynik") or "") in ("ustalono_termin", "ustalono_termin_na_termin",
                                           "nieodebral", "nieproduktywne", "zwrot",
                                           "konkret", "przelozenie", "brak_kontaktu",
                                           "kontakt_bez_konkretu")

    def _jakosc(_lst):
        """(procent, ile_ocenionych) — telefony 'do oceny' nie zaniżają wyniku."""
        _oc = [c for c in _lst if _oceniony(c)]
        if not _oc:
            return 0, 0
        return round(100 * sum(1 for c in _oc if _z_ust(c)) / len(_oc)), len(_oc)

    def _osoba(_c):
        """Konkretna osoba w grupie telefonistek — z nawiasu przy autorze wpisu."""
        if not _grupowy(_c):
            return _kto(_c)
        return (_c.get("osoba") or "").strip() or "nie udało się określić"

    def _kto_disp(_c):
        """Nazwa do PODSUMOWAŃ: operator → imię; telefonistka → grupa / osoba.
        Dzięki temu widać, kto konkretnie w grupie wykonuje telefony."""
        if not _grupowy(_c):
            return _kto(_c)
        _os = (_c.get("osoba") or "").strip()
        return f'{_kto(_c)} / {_os}' if _os else f'{_kto(_c)} / (nie udało się określić)'

    def _biz_min(_a, _b):
        """Minuty ROBOCZE (9-17, pon-pt) między dwoma momentami."""
        if not _a or not _b or _b <= _a:
            return 0
        _tot, _cur = 0, _a
        for _ in range(60):
            if _cur >= _b:
                break
            _s = _cur.replace(hour=9, minute=0, second=0, microsecond=0)
            _e = _cur.replace(hour=17, minute=0, second=0, microsecond=0)
            if _cur.weekday() < 5:
                _od, _do = max(_cur, _s), min(_b, _e)
                if _do > _od:
                    _tot += (_do - _od).total_seconds() / 60
            _cur = (_cur + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        return _tot

    def _kto(_c):
        return _c.get("operator") or "?"

    def _grupowy(_c):
        return _c.get("zrodlo") == "telefonista_zewn"

    def _kraj(_r):
        return str(_r.get("grupa") or "?")

    # Wpisy ocenione jako „nie rozmowa" zostają w bazie, ale nie są telefonem — poza statystyką.
    # 🟥 PEŁNY zbiór zostaje na potrzeby surowego logu — nic nie znika z pola widzenia.
    #    Odsiewamy wyłącznie z LICZB (skuteczność, udziały), nie z widoku.
    _calls_pelne = list(_calls)
    # 🟥 Zły numer i „nie zadzwoniła" to nie rozmowy — poza liczbą telefonów i skutecznością,
    #    ale widoczne w logu i w sekcji kontrolnej.
    _nie_rozmowy = [c for c in _calls if (c.get("wynik") or "") in ("zly_numer", "brak_telefonu")]
    _calls = [c for c in _calls if (c.get("wynik") or "") not in ("zly_numer", "brak_telefonu")]
    _calls_ext = [c for c in _calls_ext if (c.get("wynik") or "") not in ("zly_numer", "brak_telefonu")]
    _bez_rozmowy = [c for c in _calls if (c.get("wynik") or "") == "bez_rozmowy"]
    _calls = [c for c in _calls if (c.get("wynik") or "") != "bez_rozmowy"]
    _calls_ext = [c for c in _calls_ext if (c.get("wynik") or "") != "bez_rozmowy"]
    _zlecenia = [d for d in _deleg if (d.get("typ") or "zlecenie") == "zlecenie"]

    _cbn, _dbn = {}, {}
    for _c in _calls_ext:
        _cbn.setdefault(str(_c.get("numer_zamowienia") or "").strip(), []).append(_c)
    for _d in _deleg_ext:
        _dbn.setdefault(str(_d.get("numer_zamowienia") or "").strip(), []).append(_d)

    # 🟥 Telefon operatora, PO KTÓRYM powstał wpis (nieudany → zlecenie + woreczek).
    #    Taki telefon WYWOŁAŁ wpis, więc NIE MOŻE być liczony jako jego wykonanie —
    #    inaczej wychodziło „podjęte +0h 0min" i podwojona liczba telefonów.
    for _c in _calls_ext:
        _c["_wpis_po"] = None
        if not _grupowy(_c):
            _k = sorted([d for d in _dbn.get(str(_c.get("numer_zamowienia") or "").strip(), [])
                         if _mom(d) >= _mom(_c)], key=_mom)
            _c["_wpis_po"] = _k[0] if _k else None

    # 🟥 AKTUALNY STAN ZAMÓWIENIA. Zamówienie ma JEDNĄ historię telefoniczną, a zlecenie jest
    #    jej krokiem. Nie wiążemy więc zlecenia z konkretną rozmową po kolejności godzin —
    #    godziny pochodzą z dwóch zegarów (forum i nasz) i bywają w odwrotnej kolejności.
    #    Bierzemy NAJNOWSZĄ rozmowę dla tego numeru, pomijając tę, która sam wpis wywołała.
    # 🟥 PODJĘCIEM zlecenia może być wyłącznie rozmowa, która odbyła się PO nim.
    #    Tolerancja 5 minut pokrywa drgnięcie zegarów (forum vs nasz zapis) — poniżej tego
    #    nikt nie zdąży odebrać zlecenia i zadzwonić, więc to ten sam moment.
    #    Rozmowy WCZEŚNIEJSZE dotyczą wcześniejszej prośby: pokazujemy je jako kontekst sprawy,
    #    ale NIGDY jako wykonanie tego zlecenia (inaczej wychodzi „zlecone 16:15, podjęte 14:26").
    # 🟥 Zlecenie domyka WYŁĄCZNIE odpowiedź napisana POD NASZYM wpisem (albo pod jego
    #    podbiciem). Powiązanie idzie po numerze wpisu — godziny służą już tylko do liczenia
    #    czasu reakcji. Nic innego z wątku nie zamknie tego zlecenia.
    # Numery wpisów, pod którymi padła odpowiedź — podbicia z odpowiedzią dostają własne wiersze,
    # więc zlecenie nie może ich przejmować. Definicja MUSI być przed pętlą wiążącą.
    _odp_pod_id = {str(c.get("zlecenie_post_id") or "").strip()
                   for c in _calls_ext if str(c.get("zlecenie_post_id") or "").strip()}

    _pon_id = {}
    for _p in _deleg_ext:
        if (_p.get("typ") or "") == "ponaglenie":
            _pon_id.setdefault(str(_p.get("numer_zamowienia") or "").strip(), set()).add(
                str(_p.get("id_postu") or "").strip())
    for _z in _deleg_ext:
        _nr_z = str(_z.get("numer_zamowienia") or "").strip()
        # Podbicie z własnym wierszem wiąże WYŁĄCZNIE odpowiedź spod siebie.
        if (_z.get("typ") or "") == "ponaglenie":
            _moje = {str(_z.get("id_postu") or "").strip()}
        else:
            _wlasny = str(_z.get("id_postu") or "").strip()
            # 🟥 Odejmujemy TYLKO podbicia z odpowiedzią (mają własne wiersze).
            #    Własny numer zlecenia MUSI zostać — inaczej jego odpowiedź zostaje
            #    odrzucona jako „wisi pod innym wpisem" i wiersz wiecznie czeka.
            _moje = ({_wlasny} | _pon_id.get(_nr_z, set())) - (_odp_pod_id - {_wlasny})
        _moje.discard("")
        # 🟥 Telefon operatora, po którym dopiero POWSTAŁ ten wpis, nie może być jego
        #    wykonaniem — nawet gdy ma zapisany numer zlecenia. Rozpoznajemy po zerowym
        #    odstępie czasu: rozmowa i wpis w tej samej minucie to jedno zdarzenie.
        _MIN_ODST = timedelta(minutes=2)
        # 🟥 ODPOWIEDŹ NALEŻY DO WPISU, POD KTÓRYM WISI — forum mówi to jednoznacznie.
        #    Żadnego zgadywania po godzinach: wpis emilii z 30.07 nie może wykonywać
        #    delegacji z 03.08 tylko dlatego, że jest telefonistką i ma późniejszą godzinę.
        _MIN_ODSTEP = timedelta(minutes=2)
        _po = []
        for _c in _cbn.get(_nr_z, []):
            _pid_c = str(_c.get("zlecenie_post_id") or "").strip()
            if _pid_c:
                if _pid_c not in _moje:
                    continue                  # wisi pod innym wpisem — nie nasza sprawa
            elif _grupowy(_c):
                continue                      # 🟥 telefonistka BEZ powiązania: nie zgadujemy
            else:
                # telefon operatora z woreczka nie niesie numeru — wiążemy po czasie,
                # ale tylko gdy jest PÓŹNIEJSZY niż zlecenie i nie on je wywołał
                if _c.get("_wpis_po") is _z or not _dt(_c) or not _dt(_z):
                    continue
                if _dt(_c) < _dt(_z) + _MIN_ODSTEP:
                    continue
            # 🟥 TWARDY WARUNEK: rozmowa nie może być WCZEŚNIEJSZA niż zlecenie.
            if _dt(_c) and _dt(_z) and _dt(_c) < _dt(_z):
                continue
            _po.append(_c)
        _po.sort(key=_mom)
        # 🟥 AKTUALNY STAN SPRAWY = NAJNOWSZA odpowiedź. Dotąd brałem pierwszą, więc przy
        #    kolejnych podbiciach wiersz pokazywał starą rozmowę sprzed dni, a nie to,
        #    co wydarzyło się ostatnio (np. dzisiejsze ustalenie kuriera).
        _z["_call"] = _po[-1] if _po else None
        _z["_call_wcz"] = None
        _z["_ile_rozmow"] = len(_po)

    # JEDEN WIERSZ NA ZLECENIE. Ponaglenia to nie nowe zlecenia — liczą się jako podbicia.
    _dni_set = set(_dni)

    # Numery wpisów, do których należą rozmowy widoczne w zakresie — takie zlecenia
    # MUSZĄ być pokazane, choćby powstały dawno temu.
    _potrzebne_id = {str(c.get("zlecenie_post_id") or "").strip()
                     for c in _calls if str(c.get("zlecenie_post_id") or "").strip()}

    def _aktywne_w_zakresie(_z):
        """Zlecenie widoczne, gdy powstało w zakresie ALBO gdy w zakresie coś się przy nim działo
        (telefon albo podbicie) — inaczej sprawa z wczoraj domknięta dziś nie byłaby widoczna."""
        if _z.get("_dzien") in _dni_set:
            return True
        if str(_z.get("id_postu") or "").strip() in _potrzebne_id:
            return True                       # należy do niego rozmowa z zakresu
        _cc = _z.get("_call")
        if _cc and _cc.get("_dzien") in _dni_set:
            return True
        _nr = str(_z.get("numer_zamowienia") or "").strip()
        # podbicie w zakresie WCIĄGA korzeń — to ruch wykonany tego dnia (także przez automat)
        return any(_p.get("_dzien") in _dni_set for _p in _deleg_ext
                   if (_p.get("typ") or "") in ("ponaglenie", "raport")
                   and str(_p.get("numer_zamowienia") or "").strip() == _nr)
    ...
    # 🟥 FILTR PRZY WYŚWIETLANIU (nie przy zapisie): wpisy niebędące prośbą o telefon
    #    — zmiana kanału, anulowanie, informacja o zakończeniu telefonów — zostają
    #    w bazie, ale nie tworzą wierszy. Błąd w filtrze da złą tabelę, nie utratę danych.
    _NIE_TELEFON = re.compile(
        r"kana[łl] telefoniczny wyczerpany|przechodzimy na komunikacj[ęe] pisemn|"
        r"anulowanie delegacji|telefon nie jest ju[żz] potrzebny|"
        r"prosimy nie wykonywa[ćc] telefonu|pro[śs]ba o telefon[^\n]{0,20}nieaktualna",
        re.IGNORECASE)

    def _jest_telefoniczny(_d):
        _tr = str(_d.get("tresc") or _d.get("tresc_skrot") or "")
        return not (_tr and _NIE_TELEFON.search(_tr))

    # 🟥 Numery zamówień, dla których w rejestrze JEST korzeń (typ=zlecenie).
    #    Podbicie sprawy BEZ korzenia musi stworzyć własny wiersz — inaczej nocny wpis
    #    bota do sprawy, której zlecenia nigdy nie zapisano, byłby całkiem niewidoczny.
    _nr_z_korzeniem = {str(d.get("numer_zamowienia") or "").strip() for d in _deleg_ext
                       if (d.get("typ") or "zlecenie") == "zlecenie"}

    def _tworzy_wiersz(_d):
        _typ_d = (_d.get("typ") or "zlecenie")
        if _typ_d == "zlecenie":
            return True
        if str(_d.get("id_postu") or "").strip() in _id_po_tel:
            return True
        # podbicie osierocone — brak korzenia w rejestrze
        return str(_d.get("numer_zamowienia") or "").strip() not in _nr_z_korzeniem

    _zlec_only = [d for d in _deleg_ext
                  if _tworzy_wiersz(d)
                  and _jest_telefoniczny(d)
                  and _aktywne_w_zakresie(d)]
    # gdy sprawa ma kilka osieroconych podbić — zostawiamy NAJSTARSZE jako korzeń
    _widziane_os = set()
    _zlec_tmp = []
    for _d in sorted(_zlec_only, key=_mom):
        _nr_d = str(_d.get("numer_zamowienia") or "").strip()
        _sierota = ((_d.get("typ") or "zlecenie") != "zlecenie"
                    and _nr_d not in _nr_z_korzeniem
                    and str(_d.get("id_postu") or "").strip() not in _id_po_tel)
        if _sierota:
            if _nr_d in _widziane_os:
                continue
            _widziane_os.add(_nr_d)
        _zlec_tmp.append(_d)
    _zlec_only = _zlec_tmp
    # 🟥 KORZEŃ MAX RAZ: podbicie NIE tworzy drugiego wiersza — wciąga korzeń do swojego dnia.
    #    Korzeń 06.08 + podbicie 06.08  → filtr 6-6.08  = JEDEN wiersz
    #    Korzeń 06.08 + podbicie 07.08  → filtr 6-6.08  = JEDEN wiersz
    #                                   → filtr 6-7.08  = JEDEN wiersz
    #                                   → filtr 7-7.08  = JEDEN wiersz (korzeń wciągnięty)
    _pon_z_odp = []
    _pon_only = [d for d in _deleg_ext if (d.get("typ") or "") == "ponaglenie"
                 and str(d.get("id_postu") or "").strip() not in _odp_pod_id]
    _pon_bn = {}
    for _p in _pon_only:
        _pon_bn.setdefault(str(_p.get("numer_zamowienia") or "").strip(), []).append(_p)
    _nr_licz = {}
    for _z in sorted(_zlec_only, key=_mom):
        _nr = str(_z.get("numer_zamowienia") or "").strip()
        _nr_licz[_nr] = _nr_licz.get(_nr, 0) + 1
        _z["_proba_nr"] = _nr_licz[_nr]
        _pons = [p for p in _pon_bn.get(_nr, []) if _mom(p) >= _mom(_z)
                 and (not _z.get("_call") or _mom(p) <= _mom(_z["_call"]))]
        _z["_pon"] = len(_pons)
        _z["_pon_ostatni"] = _mom(sorted(_pons, key=_mom)[-1]) if _pons else ""

    _pr = {}
    for _c in sorted(_calls_ext, key=_mom):
        _n = str(_c.get("numer_zamowienia") or "").strip()
        _pr[_n] = _pr.get(_n, 0) + 1
        _c["_proba"] = _pr[_n]
    for _c in _calls_ext:
        _c["_prazem"] = _pr.get(str(_c.get("numer_zamowienia") or "").strip(), 1)

    _now = datetime.now(_TZ)
    _gr_koniec = _TZ.localize(datetime.combine(_t_to, datetime.min.time())).replace(hour=_KON)

    def _niewyk_do(_z, _gran):
        _cc = _z.get("_call")
        if not _cc:
            return True
        _d = _dt(_cc)
        return bool(_d and _d > _gran)

    # Anulowane zlecenia nie są „niewykonane" — telefon przestał być potrzebny.
    _niewyk = [z for z in _zlecenia if not z.get("anulowane") and _niewyk_do(z, min(_gr_koniec, _now))]

    # języki z obsady (najświeższy wpis danej osoby w grupie)
    _jez_map = {}
    for _o in sorted(_obsada, key=lambda x: (x.get("_dzien", ""), x.get("od_godziny", ""))):
    ...
    st.markdown("### ☎️ Zlecenia telefoniczne")
    st.caption("Widok na poziomie WPISU powstałego w danym dniu — zlecenia, podbicia i raporty "
               "z telefonów. Jeden korzeń = jeden wiersz, choćby podbić było kilka; podbicie "
               "wciąga sprawę do swojego dnia. Kolumna PODJĘTE pokazuje cały łańcuch ruchów, "
               "także z późniejszych dni. Ustalenia są 1:1 we wpisie — kliknij link.")

    _FORUM_BASE = "https://f15.pmgtechnik.com"

    def _link_md(_url, _pid):
        """Link ZAWSZE pełny — pusty adres Streamlit traktował jako względny i kierował
        do Wieżowca. Wyświetlany numer to KONKRETNY wpis (#odp-), nie identyfikator wątku."""
        _p = str(_pid or "").strip()
        if not _p:
            return ""
        if _url and str(_url).startswith("http"):
            return str(_url) if "#odp-" in str(_url) else f"{_url}#odp-{_p}"
    ...
    def _ciag_ruchow(_nr, _od=None):
        """🟥 PEŁNY CIĄG ruchów telefonicznych sprawy w JEDNEJ komórce — tak jak ustalono:
        📞 SESJA — Romana · 14:51 (NIEPRODUKTYWNE) → 💬 FORUM — sylwia · 14:56 (USTALONO TERMIN)
        Bierze telefony i wpisy telefoniczne (zlecenia, podbicia, raporty) od momentu _od."""
        _kroki = []
        for _c2 in _cbn.get(_nr, []):
            if _od and _dt(_c2) and _dt(_od) and _dt(_c2) < _dt(_od):
                continue
            _kroki.append({
                "dt": _dt(_c2), "t": _mom(_c2),
                "txt": ((f'💬 FORUM — {_kto(_c2)} / {_osoba(_c2)}' if _grupowy(_c2)
                         else f'📞 SESJA — {_kto(_c2)}')
                        + f' · {_mom(_c2)[11:]} ({_status(_c2)})'),
                "kur": bool(_c2.get("kurier_ustalony")),
            })
        for _d2 in _rap_bn.get(_nr, []):
            if _od and _dt(_d2) and _dt(_od) and _dt(_d2) < _dt(_od):
                continue
            _kroki.append({"dt": _dt(_d2), "t": _mom(_d2),
                           "txt": f'📋 raport → {_d2.get("do_kogo") or "operatorzy"}', "kur": False})
        for _p2 in _deleg_ext:
            if str(_p2.get("numer_zamowienia") or "").strip() != _nr:
                continue
            if (_p2.get("typ") or "") != "ponaglenie":
                continue
            if _od and _dt(_p2) and _dt(_od) and _dt(_p2) < _dt(_od):
                continue
            _kroki.append({"dt": _dt(_p2), "t": _mom(_p2),
                           # 🟥 Chatoszturek jest normalnym operatorem — jego podbicia widać
                           #    tak samo jak ludzkie, z godziną i podpisem.
                           "txt": (f'🤖 podbicie — chatoszturek · {_mom(_p2)[11:]}'
                                   if _p2.get("bot")
                                   else f'🔁 podbicie — {_p2.get("zlecil") or "?"} · {_mom(_p2)[11:]}'),
                           "kur": False})
        _kroki.sort(key=lambda r: r["t"])
        if not _kroki:
            return ""
        _txt = " → ".join(k["txt"] for k in _kroki)
        if any(k["kur"] for k in _kroki):
            _txt += " → 💎 kurier zamówiony"
        return _txt
    ...
    # pomocnicze — język rozmowy i wiarygodność telefonu operatora
    ...
    def _telefon_wiarygodny(_c):
        _lg = {str(j).upper() for j in _jez_map.get((_kto(_c), _kraj(_c)), [])}
        _js = _jez_calla(_c)
        return True if (not _lg or not _js) else (_js in _lg)

    _calls_op = [c for c in _calls if not _grupowy(c)]
    _podejrzane = [c for c in _calls_op if not _telefon_wiarygodny(c)]
    _r_all = []

    # ── telefony operatorów; te zakończone zleceniem dołączamy do wiersza zlecenia ──
    _tel_przed_zlec = {}
    _zlec_po_id = {str(_d.get("id_postu") or "").strip(): _d
                   for _d in _deleg_ext if str(_d.get("id_postu") or "").strip()}
    # 🟥 Raporty operatorów NIE są zleceniami — nie tworzą wierszy, ale są krokami ciągu.
    _rap_bn = {}
    for _d in _deleg_ext:
        if (_d.get("typ") or "") == "raport":
            _rap_bn.setdefault(str(_d.get("numer_zamowienia") or "").strip(), []).append(_d)
    for _c in _calls_op:
        _po_wpis = None
        ...
        _r_all.append({
            ...
            "WPIS": _link_md(_c.get("link"), _c.get("post_id")),
            "ZLECONO": _mom(_c),
            "OD KOGO": f'{_kto_disp(_c)} ({_kraj(_c)})',
            "DO KOGO": (("Telefoniści — kolejna próba"
                         if (_c.get("wynik") or "") in ("nieodebral", "brak_kontaktu", "zly_numer")
                         else "raport do operatorów")
                        if _c.get("post_id") else "— telefon bez wpisu"),
            # 🟥 Kurier TYLKO gdy POTWIERDZONY (flaga z rozmowy). Sam „ustalono termin"
            #    znaczy jedynie, że klient podał datę — kurier to osobne zdarzenie.
            #    Zgadywanie ze statusu zawyżało statystyki, a lepiej pokazać gorzej niż upiększać.
            "PODJĘTE": (_ciag_ruchow(str(_c.get("numer_zamowienia") or "").strip(), _c)
                        or (f'📞 SESJA — {_kto_disp(_c)} · {_mom(_c)[11:]}')),
            ...
            "UWAGI": ((f'wykryte {_c.get("wykryte_dnia")} · '
                      ...
                      + (("telefon operatora → zlecenie kolejnej próby"
                          if (_c.get("wynik") or "") in ("nieodebral", "brak_kontaktu", "zly_numer")
                          else "telefon operatora → raport z ustaleń")
                         if _c.get("post_id") else "telefon operatora")
                      + ("" if _telefon_wiarygodny(_c)
                         else f' · ⚠️ język {_jez_calla(_c) or "?"} poza przypisanymi')),
        })

    # 🟥 RAPORT bez zlecenia = własny wiersz. Telefon operatora zakończony ustaleniami
    #    nie tworzy zlecenia (nikt nie ma dzwonić), więc bez tego sprawa znikała z tabeli.
    _nr_ze_zlec = {str(_z.get("numer_zamowienia") or "").strip() for _z in _zlec_only}
    _rap_solo = []
    for _nr_r, _lst_r in _rap_bn.items():
        if _nr_r in _nr_ze_zlec:
            continue
        _lst_r = sorted(_lst_r, key=_mom)
        _rap_solo.append(_lst_r[-1])
    for _rr in _rap_solo:
        _nr_rr = str(_rr.get("numer_zamowienia") or "").strip()
        _cc_rr = None
        for _c3 in sorted(_cbn.get(_nr_rr, []), key=_mom):
            if not _grupowy(_c3):
                _cc_rr = _c3
        _r_all.append({
            "NR ZAMÓWIENIA": _nr_rr,
            "WPIS": _link_md(_rr.get("link"), _rr.get("id_postu")),
            "ZLECONO": _mom(_rr),
            "OD KOGO": f'{_rr.get("zlecil", "")} ({_kraj(_rr)})',
            "DO KOGO": _rr.get("do_kogo") or "raport do operatorów",
            "PODJĘTE": (_ciag_ruchow(_nr_rr) or f'📋 raport · {_mom(_rr)[11:]}'),
            "CZY W TERMINIE": "— (telefon w sesji, bez oczekiwania)",
            "STATUS": _status(_cc_rr),
            "UWAGI": "telefon operatora → raport z ustaleń",
        })
    ...
    for _z in sorted(_zlec_only, key=_mom, reverse=True):
        ...
        if not _cc and not _z.get("anulowane"):
            ...
            if _pid_odp and _pid_odp != str(_z.get("id_postu") or "").strip():
                ...
                for _p2 in _deleg_ext:
                    if str(_p2.get("id_postu") or "").strip() == _pid_odp:
                        _t0_real = _dt(_p2) or _t0
                        _link_ost = _p2.get("link")
                        break
            if _t0_real and _t1:
                _bm_ost = _biz_min(_t0_real, _t1)
                if _bm_ost > 2 * 480:
                    _uw += " · po terminie"
            _tel_pocz = _tel_przed_zlec.get(str(_z.get("id_postu") or "").strip())
            _od_kroku = _tel_pocz or _z
            _podj = _ciag_ruchow(_nr_z, _od_kroku) or (
                f'💬 FORUM — {_kto(_cc)} / {_osoba(_cc)} · {_mom(_cc)[11:]}' if _grupowy(_cc)
    ...
    if _r_all:
        st.download_button("⬇️ CSV — telefony i zlecenia", _df_all.to_csv(index=False).encode("utf-8"),
                           file_name=f"telefony_zlecenia_{_t_from}_{_t_to}.csv",
                           mime="text/csv", key="_csv_all")
        _byz = {}
    ...
    # ══ 3b. TELEFONY DO ZLECEŃ Z DANEGO DNIA ═══════════════════════════════════
    st.markdown("#### 📋 Telefony do zleceń z tego dnia")
    st.caption("Ile pracy przyniosły zlecenia złożone w wybranym okresie — kotwicą jest ZLECENIE, "
               "nie data rozmowy. Telefon wykonany dziś do wczorajszego zlecenia liczy się we "
               "WCZORAJSZYM zestawieniu. Liczby mogą jeszcze rosnąć, dopóki zlecenia są podejmowane. "
               "To inna miara niż podsumowanie osobowe niżej — tam liczą się telefony z danego dnia.")

    _KAT2 = {"ustalono_termin": "TERMIN", "konkret": "TERMIN",
             "ustalono_termin_na_termin": "TERMIN NA TERMIN", "przelozenie": "TERMIN NA TERMIN",
             "nieodebral": "NIEODEBRANE", "brak_kontaktu": "NIEODEBRANE"}

    # 🟥 Każdy telefon kończy się WPISEM i ma zapisany jego numer — wiążemy PO NUMERZE,
    #    nie po czasie. Dzięki temu liczy się też telefon operatora dzwoniącego, który
    #    ten wpis WYWOŁAŁ (był przed nim, więc dopasowanie czasowe go gubiło).
    _id_zlec = {str(_z.get("id_postu") or "").strip() for _z in _zlec_only
                if str(_z.get("id_postu") or "").strip()}
    _nr_zlec = {str(_z.get("numer_zamowienia") or "").strip() for _z in _zlec_only}

    _do_zlec = {}
    for _c4 in _calls_ext:
        _nr4 = str(_c4.get("numer_zamowienia") or "").strip()
        _pid_odp4 = str(_c4.get("zlecenie_post_id") or "").strip()   # odpowiedź POD wpisem
        _pid_tel4 = str(_c4.get("zlec_po_tel") or "").strip()        # wpis POWSTAŁY po telefonie
        _pid_rap4 = str(_c4.get("post_id") or "").strip()            # raport z telefonu
        _nalezy = (_pid_odp4 in _id_zlec or _pid_tel4 in _id_zlec or _pid_rap4 in _id_zlec)
        if not _nalezy and not (_pid_odp4 or _pid_tel4 or _pid_rap4):
            # starsze rozmowy bez numerów — dopasowanie zapasowe po numerze zamówienia
            _nalezy = _nr4 in _nr_zlec
        if not _nalezy:
            continue
        _kto4 = _kto_disp(_c4)
        _a4 = _do_zlec.setdefault(_kto4, {"KTO": _kto4, "ŁĄCZNIE": 0, "TERMIN": 0,
                                          "TERMIN NA TERMIN": 0, "NIEODEBRANE": 0, "INNE": 0})
        _a4["ŁĄCZNIE"] += 1
        _a4[_KAT2.get(_c4.get("wynik") or "", "INNE")] += 1

    if _do_zlec:
        st.dataframe(pd.DataFrame(sorted(_do_zlec.values(), key=lambda r: -r["ŁĄCZNIE"])),
                     use_container_width=True, hide_index=True)
        st.caption("Liczone są WSZYSTKIE telefony powiązane ze zleceniami z okresu — także te "
                   "operatorów dzwoniących, które zlecenie wywołały. INNE = nieproduktywne, "
                   "zwroty, zły numer i rozmowy bez oceny.")
    else:
        st.caption("Brak telefonów do zleceń z tego okresu.")

    # Lista krajów — używana niżej (porównanie sił, kurierzy). Definicja przepadła
    # przy scalaniu rozliczenia z jakością.
    _kraje = sorted({_kraj(c) for c in _calls} | {_kraj(z) for z in _zlecenia}) or ["?"]
    ...
    st.caption("Dwie siły obok siebie, osobno dla każdego kraju — kto wykonuje więcej rozmów i z jakim skutkiem.")
    _rows_vs = []
    for _k in _kraje:
        _cl_k = [c for c in _calls if _kraj(c) == _k]
        if not _cl_k:
            continue
        _tel = [c for c in _cl_k if _grupowy(c)]
        _ope = [c for c in _cl_k if not _grupowy(c)]
        for _lbl, _lst_g in (("Telefonistki", _tel), ("Operatorzy dzwoniący", _ope)):
            _jg, _ocg = _jakosc(_lst_g)
            _rows_vs.append({
                "Kraj": (_k if _lbl == "Telefonistki" else ""),
                "Grupa": _lbl,
                "Rozmowy": len(_lst_g),
                "Z ustaleniami": sum(1 for c in _lst_g if _z_ust(c)),
                "Udział w kraju": f"{round(100*len(_lst_g)/len(_cl_k))}%",
                "Jakość": (("⏳ czeka" if not _ocg
                            else (f"{_jg}%" if _ocg == len(_lst_g)
                                  else f"{_jg}% (ocenione {_ocg} z {len(_lst_g)})"))),
            })
    if _rows_vs:
        st.dataframe(pd.DataFrame(_rows_vs), use_container_width=True, hide_index=True)
    else:
        st.caption("Brak telefonów w wybranym zakresie.")

    # ══ 6b1. TELEFONY ZAKOŃCZONE KURIEREM ═══════════════════════════════════════
    st.markdown("### 💎 Telefony zakończone kurierem")
    st.caption("Liczy telefony, które dały MOŻLIWOŚĆ zamówienia kuriera — rozmowy ze statusem "
               "USTALONO TERMIN. Oceniamy jakość ROZMOWY; czy operator faktycznie zamówił kuriera, "
               "to osobna kwestia. Jest data odbioru, więc kurier ma po co "
               "przyjechać. Zwroty liczone osobno: klient oddaje nasz towar, to inny przewóz. "
               "Jeden kurier na zamówienie, choćby rozmów było kilka.")

    # 🟥 Liczymy PO STATUSIE — status już mówi, jaki to rodzaj zakończenia.
    _kur_by_nr, _zwr_by_nr = {}, {}
    for _c in _calls:
        _nr_k = str(_c.get("numer_zamowienia") or "").strip()
        if not _nr_k:
            continue
        _w_k = _c.get("wynik") or ""
        # 🟥 Potwierdzenie terminu przez osobę dzwoniącą DAJE diament — kurier ma po co
        #    przyjechać. Rozstrzyga sama rozmowa, nie osobne potwierdzenie zamówienia.
        _cel = (_kur_by_nr if _w_k in ("ustalono_termin", "konkret")
                else (_zwr_by_nr if _w_k == "zwrot" else None))
        if _cel is None:
            continue
        _prev = _cel.get(_nr_k)
        if _prev is None or _mom(_c) > _mom(_prev):
            _cel[_nr_k] = _c

    _diamenty = list(_kur_by_nr.values())
    _zwroty = list(_zwr_by_nr.values())
    _d_tel = [c for c in _diamenty if _grupowy(c)]
    _d_op = [c for c in _diamenty if not _grupowy(c)]

    _d1, _d2, _d3, _d4 = st.columns(4)
    _d1.metric("💎 Kurier po telefonie", len(_diamenty))
    _d2.metric("Telefonistki", len(_d_tel))
    _d3.metric("Operatorzy", len(_d_op))
    _d4.metric("% rozmów z kurierem", f"{round(100*len(_diamenty)/len(_calls))}%" if _calls else "—")

    if _diamenty:
        _rows_d = {}
        for _c in _diamenty:
            _k2 = (_kto_disp(_c), _kraj(_c))
            _a = _rows_d.setdefault(_k2, {"Kto": _kto_disp(_c), "Kraj": _kraj(_c),
                                          "Typ": ("telefonistka" if _grupowy(_c) else "operator"),
                                          "Kurierów": 0})
            _a["Kurierów"] += 1
        st.dataframe(pd.DataFrame(sorted(_rows_d.values(), key=lambda r: -r["Kurierów"])),
                     use_container_width=True, hide_index=True)
        _kr_d = {}
        for _c in _diamenty:
            _kr_d[_kraj(_c)] = _kr_d.get(_kraj(_c), 0) + 1
        st.caption("Per kraj: " + " · ".join(f"{k}: {v}" for k, v in sorted(_kr_d.items())))
    else:
        st.caption("Brak rozmów zakończonych ustalonym terminem odbioru w tym zakresie.")

    if _zwroty:
        st.caption(f"↩️ Osobno — **zwroty towaru: {len(_zwroty)}** "
                   f"(klient rezygnuje z wymiany i oddaje naszą część): "
                   + ", ".join(sorted({str(c.get("numer_zamowienia") or "?") for c in _zwroty})[:12]))

    # ══ 6b1b. HISTORIA SPRAW ════════════════════════════════════════════════════
    with st.expander("🕓 Historia spraw — pełny przebieg krok po kroku", expanded=False):
        st.caption("Wszystkie sprawy z zaznaczonych dni, ułożone od najdłużej trwających — "
                   "zatory są na górze. Czas przy każdym kroku liczony od kroku poprzedniego, "
        ...
        # 🟥 SPÓJNOŚĆ Z TABELĄ: historia buduje się z tych samych zdarzeń co tabela wyżej.
        #    Bierzemy zlecenia widoczne w tabeli (_zlec_only) plus ich podbicia, oraz telefony
        #    z zakresu — inaczej historia pokazywałaby sprawy, których w tabeli nie ma.
        _nr_w_tabeli = {str(_z.get("numer_zamowienia") or "").strip() for _z in _zlec_only} | \
                       {str(_c.get("numer_zamowienia") or "").strip() for _c in _calls}
        _hist = {}
        for _z in _deleg_ext:
            if str(_z.get("numer_zamowienia") or "").strip() not in _nr_w_tabeli:
                continue
            _nrh = str(_z.get("numer_zamowienia") or "").strip()
            if not _nrh:
                continue
            _bot = bool(_z.get("bot"))
            _pon = (_z.get("typ") or "") == "ponaglenie"
            _pid_h = str(_z.get("id_postu") or "").strip()
            _hist.setdefault(_nrh, []).append({
                "t": _mom(_z), "dt": _dt(_z),
                "ico": ("🤖" if _bot else ("🔁" if _pon else "📨")),
                "txt": (("PONAGLENIE (automat)" if (_bot and _pon)
                         else ("DELEGACJA (automat)" if _bot
                               else ("PODBICIE wpisu" if _pon else "ZLECENIE telefonu")))
                        + f' · zlecił: {_z.get("zlecil") or "?"}'
                        + (f' · do: {_z.get("do_kogo")}' if _z.get("do_kogo") else "")),
                "pid": _pid_h, "link": _z.get("link"),
                "typ": "deleg", "anul": bool(_z.get("anulowane")),
            })
        for _c in _calls_ext:
            _nrh = str(_c.get("numer_zamowienia") or "").strip()
            if not _nrh or _nrh not in _nr_w_tabeli:
                continue
            _os_h = (_c.get("osoba") or "").strip()
            _kto_h = (f'{_kto(_c)} / {_os_h}' if (_grupowy(_c) and _os_h)
                      else (f'{_kto(_c)} (osoba nierozpoznana)' if _grupowy(_c) else _kto(_c)))
            _hist.setdefault(_nrh, []).append({
                "t": _mom(_c), "dt": _dt(_c),
                "ico": ("💬" if _grupowy(_c) else "📞"),
                "txt": (("ODPOWIEDŹ na forum — " if _grupowy(_c) else "TELEFON w sesji — ")
                        + _kto_h + f' · {_status(_c)}'),
                "pid": str(_c.get("post_id") or "").strip(), "link": _c.get("link"),
                "typ": "tel",
            })

        # ── liczymy czas trwania i sortujemy ──
        _karty = []
        for _nrh, _kr in _hist.items():
            ...
            _ile_zl = sum(1 for r in _kr if r["typ"] == "deleg")
            _czeka = (_ile_tel == 0 and not any(r.get("anul") for r in _kr))
            if _czeka and _dts:
                _trwa = _biz_min(_dts[-1], _now)
            _karty.append({"nr": _nrh, "kroki": _kr, "trwa": _trwa,
                           "tel": _ile_tel, "zl": _ile_zl, "czeka": _czeka})
        _karty.sort(key=lambda k: -k["trwa"])

        if not _karty:
            st.caption("Brak spraw w wybranym zakresie.")
        else:
            _LIMIT = st.session_state.get("_hist_limit", 25)
            for _k in _karty[:_LIMIT]:
                _hh, _mm = int(_k["trwa"] // 60), int(_k["trwa"] % 60)
                _dni_r = int(_k["trwa"] // 480)
                _tr_txt = (f'{_dni_r} dni rob.' if _dni_r else
                           (f'{_hh}h {_mm}min' if _hh else f'{_mm} min'))
                _kolor = ("var(--text-danger)" if _k["trwa"] > 480 else
                          ("var(--text-warning)" if _k["trwa"] > 360 else "var(--text-success)"))
                _nag = (f'{_k["zl"]} zlec. · {_k["tel"]} tel. · '
                        + (f'czeka {_tr_txt}' if _k["czeka"] else _tr_txt))

                _lin = []
                _prev = None
                for _r in _k["kroki"]:
                    _delta = ""
                    if _prev and _r["dt"] and _prev["dt"]:
                        _bm = _biz_min(_prev["dt"], _r["dt"])
                        _dr = int(_bm // 480)
                        _delta = (f' <span style="color:#E5484D">+{_dr} dni rob.</span>' if _dr
                                  else (f' <span style="color:#F5A524">+{int(_bm//60)}h {int(_bm%60)}min</span>'
                                        if _bm > 360 else
                                        (f' <span style="color:#1D9E75">+{int(_bm//60)}h {int(_bm%60)}min</span>'
                                         if _bm >= 60 else f' <span style="color:#1D9E75">+{int(_bm)}min</span>')))
                    _pid_r = str(_r.get("pid") or "").strip()
                    _wp = ""
                    if _pid_r:
                        _u = _r.get("link") or f"{_FORUM_BASE}/Wpisy/detailWpis?id={_pid_r}&do_odpid={_pid_r}#odp-{_pid_r}"
                        _wp = (f' <a href="{_u}" target="_blank" '
                               f'style="color:#7F77DD;text-decoration:none">[{_pid_r}]</a>')
                    _lin.append(f'<span style="color:#888">{_r["t"]}</span> {_r["ico"]} '
                                f'{_r["txt"]}{_wp}{_delta}')
                    _prev = _r
                if _k["czeka"]:
                    _lin.append('<span style="color:#888">⏳ nikt jeszcze nie zadzwonił</span>')

                st.markdown(
                    '<div style="background:rgba(255,255,255,.03);border:0.5px solid rgba(255,255,255,.1);'
                    'border-radius:10px;padding:10px 14px;margin-bottom:9px">'
                    f'<div style="display:flex;justify-content:space-between;gap:8px;margin-bottom:7px">'
                    f'<span style="font-weight:600">{_k["nr"]}</span>'
                    f'<span style="font-size:11.5px;color:{_kolor}">{_nag}</span></div>'
                    f'<div style="font-size:12px;line-height:1.95">' + "<br>".join(_lin) + '</div></div>',
                    unsafe_allow_html=True)

            if len(_karty) > _LIMIT:
                if st.button(f"Pokaż więcej ({len(_karty) - _LIMIT} spraw)", key="_hist_more"):
                    st.session_state["_hist_limit"] = _LIMIT + 25
                    st.rerun()

    # ══ 6b2. DO KONTROLI I DO ZROBIENIA ═════════════════════════════════════════
    _niepro = [c for c in _calls if (c.get("wynik") or "") == "nieproduktywne"] + _nie_rozmowy
    _z_ozn = [c for c in _calls if c.get("oznaczenia")]
    if _niepro or _z_ozn:
        _kk1, _kk2 = st.columns(2)
        with _kk1:
            st.markdown("#### 🔍 Nieproduktywne — do kontroli")
            st.caption("Rozmowy, które nic nie wniosły, oraz zdarzenia niebędące rozmową "
                       "(zły numer, „nie zadzwoniła\") — te ostatnie nie liczą się do telefonów, "
                       "ale wymagają decyzji. Weryfikacja na forum po numerze wpisu.")
            if _niepro:
                st.dataframe(pd.DataFrame([{
                    "NR ZAMÓWIENIA": c.get("numer_zamowienia", ""),
                    "KTO": _kto_disp(c),
                    "ID WPISU": str(c.get("post_id") or "—"),
                    "GODZINA": c.get("forum_godzina") or c.get("godzina", ""),
                    "KRAJ": _kraj(c),
                    "STATUS": _status(c),
                    "TREŚĆ": (str(c.get("tresc") or "")[:90] or "—"),
                } for c in sorted(_niepro, key=_mom, reverse=True)]),
                    use_container_width=True, hide_index=True)
            else:
                st.caption("Brak — nic do kontroli.")
        with _kk2:
            st.markdown("#### 📌 Do zrobienia po telefonie")
            st.caption("Rozmowa się udała, ale klient na coś czeka — instrukcja, mail, "
                       "kontakt ze sprzedawcą albo anulowanie odbioru.")
            _ETYK = {"instrukcja_pakowania": "instrukcja pakowania", "wymaga_maila": "mail do klienta",
                     "kontakt_ze_sprzedawca": "kontakt ze sprzedawcą", "anulowanie_odbioru": "anulowanie odbioru",
                     "zmiana_kanalu": "inny kanał"}
            if _z_ozn:
                st.dataframe(pd.DataFrame([{
                    "NR ZAMÓWIENIA": c.get("numer_zamowienia", ""),
                    "DO ZROBIENIA": ", ".join(_ETYK.get(o, o) for o in (c.get("oznaczenia") or [])),
                    "STATUS": _status(c),
                    "ID WPISU": str(c.get("post_id") or "—"),
                    "KRAJ": _kraj(c),
                } for c in sorted(_z_ozn, key=_mom, reverse=True)]),
                    use_container_width=True, hide_index=True)
            else:
                st.caption("Brak zadań z telefonów.")

    # ══ 6c. ROZMOWY BEZ OCENY ═══════════════════════════════════════════════════
    # Liczone WPROST z rozmów w bazie (nie ze znaczników) — ostrzeżenie zawsze zgadza się z listą.
    # 🟥 Ocena dotyczy TEGO, CO OPERATORZY PRZEROBILI w wybranych dniach.
    #    Rozmowa dostaje status „do oceny" w momencie przerobienia sprawy (pole „wykryte_dnia"),
    #    a nie w dniu, z którego pochodzi wpis na forum. Dlatego filtrujemy po dniu PRZEROBIENIA —
    #    stare wpisy sprzed tygodni nie zaśmiecają kolejki.
    # 🟥 Zaznaczony zakres jest jedyną prawdą — kolejka nie sięga poza wybrane dni.
    #    Rozmowa jest zapisana pod DNIEM PRZEROBIENIA, więc mieści się w nim z definicji.
    _oc_dni = list(_dni)
    _do_oceny_all, _oc_stare, _oc_stare_lista = [], 0, []
    for _ds in _oc_dni:
        try:
            for _d in db.collection(col("ew_phone_log")).document(_ds).collection("calls").stream():
                _v = _d.to_dict() or {}
                if (_v.get("wynik") or "") != "do_oceny":
                    continue
                # 🟥 KRYTERIUM: kolejka awaryjna zawiera rozmowy PRZEROBIONE w wybranych dniach,
                #    których automat nie zdołał ocenić. Nie decyduje powiązanie techniczne ani wiek
                #    wpisu — decyduje to, czy operator ruszał tę sprawę w tym okresie.
                _v["_dzien"] = _ds
                _v["_id"] = _d.id
                # 🟥 Ten sam klucz co w tabelach: rozmowa należy do DNIA, W KTÓRYM SIĘ ODBYŁA.
                #    Wcześniej kolejka filtrowała po dniu PRZEROBIENIA — przez to tabela
                #    pokazywała „DO OCENY", a lista do ocenienia była pusta.
                if _ds in _dni_set:
                    _do_oceny_all.append(_v)
                else:
                    _oc_stare += 1
                    _oc_stare_lista.append(_v)
        except Exception:
            pass
    if _do_oceny_all:
        _nr_oc = sorted({str(x.get("numer_zamowienia") or "?") for x in _do_oceny_all})
        st.warning(f"⏳ **{len(_do_oceny_all)} rozmów czeka na ocenę** w {len(_nr_oc)} sprawach "
                   f"przerobionych w wybranych dniach. Zwykle domyka się sama przy otwarciu sprawy. "
                   f"Numery: " + ", ".join(_nr_oc[:20]))
    if _oc_stare:
        with st.expander(f"🧹 Rozmowy bez oceny spoza wybranych dni ({_oc_stare}) — do przejrzenia",
                         expanded=False):
            st.caption("Rozmowy spoza bieżącej pracy — starsze niż dzień przerobienia lub przerobione "
                       "w innych dniach. Najczęściej historia wątku wciągnięta starym mechanizmem. "
                       "NIE trzeba ich oceniać ani usuwać — nie liczą się do skuteczności. "
                       "Usunięcie jest nieodwracalne.")
            _df_st = pd.DataFrame([{
                "NR ZAMÓWIENIA": x.get("numer_zamowienia", ""),
                "GRUPA TELEFONISTEK": x.get("operator", ""),
                "OSOBA": (x.get("osoba") or "").strip() or "nie udało się określić",
                "DATA ROZMOWY": f'{x.get("data_str") or x.get("_dzien","")} {x.get("godzina","")}'.strip(),
                "PRZEROBIONE DNIA": x.get("wykryte_dnia", "") or "—",
                "KRAJ": x.get("grupa", ""),
                "POWIĄZANE ZE ZLECENIEM": ("TAK" if str(x.get("zlecenie_post_id") or "").strip() else "NIE"),
            } for x in sorted(_oc_stare_lista,
                              key=lambda r: f'{r.get("data_str") or r.get("_dzien","")} {r.get("godzina","")}',
                              reverse=True)])
            st.dataframe(_df_st, use_container_width=True, hide_index=True)
            st.download_button("⬇️ CSV — stare zapisy", _df_st.to_csv(index=False).encode("utf-8"),
                               file_name="stare_zapisy_bez_oceny.csv", mime="text/csv", key="_csv_stare")
            st.caption("Przejrzyj listę przed usunięciem. Zapisy z „POWIĄZANE ZE ZLECENIEM = NIE\" "
                       "to odpowiedzi na prośby, których nie ma w rejestrze — zwykle historia wątku "
                       "wciągnięta przy pierwszym otwarciu sprawy.")
            if st.button(f"🗑️ Usuń te {_oc_stare} zapisów", key="_del_stare"):
                _usun = 0
                for _v in _oc_stare_lista:
                    try:
                        db.collection(col("ew_phone_log")).document(_v["_dzien"]) \
                          .collection("calls").document(_v["_id"]).delete()
                        _usun += 1
                    except Exception:
                        pass
                st.success(f"✅ Usunięto {_usun} starych zapisów bez powiązania.")
                st.rerun()

    # ══ 6e. RĘCZNA OCENA ROZMÓW ═════════════════════════════════════════════════
    # niezależna od filtra dat — zaległości z natury są starsze niż wybrany dzień
    _do_oceny = _do_oceny_all
    if _do_oceny:
        with st.expander(f"✍️ Oceń rozmowy ręcznie ({len(_do_oceny)})", expanded=True):
            st.caption("Wyjście awaryjne: rozmowy przerobione przez operatorów w wybranych dniach, "
                       "których automat nie potrafił ocenić. Zlecenia bez przerobienia tu nie trafiają. "
                       "Po ocenie rozmowa wchodzi do skuteczności ze znacznikiem ✍️.")
            _OPCJE = {
                "— wybierz status —": "__brak__",
                "USTALONO TERMIN": "ustalono_termin",
                "USTALONO TERMIN NA TERMIN": "ustalono_termin_na_termin",
                "NIEODEBRAŁ": "nieodebral",
                "NIEPRODUKTYWNE": "nieproduktywne",
                "ZWROT TOWARU (klient rezygnuje z wymiany)": "zwrot",
                "☎️✖ ZŁY NUMER (nie liczy się jako rozmowa)": "zly_numer",
                "— NIE ZADZWONIŁA (odpisała, ale telefonu nie było)": "brak_telefonu",
                "❌ to nie była rozmowa (zostaje w logu, poza statystyką)": "__usun__",
                "📦 zarchiwizuj — nie da się ocenić": "__arch__",
            }
            _ar1, _ar2 = st.columns([3, 1.4])
            with _ar1:
                st.caption("Jeśli tych rozmów nie da się już ocenić (stare wpisy, brak treści), "
                           "możesz je zarchiwizować — znikną z tej listy, ale ZOSTANĄ w logu "
                           "i w tabelach z oznaczeniem „zarchiwizowane\". Nic nie jest kasowane.")
            with _ar2:
                if st.button(f"📦 Zarchiwizuj wszystkie ({len(_do_oceny)})", key="_arch_all"):
                    _n_ar = 0
                    for _cx in _do_oceny:
                        try:
                            db.collection(col("ew_phone_log")).document(_cx["_dzien"]) \
                              .collection("calls").document(_cx["_id"]).update({
                                  "wynik": "archiwum", "efektywny": False,
                                  "zarchiwizowane": True})
                            _n_ar += 1
                        except Exception:
                            pass
                    st.success(f"📦 Zarchiwizowano {_n_ar} rozmów — zostały w logu, poza skutecznością.")
                    st.rerun()

            # jedna pozycja na ZAMÓWIENIE — kolejne wpisy tej samej sprawy pod spodem
            _grp_oc = {}
            for _c in sorted(_do_oceny, key=_mom):
                _grp_oc.setdefault(str(_c.get("numer_zamowienia") or "?"), []).append(_c)
            for _nr_oc2, _lst_oc in _grp_oc.items():
                _c = _lst_oc[0]
                _os = (_c.get("osoba") or "").strip() or "nie udało się określić"
                _e1, _e2, _e3 = st.columns([3, 3, 1.2])
                with _e1:
                    st.markdown(f'**{_nr_oc2}** · {_c.get("operator","?")} / {_os} · {_kraj(_c)}'
                                + (f' · wpisów: {len(_lst_oc)}' if len(_lst_oc) > 1 else ''))
                    _zl_id = str(_c.get("zlecenie_post_id") or "").strip()
                    if _zl_id:
                        st.caption(f"zlecenie: post {_zl_id}")
                    for _cx in _lst_oc:
                        _tx = str(_cx.get("tresc") or "").strip()
                        _wd_x = _cx.get("wykryte_dnia")
                        _info_wd = (f' · wykryte {_wd_x}'
                                    if (_wd_x and _wd_x != _cx.get("_dzien")) else "")
                        st.caption(f'🕒 {_cx.get("_dzien","")} '
                                   f'{_cx.get("forum_godzina") or _cx.get("godzina","")}{_info_wd}'
                                   + (f' — „{_tx[:180]}”' if _tx else " — (treść niezapisana)"))
                with _e2:
                    _wyb = st.selectbox("Status:", list(_OPCJE.keys()), key=f'_oc_{_nr_oc2}',
                                        label_visibility="collapsed")
                with _e3:
                    if st.button("💾", key=f'_ocb_{_nr_oc2}',
                                 help="Zapisz status dla wszystkich wpisów tej sprawy"):
                        try:
                            _val = _OPCJE[_wyb]
                            if _val == "__brak__":
                                st.warning("Najpierw wybierz status.")
                                st.stop()
                            for _cx in _lst_oc:
                                _ref = db.collection(col("ew_phone_log")).document(_cx["_dzien"]) \
                                         .collection("calls").document(_cx["_id"])
                                if True:
                                # 🟥 NIE kasujemy — oznaczamy. Nic wykonanego nie może zniknąć.
                                    _ref.update({
                                        "wynik": ("bez_rozmowy" if _val == "__usun__"
                                                  else ("archiwum" if _val == "__arch__" else _val)),
                                        "efektywny": _val in ("ustalono_termin",
                                                              "ustalono_termin_na_termin", "zwrot"),
                                        "ocena_reczna": True})
                            st.success(f"Zapisano: {_wyb} ({len(_lst_oc)} wpis(ów))")
                            st.rerun()
                        except Exception as _e:
                            st.error(f"Błąd zapisu: {_e}")

    # ══ 7. SUROWY LOG ═══════════════════════════════════════════════════════════
    with st.expander("🔎 Surowy log zdarzeń (diagnostyka)", expanded=False):
        _raw = []
        # rozmowy SPOZA wybranych dni, ale użyte do domknięcia zleceń — żeby różnica
        # między tabelą zleceń a podsumowaniami miała widoczny powód
        _spoza = []
        for _z in _deleg:
            _raw.append({"MOMENT": _mom(_z), "TYP": (_z.get("typ") or "zlecenie").upper(),
                         "NR": _z.get("numer_zamowienia", ""), "KTO": _z.get("zlecil", ""),
                         "DO KOGO": _z.get("do_kogo", ""), "WYNIK": "", "ŹRÓDŁO": "",
                         "GRUPA": _kraj(_z), "ID POSTU": _z.get("id_postu", ""), "WYKRYTE": "—"})
        for _c in _calls_pelne:
            _raw.append({"MOMENT": _mom(_c), "TYP": ("TELEFON" if (_c.get("wynik") or "") != "bez_rozmowy"
                                                    else "WPIS BEZ ROZMOWY"),
                         "NR": _c.get("numer_zamowienia", ""),
                         "KTO": _kto_disp(_c), "DO KOGO": "", "WYNIK": _c.get("wynik", ""),
                         "ŹRÓDŁO": _c.get("zrodlo", ""), "GRUPA": _kraj(_c),
                         "ID POSTU": str(_c.get("zlecenie_post_id") or ""),
                         "WYKRYTE": (_c.get("wykryte_dnia") or "—")})
        st.dataframe(pd.DataFrame(sorted(_raw, key=lambda r: r["MOMENT"], reverse=True),
                                  columns=["MOMENT", "TYP", "NR", "KTO", "DO KOGO", "WYNIK", "ŹRÓDŁO",
                                           "GRUPA", "ID POSTU", "WYKRYTE"]),
                     use_container_width=True, hide_index=True)
        st.caption(f"Zdarzeń: {len(_raw)} · zleceń/ponagleń: {len(_deleg)} · telefonów: {len(_calls)}"
                   + (f" · wpisów bez rozmowy: {len(_bez_rozmowy)}" if _bez_rozmowy else "")
                   + (f" · rozmów spoza zakresu (użytych przy zleceniach): {len(_spoza)}" if _spoza else "")
                   + f" · obsada: {len(_obsada)}")
        st.caption("Log pokazuje WSZYSTKIE zdarzenia z zaznaczonych dni i pokrywa się z tabelami wyżej. "
                   "Rozmowa telefonistki należy do dnia, w którym operator ją przerobił — godzina "
                   "pochodzi z wpisu na forum i służy do liczenia czasu reakcji.")

    # ══ 8. CZYSZCZENIE DANYCH ═══════════════════════════════════════════════════
    with st.expander("🗑️ Wyczyść dane telefoniczne (nieodwracalne)", expanded=False):
        st.warning(f"Usunie telefony, zlecenia i obsadę z dni **{_t_from} — {_t_to}**. "
                   f"Służy do skasowania błędnych wpisów sprzed poprawki. Operacji NIE da się cofnąć.")
        _pot = st.checkbox("Rozumiem, że danych nie da się odzyskać", key="_tel_clr_ok")
        if st.button("🗑️ Wyczyść dane z tego zakresu", key="_tel_clr", disabled=not _pot):
            _ile = 0
            for _ds in _dni:
                for _sub in ("calls", "delegacje", "obsada"):
                    try:
                        for _d in db.collection(col("ew_phone_log")).document(_ds).collection(_sub).stream():
                            _d.reference.delete()
                            _ile += 1
                    except Exception as _e:
                        st.error(f"Błąd przy {_ds}/{_sub}: {_e}")
            st.success(f"✅ Usunięto {_ile} wpisów z zakresu {_t_from} — {_t_to}.")
            st.rerun()
