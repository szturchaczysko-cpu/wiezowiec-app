...
# ==========================================
# 📞 REJESTR ZLECONYCH TELEFONÓW
# ==========================================
with tab_telefony:
    st.subheader("📞 Rejestr zleconych telefonów")
    st.caption("Każdy telefon zlecony telefonistom na forum = jeden wiersz. Wypełnia się automatycznie przy "
               "delegacji (ID postu, kiedy, do kogo, kto zlecił) i przy wyniku telefonu. Imiennie „przez kogo” "
               "znamy tylko dla telefonów wykonanych z woreczka (operator zalogowany) — telefon wykonany przez "
               "telefonistę uzupełniasz ręcznie niżej.")
    ...
    # Godziny pracy do liczenia SLA (zlecenie o 16:50 nie jest „spóźnione” o poranku).
    TEL_WORK_START, TEL_WORK_END = 8, 16
    _TZ_TEL = pytz.timezone("Europe/Warsaw")

    def _tel_biz_seconds(_a, _b):
        """Sekundy ROBOCZE między dwoma momentami (pon-pt, godziny pracy)."""
        if not _a or not _b or _b <= _a:
            return 0
        _tot, _cur = 0, _a
        for _ in range(400):                     # bezpiecznik pętli
            if _cur >= _b:
                break
            _day_s = _cur.replace(hour=TEL_WORK_START, minute=0, second=0, microsecond=0)
            _day_e = _cur.replace(hour=TEL_WORK_END, minute=0, second=0, microsecond=0)
            if _cur.weekday() < 5:               # pon-pt
                _s = max(_cur, _day_s)
                _e = min(_b, _day_e)
                if _e > _s:
                    _tot += (_e - _s).total_seconds()
            _cur = (_cur + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        return _tot

    _SLA_2H = 2 * 3600
    _SLA_2DNI = 2 * (TEL_WORK_END - TEL_WORK_START) * 3600     # 2 dni robocze

    def _tel_local(_v):
        try:
            return _v.astimezone(_TZ_TEL)
        except Exception:
            return None

    # ── wczytanie rejestru ──
    try:
        _tel_docs = list(db.collection(col(TEL_REG_COL)).limit(3000).stream())
    except Exception as _e:
        _tel_docs = []
        st.error(f"Nie udało się wczytać rejestru: {_e}")

    _rows = []
    for _d in _tel_docs:
        _x = _d.to_dict() or {}
        _zl = _tel_local(_x.get("data_zlecenia"))
        _pod = _tel_local(_x.get("podjete_at"))
        _wyk = _tel_local(_x.get("wykonane_at")) or _pod
        _s2h = _s2d = ""
        if _zl and _pod:
            _sec = _tel_biz_seconds(_zl, _pod)
            _h, _m = int(_sec // 3600), int((_sec % 3600) // 60)
            _s2h = ("✅ " if _sec <= _SLA_2H else "❌ ") + f"{_h}:{_m:02d}"
        elif _zl:
            _s2h = "⏳ niepodjęty"
        if _zl and _wyk:
            _secd = _tel_biz_seconds(_zl, _wyk)
            _s2d = "✅ w 2 dni" if _secd <= _SLA_2DNI else "❌ po terminie"
        elif _zl:
            _s2d = "⏳ otwarty"
        _rows.append({
            "_id": _d.id,
            "NR ZAMÓWIENIA": _x.get("numer_zamowienia", ""),
            "ID POSTU": _x.get("id_postu", ""),
            "DATA / GODZINA": _zl.strftime("%d.%m.%y %H:%M") if _zl else "",
            "_dt": _zl,
            "DO KOGO": _x.get("do_kogo", ""),
            "GRUPA OP.": _x.get("grupa_operatorska", ""),
            "CZY PODJĘTE / przez kogo": (
                (f"TAK {_pod.strftime('%d.%m.%y %H:%M')}" if _pod else "TAK")
                + (f" / {_x.get('podjete_przez')}" if _x.get("podjete_przez") else "")
            ) if _x.get("podjete") else "NIE",
            "STATUS": _x.get("status", ""),
            "USTALENIA": _x.get("ustalenia", ""),
            "SLA 2h": _s2h,
            "W TERMINIE (2 dni)": _s2d,
            "ZLECIŁ": _x.get("zlecil", ""),
            "JĘZYK": _x.get("jezyk", ""),
        })

    if not _rows:
        st.info("Rejestr jest pusty — pojawią się tu telefony zlecone telefonistom po wdrożeniu nowej wersji Szturchacza.")
    else:
        # ── FILTRY ──
        st.markdown("#### 🔎 Filtry")
        _f1, _f2, _f3 = st.columns([2, 2, 2])
        with _f1:
            _dflt_from = (datetime.now(_TZ_TEL) - timedelta(days=7)).date()
            _d_from = st.date_input("Od dnia:", value=_dflt_from, key="_tel_from")
            _d_to = st.date_input("Do dnia:", value=datetime.now(_TZ_TEL).date(), key="_tel_to")
        with _f2:
            _grupy = sorted({r["GRUPA OP."] for r in _rows if r["GRUPA OP."]})
            _f_grupa = st.multiselect("Grupa operatorska:", _grupy, key="_tel_fg")
            _dokogo = sorted({r["DO KOGO"] for r in _rows if r["DO KOGO"]})
            _f_dokogo = st.multiselect("Grupa telefonistów:", _dokogo, key="_tel_fd")
        with _f3:
            _statusy = sorted({r["STATUS"] for r in _rows if r["STATUS"]} | set(TEL_STATUSY))
            _f_status = st.multiselect("Status:", _statusy, key="_tel_fs")
            _f_sla2h = st.selectbox("SLA 2h:", ["wszystkie", "✅ w terminie", "❌ przekroczone", "⏳ niepodjęte"], key="_tel_f2h")
            _f_sla2d = st.selectbox("Termin 2 dni:", ["wszystkie", "✅ w terminie", "❌ po terminie", "⏳ otwarte"], key="_tel_f2d")

        def _keep(_r):
            if _r["_dt"]:
                _dd = _r["_dt"].date()
                if _d_from and _dd < _d_from:
                    return False
                if _d_to and _dd > _d_to:
                    return False
            if _f_grupa and _r["GRUPA OP."] not in _f_grupa:
                return False
            if _f_dokogo and _r["DO KOGO"] not in _f_dokogo:
                return False
            if _f_status and _r["STATUS"] not in _f_status:
                return False
            if _f_sla2h != "wszystkie":
                _v = _r["SLA 2h"]
                if _f_sla2h == "✅ w terminie" and not _v.startswith("✅"):
                    return False
                if _f_sla2h == "❌ przekroczone" and not _v.startswith("❌"):
                    return False
                if _f_sla2h == "⏳ niepodjęte" and not _v.startswith("⏳"):
                    return False
            if _f_sla2d != "wszystkie":
                _v = _r["W TERMINIE (2 dni)"]
                if _f_sla2d == "✅ w terminie" and not _v.startswith("✅"):
                    return False
                if _f_sla2d == "❌ po terminie" and not _v.startswith("❌"):
                    return False
                if _f_sla2d == "⏳ otwarte" and not _v.startswith("⏳"):
                    return False
            return True

        _flt = [r for r in _rows if _keep(r)]
        _flt.sort(key=lambda r: r["_dt"] or datetime.now(_TZ_TEL), reverse=True)

        # ── PODSUMOWANIE ──
        _n = len(_flt)
        _npod = sum(1 for r in _flt if r["CZY PODJĘTE / przez kogo"] != "NIE")
        _n2h = sum(1 for r in _flt if r["SLA 2h"].startswith("✅"))
        _n2d = sum(1 for r in _flt if r["W TERMINIE (2 dni)"].startswith("✅"))
        _m1, _m2, _m3, _m4 = st.columns(4)
        _m1.metric("Zlecone telefony", _n)
        _m2.metric("Podjęte", f"{_npod}" + (f" ({round(100*_npod/_n)}%)" if _n else ""))
        _m3.metric("W SLA 2h", f"{_n2h}" + (f" ({round(100*_n2h/_n)}%)" if _n else ""))
        _m4.metric("W terminie 2 dni", f"{_n2d}" + (f" ({round(100*_n2d/_n)}%)" if _n else ""))

        _cols_show = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "GRUPA OP.",
                      "CZY PODJĘTE / przez kogo", "STATUS", "USTALENIA", "SLA 2h", "W TERMINIE (2 dni)",
                      "ZLECIŁ", "JĘZYK"]
        _df = pd.DataFrame([{k: r[k] for k in _cols_show} for r in _flt])
        st.dataframe(_df, use_container_width=True, hide_index=True)
        st.download_button("⬇️ Pobierz CSV", _df.to_csv(index=False).encode("utf-8"),
                           file_name=f"rejestr_telefonow_{_d_from}_{_d_to}.csv", mime="text/csv",
                           key="_tel_csv")

        # ── RĘCZNE UZUPEŁNIENIE (telefon wykonany przez telefonistę) ──
        st.markdown("---")
        st.markdown("#### ✏️ Uzupełnij wpis (telefon wykonany przez telefonistę)")
        _opts = {f'{r["NR ZAMÓWIENIA"]} · post {r["ID POSTU"]} · {r["DATA / GODZINA"]}': r for r in _flt[:200]}
        if _opts:
            _sel = st.selectbox("Wybierz telefon:", list(_opts.keys()), key="_tel_edit_sel")
            _r = _opts[_sel]
            _e1, _e2 = st.columns([2, 3])
            with _e1:
                _kto = st.text_input("Podjęte przez (imię telefonisty):", key="_tel_edit_kto")
                _st_new = st.selectbox("Status:", TEL_STATUSY,
                                       index=TEL_STATUSY.index(_r["STATUS"]) if _r["STATUS"] in TEL_STATUSY else 0,
                                       key="_tel_edit_status")
            with _e2:
                _ust = st.text_area("Co ustalono:", value=_r["USTALENIA"], height=90, key="_tel_edit_ust")
            if st.button("💾 Zapisz uzupełnienie", type="primary", key="_tel_edit_save"):
                try:
                    _upd = {"status": _st_new, "ustalenia": _ust}
                    if _kto.strip():
                        _upd.update({"podjete": True, "podjete_przez": _kto.strip(),
                                     "podjete_at": firestore.SERVER_TIMESTAMP,
                                     "wykonane_at": firestore.SERVER_TIMESTAMP})
                    db.collection(col(TEL_REG_COL)).document(_r["_id"]).update(_upd)
                    st.success("✅ Zapisano.")
                    st.rerun()
                except Exception as _e:
                    st.error(f"❌ Błąd zapisu: {_e}")
