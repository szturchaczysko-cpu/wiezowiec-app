...
with tab_telefony:
    ...
    _STAT = {"brak_kontaktu": "NIE ODBIERA", "kontakt_bez_konkretu": "ODEBRAŁ, BEZ USTALEŃ",
             "przelozenie": "TERMIN NA TERMIN", "konkret": "USTALONO"}

    def _status(_c):
        if not _c:
            return ""
        _w = _c.get("wynik") or ""
        return "ZAMÓWIONY" if (_w == "konkret" and _c.get("kurier_ustalony")) else _STAT.get(_w, _w)

    def _z_ust(_c):
        return (_c.get("wynik") or "") in ("konkret", "przelozenie")
    ...
    for _k in _kraje:
        ...
        _agg = {}
        for _c in _cl_k:
            _key = (_kto(_c), _kraj(_c))
            _a = _agg.setdefault(_key, {"Kto": _kto(_c),
                                        "Typ": ("grupa telefonistek" if _grupowy(_c) else f"operator {_kraj(_c)}"),
                                        "Języki": ("—" if _grupowy(_c) else "/".join(_jez_map.get(_key, [])) or "—"),
        ...
        for _v in sorted(_agg.values(), key=lambda x: -x["Telefony"]):
            _rows.append({**_v, "Udział": f'{round(100*_v["Telefony"]/_baza)}%',
                          "Skuteczność": f'{round(100*_v["Z ustaleniami"]/_v["Telefony"])}%' if _v["Telefony"] else "—"})
        if _nw_k:
            _rows.append({"Kto": "⚠️ NIEWYKONANE", "Typ": "—", "Języki": "—", "Telefony": len(_nw_k),
                          "Z ustaleniami": 0, "Udział": f'{round(100*len(_nw_k)/_baza)}%', "Skuteczność": "—"})
        st.markdown(f"**{_k}** — do zrobienia: {_baza}")
        st.dataframe(pd.DataFrame(_rows), use_container_width=True, hide_index=True)

    # ══ 2. TELEFONY WYKONANE ════════════════════════════════════════════════════
    _K1 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "CZY PODJĘTE / przez kogo",
    ...
    for _c in sorted(_calls, key=_mom, reverse=True):
        ...
        _r1.append({
            ...
            "ID POSTU": ("forum" if _g else "sesja"),
            "DATA / GODZINA": _mom(_c),
            "DO KOGO": _kto(_c),
            "CZY PODJĘTE / przez kogo": f'TAK {_mom(_c)} / {_kto(_c)}',
            "STATUS": _status(_c),
            "CZY W TERMINIE": ("—" if _g else "TAK (od razu)"),
            ...
            "UWAGI": (f'wykryte {_c.get("wykryte_dnia")}' if _c.get("wykryte_dnia")
                      else ("telefon owocny" if _z_ust(_c) else "bez ustaleń")),
        })
    ...
    _K2 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "CZY PODJĘTE / przez kogo",
           "STATUS", "CZY W TERMINIE", "TELEFONÓW", "ZLECIŁ", "GRUPA", "UWAGI"]
    ...
    for _z in sorted(_deleg, key=_mom, reverse=True):
        ...
        if _cc:
            _podj = (f'TAK {_mom(_cc)} / {_kto(_cc)}' if _grupowy(_cc)
                     else f'NIE / zadzwonił {_kto(_cc)} {_mom(_cc)}')
            if _t0 and _t1:
                _mn = (_t1 - _t0).total_seconds() / 60
                _dd = f'{int(_mn//60)}h {int(_mn%60)}min'
                _term = (f'✅ ten sam dzień (+{_dd})' if _cc.get("_dzien") == _z.get("_dzien")
                         else (f'⚠️ po terminie (+{_dd})' if _mn > 2*24*60 else f'❌ inny dzień (+{_dd})'))
            else:
                _term = "—"
        else:
            _podj = "⏳ czeka na aktualizację"
            _prz = ((_now - _t0).total_seconds()/3600) if _t0 else 0
            _term = ("⚠️ przeterminowane (>2 dni)" if _prz > 48 else "⏳ otwarte")
        _r2.append({
            "NR ZAMÓWIENIA": _z.get("numer_zamowienia", ""), "ID POSTU": _z.get("id_postu", ""),
            "DATA / GODZINA": _mom(_z), "DO KOGO": _z.get("do_kogo", ""),
            "CZY PODJĘTE / przez kogo": _podj, "STATUS": _status(_cc), "CZY W TERMINIE": _term,
            "TELEFONÓW": _z.get("_ile_tel", 0), "ZLECIŁ": _z.get("zlecil", ""), "GRUPA": _kraj(_z),
            "UWAGI": ("ponaglenie" if _z.get("typ") == "ponaglenie" else "zlecenie")
    ...
    for _k in _kraje:
        ...
        for _g in sorted({_kto(c) for c in _cl_k if _grupowy(c)}):
            ...
            _zu = sum(1 for c in _lst if _z_ust(c))
            _rows_w.append({"Kto": _g, "Typ": "grupa telefonistek", "Języki": "—", "Rozmowy": len(_lst),
                            "Jakość %": (round(100*_zu/len(_lst)) if _lst else 0),
                            "Wykonalność %": (round(100*len(_lst)/_suma_k) if _suma_k else 0),
        ...
        _wykres_grup(sorted(_rows_w, key=lambda r: -r["Wykonalność %"]), _k)
