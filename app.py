...
with tab_telefony:
    ...
    def _oceniony(_c):
        ...
        return (_c.get("wynik") or "") not in ("do_oceny", "archiwum")
    ...
    for _k in _kraje:
        ...
        for (_op, _gr) in sorted(_dzw_set):
            ...
            _rows_w.append({"Kto": _op, "Rozmowy": len(_lst),
                            ...
                            "Jakość": (f"{_jp}% ({_joc} z {len(_lst)})" if _joc else "⏳ czeka na ocenę"),
    ...
    for _k in _kraje:
        ...
        for _lbl, _lst_g in (("Telefonistki", _tel), ("Operatorzy dzwoniący", _ope)):
            ...
            _rows_vs.append({
                ...
                "Jakość": (f"{_jg}% ({_ocg} z {len(_lst_g)})" if _ocg else "⏳ czeka"),
            })
    if _rows_vs:
    ...
    try:
        _diam = _fetch_diamonds_range(_t_from.strftime("%Y-%m-%d"), _t_to.strftime("%Y-%m-%d"), _COL_PREFIX)
        _diam_nr = {str(r.get("numer") or "") for r in _diam if _is_diament_row(r) and r.get("numer")}
    except Exception:
        _diam_nr = set()
    ...
    for _c in _calls:
        ...
        if _prev is None or _mom(_c) > _mom(_prev):
            _kur_by_nr[_nr_k] = _c
    _diamenty = [c for nr, c in _kur_by_nr.items() if nr in _diam_nr] if _diam_nr else []
    _zapowiedzi = [c for nr, c in _kur_by_nr.items() if nr not in _diam_nr]
