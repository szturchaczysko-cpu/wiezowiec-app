...
with tab_telefony:
    ...
    for _ds in _oc_dni:
        try:
            for _d in db.collection(col("ew_phone_log")).document(_ds).collection("calls").stream():
                ...
                _v["_dzien"] = _ds
                _v["_id"] = _d.id
                # 🟥 DWA warunki naraz: operator PRZEROBIŁ sprawę w wybranych dniach ORAZ
                #    automat nie zdołał ocenić rozmowy. Zlecenia bez przerobienia tu nie trafiają —
                #    nie ma czego oceniać, skoro nikt sprawy nie ruszał.
    ...
    if _oc_stare:
        with st.expander(f"🧹 Rozmowy bez oceny spoza wybranych dni ({_oc_stare}) — do przejrzenia",
                         expanded=False):
            st.caption("Rozmowy, których nikt nie przerabiał w wybranym zakresie — najczęściej historia "
                       "wątku wciągnięta przy pierwszym otwarciu sprawy. Obejrzyj i zdecyduj sama; "
