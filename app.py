...
def build_autopilot_queue(percent, obsada, ap_work_date_str):
    ...
    all_wolne_docs = db.collection("ew_cases").where("status", "==", "wolny").get()
    wolne = []
    ...
    if batches_to_process:
        ...
        if batches_done < total_batches:
            ...
            if autopilot_pct > 0 and gen_obsada and not st.session_state.get("_ew_autopilot_started"):
                ...
                if ap_state == "idle":
                    st.markdown("---")
                    st.markdown(f"### 🤖 Autopilot — gotowy do przeliczenia **{autopilot_pct}%** casów")
                    
                    # Pokaż ile casów per grupa
                    preview_wolne = db.collection("ew_cases").where("status", "==", "wolny").get()
                    preview_counts = {"DE": 0, "FR": 0, "UKPL": 0}
                    preview_total = 0
                    for pdoc in preview_wolne:
                        pd_data = pdoc.to_dict()
                        if pd_data.get("autopilot_status") != "calculated":
                            g = pd_data.get("grupa", "")
                            if g in preview_counts:
                                preview_counts[g] += 1
                                preview_total += 1
                    
                    target = max(1, int(preview_total * autopilot_pct / 100))
                    st.info(f"📋 Wolnych bez przeliczenia: **{preview_total}** (DE: {preview_counts['DE']}, FR: {preview_counts['FR']}, UKPL: {preview_counts['UKPL']})\n\n"
                            f"🎯 Do przeliczenia: **{target}** casów ({autopilot_pct}%, globalnie po score)")
                    
                    if st.button("🤖 Uruchom autopilot", type="primary", key="auto_start_ap"):
                        ...
                        if case_queue:
                            ...
                            st.success(f"🤖 Autopilot uruchomiony: {len(case_queue)} casów w kolejce!")
                            st.rerun()
                        else:
                            st.warning("Brak casów do przeliczenia (brak obsady lub 0 wolnych).")
                elif ap_state == "running":
                    st.info("🤖 Autopilot już działa — przejdź do zakładki **Dolewka + Status**.")
...
with tab_autopilot:
    ...
    bak_docs = db.collection("ew_cases").where("status", "==", "wolny").get()
    ...
    if any(v > 0 for v in dolewka_pcts.values()):
        ...
        if state == "running":
            ...
            obsada = st.session_state.get("_gen_obsada", {})
            if not obsada:
                st.error("⚠️ Brak obsady — ustaw operatorów w zakładce 'Generuj + Autopilot'.")
            else:
                ...
                for g, pct in dolewka_pcts.items():
                    if pct <= 0 or g not in obsada or not obsada[g]:
                        continue
                    # Znajdź nieprzeliczone wolne casy w tej grupie
                    g_cases = []
                    for bdoc in bak_docs:
                        d = bdoc.to_dict()
                ...
                if dolewka_queue:
                    work_date_str = ""
                    dob = st.session_state.get("_gen_data_obrobki")
                    if dob:
                        work_date_str = dob.strftime('%d.%m')
                    else:
                        work_date_str = datetime.now(pytz.timezone('Europe/Warsaw')).strftime('%d.%m')
                    
                    ap_prompt_url = st.session_state.get("_gen_ap_prompt_url", "")
                    ap_prompt_name = st.session_state.get("_gen_ap_prompt_name", "")
                    ap_model = st.session_state.get("_gen_ap_model", "gemini-2.5-pro")
                    ap_pause = st.session_state.get("_gen_ap_pause", 30)
                    ap_key_indices = st.session_state.get("_gen_ap_key_indices", [0])
                    ...
                    set_autopilot_status({
                        ...
                        "tryb": "od_szturchacza",
                        "key_indices": ap_key_indices,
                        "obsada": {g: ops for g, ops in obsada.items()},
                        "started_at": firestore.SERVER_TIMESTAMP,
                    })
                    db.collection("autopilot_config").document("queue").set({
                        "cases": dolewka_queue,
