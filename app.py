...
with tab_telefony:
    ...
    # ══ 2. TELEFONY WYKONANE ════════════════════════════════════════════════════
    _K1 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "KTO DZWONIŁ", "DO KOGO",
           "STATUS", "CZY W TERMINIE", "PRÓBA", "GRUPA", "UWAGI"]
    ...
    for _c in sorted(_calls_op, key=_mom, reverse=True):
        ...
        # czy w terminie = 2h ROBOCZE (9-17) od zlecenia, które ten telefon wykonał
        _zl_src = None
        for _d in _dbn.get(str(_c.get("numer_zamowienia") or "").strip(), []):
            if _mom(_d) <= _mom(_c) and _c.get("_wpis_po") is not _d:
                if _zl_src is None or _mom(_d) > _mom(_zl_src):
                    _zl_src = _d
        if _zl_src:
            _bm = _biz_min(_dt(_zl_src), _dt(_c))
            _term1 = (f'✅ w 2h ({int(_bm//60)}h {int(_bm%60)}min)' if _bm <= 120
                      else f'❌ po 2h ({int(_bm//60)}h {int(_bm%60)}min)')
        else:
            _term1 = "— (bez zlecenia)"
        _r1.append({
            ...
            "DO KOGO": _do_kogo,
            "STATUS": _status(_c),
            "CZY W TERMINIE": _term1,
            "PRÓBA": (f'{_c.get("_proba",1)} z {_c.get("_prazem",1)}' if _c.get("_prazem", 1) > 1 else "1"),
            "GRUPA": _kraj(_c),
            "UWAGI": (f'wykryte {_c.get("wykryte_dnia")}' if _c.get("wykryte_dnia")
                      else ("telefon owocny" if _z_ust(_c)
                            else ("nieudany → wpis na forum + woreczek" if _po_wpis else "bez ustaleń"))),
        })
    ...
    # ══ 3. ZLECENIA NA FORUM ════════════════════════════════════════════════════
    _K2 = ["NR ZAMÓWIENIA", "ID POSTU", "DATA / GODZINA", "DO KOGO", "CZY PODJĘTE / przez kogo",
           "STATUS", "TELEFONÓW", "ZLECIŁ", "GRUPA", "UWAGI"]
    st.markdown("### 📨 Zlecenia telefonu na forum")
    _r2 = []
    for _z in sorted(_deleg, key=_mom, reverse=True):
        _cc = _z.get("_call")
        ...
        _r2.append({
            ...
            "TELEFONÓW": _z.get("_ile_tel", 0), "ZLECIŁ": _z.get("zlecil", ""), "GRUPA": _kraj(_z),
            "UWAGI": _uw,
        })
    ...
    _dzw_set = {(_o.get("operator"), _kraj(_o)) for _o in _obsada if _o.get("dzwoni")}
    if not _obsada:
