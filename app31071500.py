...
with tab_telefony:
    ...
    _dni_hist = _dni_range(_t_from - timedelta(days=14), _t_to, _extra=5)
    ...
    # ══ 5. PODZIAŁ NA KRAJE ═════════════════════════════════════════════════════
    st.markdown("### 🌍 Podział na kraje")
    _rk = []
    for _k in _kraje:
        _zl_k = [z for z in _zlecenia if _kraj(z) == _k]
        _cl_k = [c for c in _calls if _kraj(c) == _k]
        _nw_k = [z for z in _niewyk if _kraj(z) == _k]
        _zu_k = sum(1 for c in _cl_k if _z_ust(c))
        _rk.append({"Kraj": _k, "Zlecone": len(_zl_k), "Telefony": len(_cl_k), "Niewykonane": len(_nw_k),
                    "Skuteczność": (f"{round(100*_zu_k/len(_cl_k))}%" if _cl_k else "—")})
    if _rk:
    ...
    _oc_dni = _dni_range(_t_to - timedelta(days=30), _t_to, _extra=5)
    ...
    if _do_oceny:
        with st.expander(f"✍️ Oceń rozmowy ręcznie ({len(_do_oceny)})", expanded=True):
            ...
            for _c in sorted(_do_oceny, key=_mom):
                ...
                with _e1:
                    st.markdown(f'**{_c.get("numer_zamowienia","?")}** · {_c.get("operator","?")} / {_os}'
                                f' · {_mom(_c)} · {_kraj(_c)}')
                with _e2:
                    _wyb = st.selectbox("Status:", list(_OPCJE.keys()),
                                        key=f'_oc_{_c.get("_dzien")}_{_c.get("_id")}',
                                        label_visibility="collapsed")
                with _e3:
                    if st.button("💾", key=f'_ocb_{_c.get("_dzien")}_{_c.get("_id")}',
                                 help="Zapisz status tej rozmowy"):
                        try:
                            _ref = db.collection(col("ew_phone_log")).document(_c["_dzien"]) \
                                     .collection("calls").document(_c["_id"])
                            _val = _OPCJE[_wyb]
                            if _val == "__usun__":
                                # 🟥 NIE kasujemy — oznaczamy. Nic wykonanego nie może zniknąć.
                                _ref.update({"wynik": "bez_rozmowy", "efektywny": False,
                                             "ocena_reczna": True})
                                st.success("Oznaczono jako wpis bez rozmowy (zostaje w logu, poza statystyką).")
                            else:
                                _ref.update({"wynik": _val,
                                             "efektywny": _val in ("ustalono_termin", "ustalono_termin_na_termin"),
                                             "ocena_reczna": True})
                                st.success(f"Zapisano: {_wyb}")
                            st.rerun()
                        except Exception as _e:
                            st.error(f"Błąd zapisu: {_e}")
    ...
    with st.expander("🔎 Surowy log zdarzeń (diagnostyka)", expanded=False):
        ...
        for _z in _zlec_only:
            _cc = _z.get("_call")
            if _cc and _cc.get("_dzien") not in _dni_set:
                _spoza.append(_cc)
        for _c in _spoza:
            _raw.append({"MOMENT": _mom(_c), "TYP": "TELEFON (spoza zakresu)",
                         "NR": _c.get("numer_zamowienia", ""), "KTO": _kto_disp(_c), "DO KOGO": "",
                         "WYNIK": _c.get("wynik", ""), "ŹRÓDŁO": _c.get("zrodlo", ""),
                         "GRUPA": _kraj(_c), "ID POSTU": str(_c.get("zlecenie_post_id") or "")})
        for _z in _deleg:
        ...
        st.caption("Ten log pokazuje WSZYSTKO z wybranych dni. Do liczb w podsumowaniach nie wchodzą: "
                   "wpisy bez rozmowy oraz rozmowy spoza zakresu — są tu widoczne z własnym oznaczeniem.")
